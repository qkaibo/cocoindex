...
` + e.stack) : e;
	...
	}, oe = class Oo extends Error {
		constructor(t) {
			super(t || "An unexpected bug occurred."), Object.setPrototypeOf(this, Oo.prototype);
		}
	};
	...
	var Pt = class Fo {
		...
		add(t) {
			if (!t || t === Ye.None) return t;
			if (t === this) throw new Error("Cannot register a disposable on itself!");
			return this._isDisposed ? Fo.DISABLE_DISPOSED_WARNING || console.warn((/* @__PURE__ */ new Error("Trying to add a disposable to a DisposableStore that has already been disposed of. The added object will be leaked!")).stack) : this._toDispose.add(t), t;
		}
		delete(t) {
			if (t) {
	...
	Gi = Pt, Gi.DISABLE_DISPOSED_WARNING = !1;
	var Ye = class {
		constructor() {
	...
	Xi = Ye, Xi.None = Object.freeze({ dispose() {} });
	var Sn, Z = class Hi {
		constructor(t) {
			this.element = t, this.next = Hi.Undefined, this.prev = Hi.Undefined;
		}
	};
	Sn = Z, Sn.Undefined = new Sn(void 0);
	var hl = class {
		constructor() {
	...
	const ul = globalThis.performance.now.bind(globalThis.performance);
	var Yi = class Bo {
		static create(t) {
			return new Bo(t);
		}
		constructor(t) {
			this._now = t === !1 ? Date.now : ul, this._startTime = this._now(), this._stopTime = -1;
		}
		stop() {
	...
	(function(e) {
		...
		e.signal = o;
		function l(...S) {
			return (A, y = null, R) => d(ll(...S.map((N) => N((H) => A.call(y, H)))), R);
		}
		e.any = l;
		function c(S, A, y, R) {
			let N = y;
		...
		function m(S, A, y = 100, R = !1, N = !1, H, q) {
			...
			const Ie = new Se({
				...
				onWillAddFirstListener() {
					P = S((yd) => {
						K++, W = A(W, yd), R && !X && (Ie.fire(W), W = void 0), ae = () => {
							const Td = W;
							W = void 0, X = void 0, (!R || K > 1) && Ie.fire(Td), K = 0;
						}, typeof y == "number" ? (X && clearTimeout(X), X = setTimeout(ae, y)) : X === void 0 && (X = null, queueMicrotask(ae));
					});
				},
		...
		function I(S, A, y = (R) => R) {
			const R = (...P) => q.fire(y(...P)), N = () => S.on(A, R), H = () => S.removeListener(A, R), q = new Se({
		...
		function D(S, A, y = (R) => R) {
			const R = (...P) => q.fire(y(...P)), N = () => S.addEventListener(A, R), H = () => S.removeEventListener(A, R), q = new Se({
				onWillAddFirstListener: N,
				onDidRemoveLastListener: H
			});
			return q.event;
		...
		class C {
			constructor(A, y) {
				...
				this.emitter = new Se(R), y && y.add(this.emitter);
			}
			beginUpdate(A) {
				this._counter++;
			}
		...
		function L(S) {
			return (A, y, R) => {
				...
				return R instanceof Pt ? R.add(P) : Array.isArray(R) && R.push(P), P;
			};
		}
	...
	var Zi = class Ui {
		constructor(t) {
			this.listenerCount = 0, this.invocationCount = 0, this.elapsedOverall = 0, this.durations = [], this.name = `${t}_${Ui._idPool++}`, Ui.all.add(this);
		}
		start(t) {
			this._stopWatch = new Yi(), this.listenerCount = t;
		}
		stop() {
			if (this._stopWatch) {
	...
	Ln = Zi, Ln.all = /* @__PURE__ */ new Set(), Ln._idPool = 0;
	let dl = -1;
	var Ki = class Vo {
		constructor(t, n, r = (Vo._idPool++).toString(16).padStart(3, "0")) {
	...
	var ml = class jo {
		static create() {
			return new jo((/* @__PURE__ */ new Error()).stack ?? "");
		}
		constructor(t) {
			this.value = t;
		}
...
`));
	...
	var Se = class {
		constructor(e) {
			this._size = 0, this._options = e, this._leakageMon = this._options?.leakWarningThreshold ? new Ki(e?.onListenerError ?? Ht, this._options?.leakWarningThreshold ?? dl) : void 0, this._perfMon = this._options?._profName ? new Zi(this._options._profName) : void 0, this._deliveryQueue = this._options?.deliveryQueue;
		}
		dispose() {
		...
		get event() {
			return this._event ?? (this._event = (e, t, n) => {
				if (this._leakageMon && this._size > this._leakageMon.threshold ** 2) {
					...
					console.warn(a);
					const o = this._leakageMon.getMostFrequentStack() ?? ["UNKNOWN stack", -1], l = new pl(`${a}. HINT: Stack shows most frequent listener (${o[1]}-times)`, o[0]);
					return (this._options?.onListenerError || Ht)(l), Ye.None;
				}
				if (this._disposed) return Ye.None;
				t && (e = e.bind(t));
				const r = new An(e);
				let i;
				this._leakageMon && this._size >= Math.ceil(this._leakageMon.threshold * .2) && (r.stack = ml.create(), i = this._leakageMon.check(r.stack, this._size + 1)), this._listeners ? this._listeners instanceof An ? (this._deliveryQueue ?? (this._deliveryQueue = new bl()), this._listeners = [this._listeners, r]) : this._listeners.push(r) : (this._options?.onWillAddFirstListener?.(this), this._listeners = r, this._options?.onDidAddFirstListener?.(this)), this._options?.onDidAddListener?.(this), this._size++;
				const s = Wt(() => {
					i?.(), this._removeListener(r);
				});
				return n instanceof Pt ? n.add(s) : Array.isArray(n) && n.push(s), s;
			}), this._event;
		}
		_removeListener(e) {
		...
		_deliver(e, t) {
			if (!e) return;
			const n = this._options?.onListenerError || Ht;
			if (!n) {
				e.value(t);
				return;
			}
		...
		fire(e) {
			if (this._deliveryQueue?.current && (this._deliverQueue(this._deliveryQueue), this._perfMon?.stop()), this._perfMon?.start(this._size), this._listeners) if (this._listeners instanceof An) this._deliver(this._listeners, e);
			else {
	...
	const _l = er() === "pseudo" || typeof document < "u" && document.location && typeof document.location.hash == "string" && document.location.hash.indexOf("pseudo=true") >= 0;
	function tr(e, t) {
		let n;
		...
		}), _l && (n = "［" + n.replace(/[aouei]/g, "$&$&") + "］"), n;
	}
	function F(e, t, ...n) {
		return tr(typeof e == "number" ? vl(e, t) : t, n);
	}
	function vl(e, t) {
		const n = wl()?.[e];
		if (typeof n != "string") {
	...
	let qt = !1, Ot = !1, Ft = !1, nr = !1, Ae;
	const Re = globalThis;
	let ce;
	typeof Re.vscode < "u" && typeof Re.vscode.process < "u" ? ce = Re.vscode.process : typeof process < "u" && typeof process?.versions?.node == "string" && (ce = process);
	const Sl = typeof ce?.versions?.electron == "string" && ce?.type === "renderer";
	if (typeof ce == "object") {
		qt = ce.platform === "win32", Ot = ce.platform === "darwin", Ft = ce.platform === "linux", Ft && ce.env.SNAP && ce.env.SNAP_REVISION, ce.env.CI || ce.env.BUILD_ARTIFACTSTAGINGDIRECTORY || ce.env.GITHUB_WORKSPACE;
		const e = ce.env.VSCODE_NLS_CONFIG;
		if (e) try {
			const t = JSON.parse(e);
			t.userLocale, t.osLocale, t.resolvedLanguage, t.languagePack?.translationsConfigFile;
		} catch {}
	} else typeof navigator == "object" && !Sl ? (Ae = navigator.userAgent, qt = Ae.indexOf("Windows") >= 0, Ot = Ae.indexOf("Macintosh") >= 0, (Ae.indexOf("Macintosh") >= 0 || Ae.indexOf("iPad") >= 0 || Ae.indexOf("iPhone") >= 0) && navigator.maxTouchPoints && navigator.maxTouchPoints, Ft = Ae.indexOf("Linux") >= 0, Ae?.indexOf("Mobi"), nr = !0, er(), navigator.language.toLowerCase()) : console.error("Unable to resolve platform.");
	const ft = qt, Ll = Ot;
	nr && typeof Re.importScripts == "function" && Re.origin;
	const Le = Ae, xl = typeof Re.postMessage == "function" && !Re.importScripts;
	(() => {
		if (xl) {
			const e = [];
	...
	const Rl = !!(Le && Le.indexOf("Chrome") >= 0);
	Le && Le.indexOf("Firefox");
	!Rl && Le && Le.indexOf("Safari");
	Le && Le.indexOf("Edg/");
	...
	var El = class {
		constructor(e, t) {
			this.lastCache = void 0, this.lastArgKey = void 0, typeof e == "function" ? (this._fn = e, this._computeKey = Ml) : (this._fn = t, this._computeKey = e.getCacheKey);
		}
		get(e) {
	...
	var En = class {
		constructor(e) {
			this.executor = e, this._state = Oe.Uninitialized;
		}
		get value() {
			if (this._state === Oe.Uninitialized) {
				this._state = Oe.Running;
				try {
					this._value = this.executor();
				} catch (e) {
					this._error = e;
				} finally {
					this._state = Oe.Completed;
				}
			} else if (this._state === Oe.Running) throw new Error("Cannot read the value of a lazy that is being initialized");
	...
	var Vt = class Di {
		static getInstance(t) {
			return Di.cache.get(Array.from(t));
		}
		static getLocales() {
			return Di._locales.value;
		}
		constructor(t) {
	...
	Fe = Vt, Fe.ambiguousCharacterData = new En(() => JSON.parse("{\"_common\":[8232,32,8233,32,5760,32,8192,32,8193,32,8194,32,8195,32,8196,32,8197,32,8198,32,8200,32,8201,32,8202,32,8287,32,8199,32,8239,32,2042,95,65101,95,65102,95,65103,95,8208,45,8209,45,8210,45,65112,45,1748,45,8259,45,727,45,8722,45,10134,45,11450,45,1549,44,1643,44,184,44,42233,44,894,59,2307,58,2691,58,1417,58,1795,58,1796,58,5868,58,65072,58,6147,58,6153,58,8282,58,1475,58,760,58,42889,58,8758,58,720,58,42237,58,451,33,11601,33,660,63,577,63,2429,63,5038,63,42731,63,119149,46,8228,46,1793,46,1794,46,42510,46,68176,46,1632,46,1776,46,42232,46,1373,96,65287,96,8219,96,1523,96,8242,96,1370,96,8175,96,65344,96,900,96,8189,96,8125,96,8127,96,8190,96,697,96,884,96,712,96,714,96,715,96,756,96,699,96,701,96,700,96,702,96,42892,96,1497,96,2036,96,2037,96,5194,96,5836,96,94033,96,94034,96,65339,91,10088,40,10098,40,12308,40,64830,40,65341,93,10089,41,10099,41,12309,41,64831,41,10100,123,119060,123,10101,125,65342,94,8270,42,1645,42,8727,42,66335,42,5941,47,8257,47,8725,47,8260,47,9585,47,10187,47,10744,47,119354,47,12755,47,12339,47,11462,47,20031,47,12035,47,65340,92,65128,92,8726,92,10189,92,10741,92,10745,92,119311,92,119355,92,12756,92,20022,92,12034,92,42872,38,708,94,710,94,5869,43,10133,43,66203,43,8249,60,10094,60,706,60,119350,60,5176,60,5810,60,5120,61,11840,61,12448,61,42239,61,8250,62,10095,62,707,62,119351,62,5171,62,94015,62,8275,126,732,126,8128,126,8764,126,65372,124,65293,45,118002,50,120784,50,120794,50,120804,50,120814,50,120824,50,130034,50,42842,50,423,50,1000,50,42564,50,5311,50,42735,50,119302,51,118003,51,120785,51,120795,51,120805,51,120815,51,120825,51,130035,51,42923,51,540,51,439,51,42858,51,11468,51,1248,51,94011,51,71882,51,118004,52,120786,52,120796,52,120806,52,120816,52,120826,52,130036,52,5070,52,71855,52,118005,53,120787,53,120797,53,120807,53,120817,53,120827,53,130037,53,444,53,71867,53,118006,54,120788,54,120798,54,120808,54,120818,54,120828,54,130038,54,11474,54,5102,54,71893,54,119314,55,118007,55,120789,55,120799,55,120809,55,120819,55,120829,55,130039,55,66770,55,71878,55,2819,56,2538,56,2666,56,125131,56,118008,56,120790,56,120800,56,120810,56,120820,56,120830,56,130040,56,547,56,546,56,66330,56,2663,57,2920,57,2541,57,3437,57,118009,57,120791,57,120801,57,120811,57,120821,57,120831,57,130041,57,42862,57,11466,57,71884,57,71852,57,71894,57,9082,97,65345,97,119834,97,119886,97,119938,97,119990,97,120042,97,120094,97,120146,97,120198,97,120250,97,120302,97,120354,97,120406,97,120458,97,593,97,945,97,120514,97,120572,97,120630,97,120688,97,120746,97,65313,65,117974,65,119808,65,119860,65,119912,65,119964,65,120016,65,120068,65,120120,65,120172,65,120224,65,120276,65,120328,65,120380,65,120432,65,913,65,120488,65,120546,65,120604,65,120662,65,120720,65,5034,65,5573,65,42222,65,94016,65,66208,65,119835,98,119887,98,119939,98,119991,98,120043,98,120095,98,120147,98,120199,98,120251,98,120303,98,120355,98,120407,98,120459,98,388,98,5071,98,5234,98,5551,98,65314,66,8492,66,117975,66,119809,66,119861,66,119913,66,120017,66,120069,66,120121,66,120173,66,120225,66,120277,66,120329,66,120381,66,120433,66,42932,66,914,66,120489,66,120547,66,120605,66,120663,66,120721,66,5108,66,5623,66,42192,66,66178,66,66209,66,66305,66,65347,99,8573,99,119836,99,119888,99,119940,99,119992,99,120044,99,120096,99,120148,99,120200,99,120252,99,120304,99,120356,99,120408,99,120460,99,7428,99,1010,99,11429,99,43951,99,66621,99,128844,67,71913,67,71922,67,65315,67,8557,67,8450,67,8493,67,117976,67,119810,67,119862,67,119914,67,119966,67,120018,67,120174,67,120226,67,120278,67,120330,67,120382,67,120434,67,1017,67,11428,67,5087,67,42202,67,66210,67,66306,67,66581,67,66844,67,8574,100,8518,100,119837,100,119889,100,119941,100,119993,100,120045,100,120097,100,120149,100,120201,100,120253,100,120305,100,120357,100,120409,100,120461,100,1281,100,5095,100,5231,100,42194,100,8558,68,8517,68,117977,68,119811,68,119863,68,119915,68,119967,68,120019,68,120071,68,120123,68,120175,68,120227,68,120279,68,120331,68,120383,68,120435,68,5024,68,5598,68,5610,68,42195,68,8494,101,65349,101,8495,101,8519,101,119838,101,119890,101,119942,101,120046,101,120098,101,120150,101,120202,101,120254,101,120306,101,120358,101,120410,101,120462,101,43826,101,1213,101,8959,69,65317,69,8496,69,117978,69,119812,69,119864,69,119916,69,120020,69,120072,69,120124,69,120176,69,120228,69,120280,69,120332,69,120384,69,120436,69,917,69,120492,69,120550,69,120608,69,120666,69,120724,69,11577,69,5036,69,42224,69,71846,69,71854,69,66182,69,119839,102,119891,102,119943,102,119995,102,120047,102,120099,102,120151,102,120203,102,120255,102,120307,102,120359,102,120411,102,120463,102,43829,102,42905,102,383,102,7837,102,1412,102,119315,70,8497,70,117979,70,119813,70,119865,70,119917,70,120021,70,120073,70,120125,70,120177,70,120229,70,120281,70,120333,70,120385,70,120437,70,42904,70,988,70,120778,70,5556,70,42205,70,71874,70,71842,70,66183,70,66213,70,66853,70,65351,103,8458,103,119840,103,119892,103,119944,103,120048,103,120100,103,120152,103,120204,103,120256,103,120308,103,120360,103,120412,103,120464,103,609,103,7555,103,397,103,1409,103,117980,71,119814,71,119866,71,119918,71,119970,71,120022,71,120074,71,120126,71,120178,71,120230,71,120282,71,120334,71,120386,71,120438,71,1292,71,5056,71,5107,71,42198,71,65352,104,8462,104,119841,104,119945,104,119997,104,120049,104,120101,104,120153,104,120205,104,120257,104,120309,104,120361,104,120413,104,120465,104,1211,104,1392,104,5058,104,65320,72,8459,72,8460,72,8461,72,117981,72,119815,72,119867,72,119919,72,120023,72,120179,72,120231,72,120283,72,120335,72,120387,72,120439,72,919,72,120494,72,120552,72,120610,72,120668,72,120726,72,11406,72,5051,72,5500,72,42215,72,66255,72,731,105,9075,105,65353,105,8560,105,8505,105,8520,105,119842,105,119894,105,119946,105,119998,105,120050,105,120102,105,120154,105,120206,105,120258,105,120310,105,120362,105,120414,105,120466,105,120484,105,618,105,617,105,953,105,8126,105,890,105,120522,105,120580,105,120638,105,120696,105,120754,105,1110,105,42567,105,1231,105,43893,105,5029,105,71875,105,65354,106,8521,106,119843,106,119895,106,119947,106,119999,106,120051,106,120103,106,120155,106,120207,106,120259,106,120311,106,120363,106,120415,106,120467,106,1011,106,1112,106,65322,74,117983,74,119817,74,119869,74,119921,74,119973,74,120025,74,120077,74,120129,74,120181,74,120233,74,120285,74,120337,74,120389,74,120441,74,42930,74,895,74,1032,74,5035,74,5261,74,42201,74,119844,107,119896,107,119948,107,120000,107,120052,107,120104,107,120156,107,120208,107,120260,107,120312,107,120364,107,120416,107,120468,107,8490,75,65323,75,117984,75,119818,75,119870,75,119922,75,119974,75,120026,75,120078,75,120130,75,120182,75,120234,75,120286,75,120338,75,120390,75,120442,75,922,75,120497,75,120555,75,120613,75,120671,75,120729,75,11412,75,5094,75,5845,75,42199,75,66840,75,1472,108,8739,73,9213,73,65512,73,1633,108,1777,73,66336,108,125127,108,118001,108,120783,73,120793,73,120803,73,120813,73,120823,73,130033,73,65321,73,8544,73,8464,73,8465,73,117982,108,119816,73,119868,73,119920,73,120024,73,120128,73,120180,73,120232,73,120284,73,120336,73,120388,73,120440,73,65356,108,8572,73,8467,108,119845,108,119897,108,119949,108,120001,108,120053,108,120105,73,120157,73,120209,73,120261,73,120313,73,120365,73,120417,73,120469,73,448,73,120496,73,120554,73,120612,73,120670,73,120728,73,11410,73,1030,73,1216,73,1493,108,1503,108,1575,108,126464,108,126592,108,65166,108,65165,108,1994,108,11599,73,5825,73,42226,73,93992,73,66186,124,66313,124,119338,76,8556,76,8466,76,117985,76,119819,76,119871,76,119923,76,120027,76,120079,76,120131,76,120183,76,120235,76,120287,76,120339,76,120391,76,120443,76,11472,76,5086,76,5290,76,42209,76,93974,76,71843,76,71858,76,66587,76,66854,76,65325,77,8559,77,8499,77,117986,77,119820,77,119872,77,119924,77,120028,77,120080,77,120132,77,120184,77,120236,77,120288,77,120340,77,120392,77,120444,77,924,77,120499,77,120557,77,120615,77,120673,77,120731,77,1018,77,11416,77,5047,77,5616,77,5846,77,42207,77,66224,77,66321,77,119847,110,119899,110,119951,110,120003,110,120055,110,120107,110,120159,110,120211,110,120263,110,120315,110,120367,110,120419,110,120471,110,1400,110,1404,110,65326,78,8469,78,117987,78,119821,78,119873,78,119925,78,119977,78,120029,78,120081,78,120185,78,120237,78,120289,78,120341,78,120393,78,120445,78,925,78,120500,78,120558,78,120616,78,120674,78,120732,78,11418,78,42208,78,66835,78,3074,111,3202,111,3330,111,3458,111,2406,111,2662,111,2790,111,3046,111,3174,111,3302,111,3430,111,3664,111,3792,111,4160,111,1637,111,1781,111,65359,111,8500,111,119848,111,119900,111,119952,111,120056,111,120108,111,120160,111,120212,111,120264,111,120316,111,120368,111,120420,111,120472,111,7439,111,7441,111,43837,111,959,111,120528,111,120586,111,120644,111,120702,111,120760,111,963,111,120532,111,120590,111,120648,111,120706,111,120764,111,11423,111,4351,111,1413,111,1505,111,1607,111,126500,111,126564,111,126596,111,65259,111,65260,111,65258,111,65257,111,1726,111,64428,111,64429,111,64427,111,64426,111,1729,111,64424,111,64425,111,64423,111,64422,111,1749,111,3360,111,4125,111,66794,111,71880,111,71895,111,66604,111,1984,79,2534,79,2918,79,12295,79,70864,79,71904,79,118000,79,120782,79,120792,79,120802,79,120812,79,120822,79,130032,79,65327,79,117988,79,119822,79,119874,79,119926,79,119978,79,120030,79,120082,79,120134,79,120186,79,120238,79,120290,79,120342,79,120394,79,120446,79,927,79,120502,79,120560,79,120618,79,120676,79,120734,79,11422,79,1365,79,11604,79,4816,79,2848,79,66754,79,42227,79,71861,79,66194,79,66219,79,66564,79,66838,79,9076,112,65360,112,119849,112,119901,112,119953,112,120005,112,120057,112,120109,112,120161,112,120213,112,120265,112,120317,112,120369,112,120421,112,120473,112,961,112,120530,112,120544,112,120588,112,120602,112,120646,112,120660,112,120704,112,120718,112,120762,112,120776,112,11427,112,65328,80,8473,80,117989,80,119823,80,119875,80,119927,80,119979,80,120031,80,120083,80,120187,80,120239,80,120291,80,120343,80,120395,80,120447,80,929,80,120504,80,120562,80,120620,80,120678,80,120736,80,11426,80,5090,80,5229,80,42193,80,66197,80,119850,113,119902,113,119954,113,120006,113,120058,113,120110,113,120162,113,120214,113,120266,113,120318,113,120370,113,120422,113,120474,113,1307,113,1379,113,1382,113,8474,81,117990,81,119824,81,119876,81,119928,81,119980,81,120032,81,120084,81,120188,81,120240,81,120292,81,120344,81,120396,81,120448,81,11605,81,119851,114,119903,114,119955,114,120007,114,120059,114,120111,114,120163,114,120215,114,120267,114,120319,114,120371,114,120423,114,120475,114,43847,114,43848,114,7462,114,11397,114,43905,114,119318,82,8475,82,8476,82,8477,82,117991,82,119825,82,119877,82,119929,82,120033,82,120189,82,120241,82,120293,82,120345,82,120397,82,120449,82,422,82,5025,82,5074,82,66740,82,5511,82,42211,82,94005,82,65363,115,119852,115,119904,115,119956,115,120008,115,120060,115,120112,115,120164,115,120216,115,120268,115,120320,115,120372,115,120424,115,120476,115,42801,115,445,115,1109,115,43946,115,71873,115,66632,115,65331,83,117992,83,119826,83,119878,83,119930,83,119982,83,120034,83,120086,83,120138,83,120190,83,120242,83,120294,83,120346,83,120398,83,120450,83,1029,83,1359,83,5077,83,5082,83,42210,83,94010,83,66198,83,66592,83,119853,116,119905,116,119957,116,120009,116,120061,116,120113,116,120165,116,120217,116,120269,116,120321,116,120373,116,120425,116,120477,116,8868,84,10201,84,128872,84,65332,84,117993,84,119827,84,119879,84,119931,84,119983,84,120035,84,120087,84,120139,84,120191,84,120243,84,120295,84,120347,84,120399,84,120451,84,932,84,120507,84,120565,84,120623,84,120681,84,120739,84,11430,84,5026,84,42196,84,93962,84,71868,84,66199,84,66225,84,66325,84,119854,117,119906,117,119958,117,120010,117,120062,117,120114,117,120166,117,120218,117,120270,117,120322,117,120374,117,120426,117,120478,117,42911,117,7452,117,43854,117,43858,117,651,117,965,117,120534,117,120592,117,120650,117,120708,117,120766,117,1405,117,66806,117,71896,117,8746,85,8899,85,117994,85,119828,85,119880,85,119932,85,119984,85,120036,85,120088,85,120140,85,120192,85,120244,85,120296,85,120348,85,120400,85,120452,85,1357,85,4608,85,66766,85,5196,85,42228,85,94018,85,71864,85,8744,118,8897,118,65366,118,8564,118,119855,118,119907,118,119959,118,120011,118,120063,118,120115,118,120167,118,120219,118,120271,118,120323,118,120375,118,120427,118,120479,118,7456,118,957,118,120526,118,120584,118,120642,118,120700,118,120758,118,1141,118,1496,118,71430,118,43945,118,71872,118,119309,86,1639,86,1783,86,8548,86,117995,86,119829,86,119881,86,119933,86,119985,86,120037,86,120089,86,120141,86,120193,86,120245,86,120297,86,120349,86,120401,86,120453,86,1140,86,11576,86,5081,86,5167,86,42719,86,42214,86,93960,86,71840,86,66845,86,623,119,119856,119,119908,119,119960,119,120012,119,120064,119,120116,119,120168,119,120220,119,120272,119,120324,119,120376,119,120428,119,120480,119,7457,119,1121,119,1309,119,1377,119,71434,119,71438,119,71439,119,43907,119,71910,87,71919,87,117996,87,119830,87,119882,87,119934,87,119986,87,120038,87,120090,87,120142,87,120194,87,120246,87,120298,87,120350,87,120402,87,120454,87,1308,87,5043,87,5076,87,42218,87,5742,120,10539,120,10540,120,10799,120,65368,120,8569,120,119857,120,119909,120,119961,120,120013,120,120065,120,120117,120,120169,120,120221,120,120273,120,120325,120,120377,120,120429,120,120481,120,5441,120,5501,120,5741,88,9587,88,66338,88,71916,88,65336,88,8553,88,117997,88,119831,88,119883,88,119935,88,119987,88,120039,88,120091,88,120143,88,120195,88,120247,88,120299,88,120351,88,120403,88,120455,88,42931,88,935,88,120510,88,120568,88,120626,88,120684,88,120742,88,11436,88,11613,88,5815,88,42219,88,66192,88,66228,88,66327,88,66855,88,611,121,7564,121,65369,121,119858,121,119910,121,119962,121,120014,121,120066,121,120118,121,120170,121,120222,121,120274,121,120326,121,120378,121,120430,121,120482,121,655,121,7935,121,43866,121,947,121,8509,121,120516,121,120574,121,120632,121,120690,121,120748,121,1199,121,4327,121,71900,121,65337,89,117998,89,119832,89,119884,89,119936,89,119988,89,120040,89,120092,89,120144,89,120196,89,120248,89,120300,89,120352,89,120404,89,120456,89,933,89,978,89,120508,89,120566,89,120624,89,120682,89,120740,89,11432,89,1198,89,5033,89,5053,89,42220,89,94019,89,71844,89,66226,89,119859,122,119911,122,119963,122,120015,122,120067,122,120119,122,120171,122,120223,122,120275,122,120327,122,120379,122,120431,122,120483,122,7458,122,43923,122,71876,122,71909,90,66293,90,65338,90,8484,90,8488,90,117999,90,119833,90,119885,90,119937,90,119989,90,120041,90,120197,90,120249,90,120301,90,120353,90,120405,90,120457,90,918,90,120493,90,120551,90,120609,90,120667,90,120725,90,5059,90,42204,90,71849,90,65282,34,65283,35,65284,36,65285,37,65286,38,65290,42,65291,43,65294,46,65295,47,65296,48,65298,50,65299,51,65300,52,65301,53,65302,54,65303,55,65304,56,65305,57,65308,60,65309,61,65310,62,65312,64,65316,68,65318,70,65319,71,65324,76,65329,81,65330,82,65333,85,65334,86,65335,87,65343,95,65346,98,65348,100,65350,102,65355,107,65357,109,65358,110,65361,113,65362,114,65364,116,65365,117,65367,119,65370,122,65371,123,65373,125,119846,109],\"_default\":[160,32,8211,45,65374,126,8218,44,65306,58,65281,33,8216,96,8217,96,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"cs\":[65374,126,8218,44,65306,58,65281,33,8216,96,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"de\":[65374,126,65306,58,65281,33,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"es\":[8211,45,65374,126,8218,44,65306,58,65281,33,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"fr\":[65374,126,8218,44,65306,58,65281,33,8216,96,8245,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"it\":[160,32,8211,45,65374,126,8218,44,65306,58,65281,33,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"ja\":[8211,45,8218,44,65281,33,8216,96,8245,96,180,96,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65292,44,65297,49,65307,59],\"ko\":[8211,45,65374,126,8218,44,65306,58,65281,33,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"pl\":[65374,126,65306,58,65281,33,8216,96,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"pt-BR\":[65374,126,8218,44,65306,58,65281,33,8216,96,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"qps-ploc\":[160,32,8211,45,65374,126,8218,44,65306,58,65281,33,8216,96,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"ru\":[65374,126,8218,44,65306,58,65281,33,8216,96,8245,96,180,96,12494,47,305,105,921,73,1009,112,215,120,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"tr\":[160,32,8211,45,65374,126,8218,44,65306,58,65281,33,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"zh-hans\":[160,32,65374,126,8218,44,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65297,49],\"zh-hant\":[8211,45,65374,126,8218,44,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89]}")), Fe.cache = new El({ getCacheKey: JSON.stringify }, (e) => {
		...
		const i = Fe.ambiguousCharacterData.value;
		let s = e.filter((l) => !l.startsWith("_") && Object.hasOwn(i, l));
		...
		const o = n(t(i._common), a);
		return new Fe(o);
	}), Fe._locales = new En(() => Object.keys(Fe.ambiguousCharacterData.value).filter((e) => !e.startsWith("_")));
	var jt = class wn {
		static getRawData() {
	...
	rr = jt, rr._data = void 0;
	const Cn = "default", Bl = "$initialize";
	var Vl = class {
		...
		createProxyToRemoteChannel(e, t) {
			return new Proxy(Object.create(null), { get: (n, r) => (typeof r == "string" && !n[r] && (cr(r) ? n[r] = (i) => this.listen(e, r, i) : lr(r) ? n[r] = this.listen(e, r, void 0) : r.charCodeAt(0) === 36 && (n[r] = async (...i) => (await t?.(), this.sendMessage(e, r, i)))), n[r]) });
		}
		_handleMessage(e) {
			switch (e.type) {
		...
		_handleRequestMessage(e) {
			...
			this._handler.handleMessage(e.channel, e.method, e.args).then((n) => {
				this._send(new or(this._workerId, t, n, void 0));
			}, (n) => {
				n.detail instanceof Error && (n.detail = Tn(n.detail)), this._send(new or(this._workerId, t, void 0, Tn(n)));
			});
		}
	...
	function lr(e) {
		return e[0] === "o" && e[1] === "n" && sr(e.charCodeAt(2));
	}
	function cr(e) {
		return /^onDynamic/.test(e) && sr(e.charCodeAt(9));
	}
	var Ql = class {
		constructor(e, t) {
		...
		_handleMessage(e, t, n) {
			if (e === Cn && t === Bl) return this.initialize(n[0]);
			const r = e === Cn ? this.requestHandler : this._localChannels.get(e);
	...
	let hr = !1;
	function Yl(e) {
		if (hr) throw new Error("WebWorker already initialized!");
		hr = !0;
		const t = new Ql((n) => globalThis.postMessage(n), (n) => e(n));
	...
	const ur = typeof Buffer < "u";
	new En(() => new Uint8Array(256));
	let In;
	var Jl = class $o {
		static wrap(t) {
			return ur && !Buffer.isBuffer(t) && (t = Buffer.from(t.buffer, t.byteOffset, t.byteLength)), new $o(t);
		}
		constructor(t) {
			this.buffer = t, this.byteLength = this.buffer.byteLength;
		}
		toString() {
			return ur ? this.buffer.toString() : (In || (In = new TextDecoder()), In.decode(this.buffer));
		}
	};
	...
	function Kl(e, t) {
		t = fr(149417, t);
		for (let n = 0, r = e.length; n < r; n++) t = fr(e.charCodeAt(n), t);
		return t;
	}
	...
	function pt(e, t = 32) {
		return e instanceof ArrayBuffer ? Zl(Jl.wrap(new Uint8Array(e))) : (e >>> 0).toString(16).padStart(t / 4, "0");
	}
	mr = class Go {
		constructor() {
		...
		_step() {
			...
			for (let d = 64; d < 320; d += 4) t.setUint32(d, zn(t.getUint32(d - 12, !1) ^ t.getUint32(d - 32, !1) ^ t.getUint32(d - 56, !1) ^ t.getUint32(d - 64, !1), 1), !1);
			let r = this._h0, i = this._h1, s = this._h2, a = this._h3, o = this._h4, l, c, u;
			for (let d = 0; d < 80; d++) d < 20 ? (l = i & s | ~i & a, c = 1518500249) : d < 40 ? (l = i ^ s ^ a, c = 1859775393) : d < 60 ? (l = i & s | i & a | s & a, c = 2400959708) : (l = i ^ s ^ a, c = 3395469782), u = zn(r, 5) + l + o + c + t.getUint32(d * 4, !1) & 4294967295, o = a, a = s, s = zn(i, 30), i = r, r = u;
	...
	}, mr._bigBlock32 = /* @__PURE__ */ new DataView(/* @__PURE__ */ new ArrayBuffer(320));
	var pr = class {
		constructor(e) {
			this.source = e;
		}
	...
	function tc(e, t, n) {
		return new br(new pr(e), new pr(t)).ComputeDiff(n).changes;
	}
	var Je = class {
		static Assert(e, t) {
		...
		constructor(t, n, r = null) {
			...
			const [i, s, a] = ut._getElements(t), [o, l, c] = ut._getElements(n);
		...
		ElementsAreStrictEqual(t, n) {
			return this.ElementsAreEqual(t, n) ? ut._getStrictElement(this._originalSequence, t) === ut._getStrictElement(this._modifiedSequence, n) : !1;
		}
		...
		ComputeDiffRecursive(t, n, r, i, s) {
			...
			if (t > n || r > i) {
				let d;
				return r <= i ? (Je.Assert(t === n + 1, "originalStart should only be one more than originalEnd"), d = [new He(t, 0, r, i - r + 1)]) : t <= n ? (Je.Assert(r === i + 1, "modifiedStart should only be one more than modifiedEnd"), d = [new He(t, n - t + 1, r, 0)]) : (Je.Assert(t === n + 1, "originalStart should only be one more than originalEnd"), Je.Assert(r === i + 1, "modifiedStart should only be one more than modifiedEnd"), d = []), d;
			}
		...
		WALKTRACE(t, n, r, i, s, a, o, l, c, u, d, m, p, b, g, v, T, k) {
			let E = null, I = null, D = new gr(), w = n, f = r, _ = p[0] - v[0] - i, C = -1073741824, z = this.m_forwardHistory.length - 1;
			do {
				const L = _ + t;
			...
			if (E = D.getReverseChanges(), k[0]) {
				...
				D = new gr(), w = a, f = o, _ = p[0] - v[0] - l, C = 1073741824, z = T ? this.m_reverseHistory.length - 1 : this.m_reverseHistory.length - 2;
				do {
					const L = _ + s;
		...
		ComputeRecursionPoint(t, n, r, i, s, a, o) {
			...
			for (let f = 1; f <= b / 2 + 1; f++) {
				...
				if (f <= 1447) {
					...
					L[0] = k - u + 1, Ze.Copy2(v, u, L, 1, d - u + 1), this.m_forwardHistory.push(L), L = new Int32Array(p - m + 2), L[0] = E - m + 1, Ze.Copy2(T, m, L, 1, p - m + 1), this.m_reverseHistory.push(L);
				}
			}
	...
	function ac() {
		if (gt === null) {
			gt = new nc(0);
			const e = ` 	<>'"、。｡､，．：；‘〈「『〔（［｛｢｣｝］）〕』」〉’｀～…|`;
			for (let n = 0; n < 36; n++) gt.set(e.charCodeAt(n), 1);
			const t = ".,;:";
			for (let n = 0; n < 4; n++) gt.set(t.charCodeAt(n), 2);
		}
		return gt;
	}
	var oc = class Wi {
			...
			for (let s = 1, a = t.getLineCount(); s <= a; s++) {
				...
				for (; c < l;) {
					...
					if (m === 13) {
						...
						E === 1 && (i.push(Wi._createLink(r, o, s, u, c)), T = !0);
					} else if (m === 12) {
						let E;
				...
				m === 13 && i.push(Wi._createLink(r, o, s, u, l));
			}
			return i;
		}
	};
	...
	var Gt = class {
		...
		get onCancellationRequested() {
			return this._isCancelled ? vr : (this._emitter || (this._emitter = new Se()), this._emitter.event);
		}
		dispose() {
		...
		cancel() {
			this._token ? this._token instanceof Gt && this._token.cancel() : this._token = $t.Cancelled;
		}
		dispose(e = !1) {
			e && this.cancel(), this._parentListener?.dispose(), this._token ? this._token instanceof Gt && this._token.dispose() : this._token = $t.None;
		}
	}, Dn = class {
		constructor() {
	...
	const Xt = new Dn(), Wn = new Dn(), Pn = new Dn(), hc = new Array(230), uc = Object.create(null), dc = Object.create(null), yr = [];
	for (let e = 0; e <= 193; e++) yr[e] = -1;
	(function() {
		const t = [
			[
				1,
		...
		for (const i of t) {
			...
			if (r[a] || (r[a] = !0, uc[o] = a, dc[o.toLowerCase()] = a, s && (yr[a] = l)), !n[l]) {
				if (n[l] = !0, !c) throw new Error(`String representation missing for key code ${l} around scan code ${o}`);
				Xt.define(l, c), Wn.define(l, m || c), Pn.define(l, p || m || c);
			}
			u && (hc[u] = l);
		}
	})();
	...
	(function(e) {
		...
		function s(o) {
			return Wn.strToKeyCode(o) || Pn.strToKeyCode(o);
		}
	...
	if (typeof qn < "u" && typeof qn.process < "u") {
	...
	} else typeof process < "u" && typeof process?.versions?.node == "string" ? et = {
		get platform() {
		...
		get platform() {
			return ft ? "win32" : Ll ? "darwin" : "linux";
		},
		get arch() {},
		get env() {
			return {};
	...
	const Qt = et.cwd, fc = et.env, pc = et.platform, gc = 65, bc = 97, wc = 90, _c = 122, Be = 46, ie = 47, he = 92, Me = 58, vc = 63;
	...
	function yc(e, t) {
		if (e === null || typeof e != "object") throw new kr(t, "Object", e);
	}
	function ee(e, t) {
		if (typeof e != "string") throw new kr(t, "string", e);
	}
	const Ue = pc === "win32";
	function j(e) {
		return e === ie || e === he;
	...
	function Sr(e, t) {
		yc(t, "pathObject");
		const n = t.dir || t.root, r = t.base || `${t.name || ""}${Tc(t.ext)}`;
	...
	const le = {
		resolve(...e) {
			...
			for (let i = e.length - 1; i >= -1; i--) {
				...
				} else t.length === 0 ? s = Qt() : (s = fc[`=${t}`] || Qt(), (s === void 0 || s.slice(0, 2).toLowerCase() !== t.toLowerCase() && s.charCodeAt(2) === he) && (s = `${t}\\`));
				const a = s.length;
				let o = 0, l = "", c = !1;
				...
				else Ee(u) && s.charCodeAt(1) === Me && (l = s.slice(0, 2), o = 2, a > 2 && j(s.charCodeAt(2)) && (c = !0, o = 3));
		...
		normalize(e) {
			...
			else Ee(s) && e.charCodeAt(1) === Me && (r = e.slice(0, 2), n = 2, t > 2 && j(e.charCodeAt(2)) && (i = !0, n = 3));
			let a = n < t ? Yt(e.slice(n), !i, "\\", j) : "";
			if (a.length === 0 && !i && (a = "."), a.length > 0 && j(e.charCodeAt(t - 1)) && (a += "\\"), !i && r === void 0 && e.includes(":")) {
				if (a.length >= 2 && Ee(a.charCodeAt(0)) && a.charCodeAt(1) === Me) return `.\\${a}`;
				let o = e.indexOf(":");
				do
		...
		isAbsolute(e) {
			...
			return j(n) || t > 2 && Ee(n) && e.charCodeAt(1) === Me && j(e.charCodeAt(2));
		},
		join(...e) {
		...
		toNamespacedPath(e) {
			...
			if (t.charCodeAt(0) === he) {
				if (t.charCodeAt(1) === he) {
					...
					if (n !== vc && n !== Be) return `\\\\?\\UNC\\${t.slice(2)}`;
				}
			} else if (Ee(t.charCodeAt(0)) && t.charCodeAt(1) === Me && t.charCodeAt(2) === he) return `\\\\?\\${t}`;
			return t;
		},
		dirname(e) {
			ee(e, "path");
			...
			} else Ee(i) && e.charCodeAt(1) === Me && (n = t > 2 && j(e.charCodeAt(2)) ? 3 : 2, r = n);
			let s = -1, a = !0;
		...
		basename(e, t) {
			...
			let n = 0, r = -1, i = !0, s;
			if (e.length >= 2 && Ee(e.charCodeAt(0)) && e.charCodeAt(1) === Me && (n = 2), t !== void 0 && t.length > 0 && t.length <= e.length) {
				if (t === e) return "";
		...
		extname(e) {
			...
			e.length >= 2 && e.charCodeAt(1) === Me && Ee(e.charCodeAt(0)) && (t = r = 2);
		...
		parse(e) {
			...
			} else if (Ee(i) && e.charCodeAt(1) === Me) {
		...
		resolve(...e) {
			...
			for (let r = e.length - 1; r >= 0 && !n; r--) {
				const i = e[r];
				ee(i, `paths[${r}]`), i.length !== 0 && (t = `${i}/${t}`, n = i.charCodeAt(0) === ie);
			}
			if (!n) {
				const r = kc();
				t = `${r}/${t}`, n = r.charCodeAt(0) === ie;
			}
			return t = Yt(t, !n, "/", On), n ? `/${t}` : t.length > 0 ? t : ".";
		},
		normalize(e) {
			if (ee(e, "path"), e.length === 0) return ".";
			const t = e.charCodeAt(0) === ie, n = e.charCodeAt(e.length - 1) === ie;
			return e = Yt(e, !t, "/", On), e.length === 0 ? t ? "/" : n ? "./" : "." : (n && (e += "/"), t ? `/${e}` : e);
		},
		isAbsolute(e) {
		...
		relative(e, t) {
			...
			if (c === o) if (a > o) {
				if (t.charCodeAt(s + c) === ie) return t.slice(s + c + 1);
				if (c === 0) return t.slice(s + c);
			} else i > o && (e.charCodeAt(n + c) === ie ? l = c : c === 0 && (l = 0));
			let u = "";
			for (c = n + l + 1; c <= r; ++c) (c === r || e.charCodeAt(c) === ie) && (u += u.length === 0 ? ".." : "/..");
	...
	function Ac(e, t) {
		...
		if (e.scheme && !Sc.test(e.scheme)) throw new Error("[UriError]: Scheme contains illegal characters.");
		if (e.path) {
			if (e.authority) {
	...
	var Fn = class _n {
		...
		constructor(t, n, r, i, s, a = !1) {
			typeof t == "object" ? (this.scheme = t.scheme || J, this.authority = t.authority || J, this.path = t.path || J, this.query = t.query || J, this.fragment = t.fragment || J) : (this.scheme = Rc(t, a), this.authority = n || J, this.path = Mc(this.scheme, r || J), this.query = i || J, this.fragment = s || J, Ac(this, a));
		}
		get fsPath() {
			return Bn(this, !1);
		}
		with(t) {
			...
			return n === void 0 ? n = this.scheme : n === null && (n = J), r === void 0 ? r = this.authority : r === null && (r = J), i === void 0 ? i = this.path : i === null && (i = J), s === void 0 ? s = this.query : s === null && (s = J), a === void 0 ? a = this.fragment : a === null && (a = J), n === this.scheme && r === this.authority && i === this.path && s === this.query && a === this.fragment ? this : new tt(n, r, i, s, a);
		}
		static parse(t, n = !1) {
			const r = Ec.exec(t);
			return r ? new tt(r[2] || J, Jt(r[4] || J), Jt(r[5] || J), Jt(r[7] || J), Jt(r[9] || J), n) : new tt(J, J, J, J, J);
		}
		static file(t) {
			let n = J;
			if (ft && (t = t.replace(/\\/g, _e)), t[0] === _e && t[1] === _e) {
				const r = t.indexOf(_e, 2);
				r === -1 ? (n = t.substring(2), t = _e) : (n = t.substring(2, r), t = t.substring(r) || _e);
			}
			return new tt("file", n, t, J, J);
		}
		static from(t, n) {
			return new tt(t.scheme, t.authority, t.path, t.query, t.fragment, n);
		}
		static joinPath(t, ...n) {
			if (!t.path) throw new Error("[UriError]: cannot call joinPath on URI without path");
			let r;
			return ft && t.scheme === "file" ? r = _n.file(le.join(Bn(t, !0), ...n)).path : r = ue.join(t.path, ...n), t.with({ path: r });
		}
		toString(t = !1) {
			return Vn(this, t);
		}
		...
		static revive(t) {
			if (t) {
				...
				{
					const n = new tt(t);
					return n._formatted = t.external ?? null, n._fsPath = t._sep === Lr ? t.fsPath ?? null : null, n;
				}
			} else return t;
		}
	};
	const Lr = ft ? 1 : void 0;
	var tt = class extends Fn {
		constructor() {
		...
		toJSON() {
			const e = { $mid: 1 };
			return this._fsPath && (e.fsPath = this._fsPath, e._sep = Lr), this._formatted && (e.external = this._formatted), this.path && (e.path = this.path), this.scheme && (e.scheme = this.scheme), this.authority && (e.authority = this.authority), this.query && (e.query = this.query), this.fragment && (e.fragment = this.fragment), e;
		}
	};
	const xr = {
		58: "%3A",
	...
	function Vn(e, t) {
		...
		return o && (r += "?", r += n(o, !1, !1)), l && (r += "#", r += t ? l : Ar(l, !1, !1)), r;
	}
	function Rr(e) {
		try {
	...
	function Jt(e) {
		return e.match(Mr) ? e.replace(Mr, (t) => Rr(t)) : e;
	}
	var Cc = class we extends V {
		constructor(t, n, r, i) {
	...
	var Hc = class {
		...
		register(e, t) {
			return this._tokenizationSupports.set(e, t), this.handleChange([e]), Wt(() => {
		...
		registerFactory(e, t) {
			...
			return this._factories.set(e, n), Wt(() => {
	...
	Ve = class Ct {
		static fromValue(t) {
			switch (t) {
				case "comment": return Ct.Comment;
				case "imports": return Ct.Imports;
				case "region": return Ct.Region;
			}
			return new Ct(t);
		}
		constructor(t) {
	...
	bt = zs, bt.CtrlCmd = 2048, bt.Shift = 1024, bt.Alt = 512, bt.WinCtrl = 256;
	function Pc() {
		return {
			editor: void 0,
			languages: void 0,
	...
	Us = (Ds = Symbol.toStringTag, Symbol.iterator);
	Hs = class It {
		constructor(t, n) {
			if (this[Ds] = "ResourceMap", t instanceof It) this.map = new Map(t.map), this.toKey = n ?? It.defaultToKey;
			else if (Oc(t)) {
	...
	}, Hs.defaultToKey = (e) => e.toString();
	var Fc = class {
		constructor() {
			this[Ps] = "LinkedMap", this._map = /* @__PURE__ */ new Map(), this._head = void 0, this._tail = void 0, this._size = 0, this._state = 0;
		}
		clear() {
	...
	var Yc = class {
		...
		next(e) {
			...
			do {
				...
				if (this._prevMatchStartIndex = r, this._prevMatchLength = i, !this._wordSeparators || Qc(this._wordSeparators, e, t, r, i)) return n;
			} while (n);
			return null;
		}
	};
	...
	function Yn(e, t, n, r, i) {
		if (t = js(t), i || (i = Dt.first($s)), n.length > i.maxLen) {
			let c = e - i.maxLen / 2;
			return c < 0 ? c = 0 : r += c, n = n.substring(c, e + i.maxLen / 2), Yn(e, t, n, r, i);
		}
	...
	var eh = class {
		static computeUnicodeHighlights(e, t, n) {
			const r = n ? n.startLineNumber : 1, i = n ? n.endLineNumber : e.getLineCount(), s = new Gs(t), a = s.getCandidateCodePoints();
			let o;
			...
			e: for (let g = r, v = i; g <= v; g++) {
				...
				do
					if (d = l.next(T), d) {
						...
						E > 0 && Bt(T.charCodeAt(E - 1)) && E--, I + 1 < k && Bt(T.charCodeAt(I - 1)) && I++;
		...
		static computeUnicodeHighlightReason(e, t) {
			...
			switch (n.shouldHighlightNonBasicASCII(e, null)) {
				...
				case 3: {
					const r = e.codePointAt(0), i = n.ambiguousCharacters.getPrimaryConfusable(r), s = Vt.getLocales().filter((a) => !Vt.getInstance(new Set([...t.allowedLocales, a])).isAmbiguous(r));
					return {
						kind: 0,
	...
	function th(e, t) {
		return `[${Nl(e.map((n) => String.fromCodePoint(n)).join(""))}]`;
	}
	var Gs = class {
		constructor(e) {
		...
		getCandidateCodePoints() {
			...
			if (this.options.invisibleCharacters) for (const t of jt.codePoints) Xs(String.fromCodePoint(t)) || e.add(t);
		...
		shouldHighlightNonBasicASCII(e, t) {
			...
			if (t) for (const s of t) {
				const a = s.codePointAt(0), o = ql(s);
				r = r || o, !o && !this.ambiguousCharacters.isAmbiguous(a) && !jt.isInvisibleCharacter(a) && (i = !0);
			}
			return !r && i ? 0 : this.options.invisibleCharacters && !Xs(e) && jt.isInvisibleCharacter(n) ? 2 : this.options.ambiguousCharacters && this.ambiguousCharacters.isAmbiguous(n) ? 3 : 0;
		}
	};
	function Xs(e) {
		return e === " " || e === `
` || e === "	";
	...
	Jn = class Pi {
		...
		filter(t) {
			return new Pi((n) => this.iterate((r) => t(r) ? n(r) : !0));
		}
		map(t) {
			return new Pi((n) => this.iterate((r) => n(t(r))));
		}
		findLast(t) {
			let n;
		...
		findLastMaxBy(t) {
			let n, r = !0;
			return this.iterate((i) => ((r || Zn.isGreaterThan(t(i, n))) && (r = !1, n = i), !0)), n;
		}
	}, Jn.empty = new Jn((e) => {});
	var Y = class ge {
		static fromTo(t, n) {
	...
	function Kn(e, t, n = 0, r = e.length) {
		let i = n, s = r;
		for (; i < s;) {
			const a = Math.floor((i + s) / 2);
			t(e[a]) ? s = a : i = a + 1;
	...
	var Ys, G = class Te {
		...
		static joinMany(t) {
			...
			let n = new Kt(t[0].slice());
			for (let r = 1; r < t.length; r++) n = n.getUnion(new Kt(t[r].slice()));
			return n.ranges;
		}
		static join(t) {
	...
	Ys = G, Ys.compareByStart = wt((e) => e.startLineNumber, _t);
	var Kt = class dt {
		constructor(t = []) {
		...
		addRange(t) {
			if (t.length === 0) return;
			const n = Kn(this._normalizedRanges, (i) => i.endLineNumberExclusive >= t.startLineNumber), r = it(this._normalizedRanges, (i) => i.startLineNumber <= t.endLineNumberExclusive) + 1;
		...
		getUnion(t) {
			...
			return s !== null && n.push(s), new dt(n);
		}
		subtractFrom(t) {
			const n = Kn(this._normalizedRanges, (a) => a.endLineNumberExclusive >= t.startLineNumber), r = it(this._normalizedRanges, (a) => a.startLineNumber <= t.endLineNumberExclusive) + 1;
			if (n === r) return new dt([t]);
			const i = [];
			...
			return s < t.endLineNumberExclusive && i.push(new G(s, t.endLineNumberExclusive)), new dt(i);
		}
		toString() {
		...
		getWithDelta(t) {
			return new dt(this._normalizedRanges.map((n) => n.delta(t)));
		}
	}, ti, en = class qe {
		static betweenPositions(t, n) {
			return t.lineNumber === n.lineNumber ? new qe(0, n.column - t.column) : new qe(n.lineNumber - t.lineNumber, n.column - 1);
		}
		static fromPosition(t) {
			return new qe(t.lineNumber - 1, t.column - 1);
		}
		static ofRange(t) {
			return qe.betweenPositions(t.getStartPosition(), t.getEndPosition());
		}
		static ofText(t) {
			let n = 0, r = 0;
			for (const i of t) i === `
` ? (n++, r = 0) : r++;
		...
		add(t) {
			return t.lineCount === 0 ? new qe(this.lineCount, this.columnCount + t.columnCount) : new qe(this.lineCount + t.lineCount, t.columnCount);
		}
		createRange(t) {
		...
		toLineRange() {
			return G.ofLength(1, this.lineCount + 1);
		}
		addToPosition(t) {
	...
	ti = en, ti.zero = new ti(0, 0);
	var Js, uh = class {
		getOffsetRange(e) {
			return new Y(this.getOffset(e.getStartPosition()), this.getOffset(e.getEndPosition()));
		}
		getRange(e) {
	...
	Js = vt, Js._deps = void 0;
	var dh = class extends uh {
		constructor(e) {
...
` && (this.lineStartOffsetByLineIdx.push(t + 1), t > 0 && e.charAt(t - 1) === "\r" ? this.lineEndOffsetByLineIdx.push(t - 1) : this.lineEndOffsetByLineIdx.push(t));
		...
		_validatePosition(e) {
			...
			if (e.lineNumber > t) return new Q(t, this.getLineLength(t) + 1);
			if (e.column < 1) return new Q(e.lineNumber, 1);
			const n = this.getLineLength(e.lineNumber);
			return e.column - 1 > n ? new Q(e.lineNumber, n + 1) : e;
		}
		getPosition(e) {
			const t = it(this.lineStartOffsetByLineIdx, (n) => n <= e);
			return new Q(t + 1, e - this.lineStartOffsetByLineIdx[t] + 1);
		}
		get textLength() {
		...
		getTransformer() {
			return this._transformer || (this._transformer = new dh(this.getValue())), this._transformer;
		}
		getLineAt(e) {
...
` + this._getLineContent(e.endLineNumber).substring(0, e.endColumn - 1), t;
		...
		get length() {
			...
			return new en(this._lineCount - 1, e.length);
		}
	}, tn = class extends fh {
		constructor(e) {
...
`, `
`), i = Ul(n, r), s = en.ofText(n.substring(0, i)).addToPosition(this.range.getStartPosition()), a = r.substring(i);
...
`), r = this.text.replaceAll(`\r
`, `
`), i = Dl(n, r), s = en.ofText(n.substring(0, n.length - i)).addToPosition(this.range.getStartPosition()), a = r.substring(0, r.length - i);
		...
		static inverse(t, n, r) {
			...
			let s = 1, a = 1;
			for (const l of t) {
				const c = new mt(new G(s, l.original.startLineNumber), new G(a, l.modified.startLineNumber));
				c.modified.isEmpty || i.push(c), s = l.original.endLineNumberExclusive, a = l.modified.endLineNumberExclusive;
			}
			const o = new mt(new G(s, n + 1), new G(a, r + 1));
		...
		static clip(t, n, r) {
			...
			for (const s of t) {
				...
				a && !a.isEmpty && o && !o.isEmpty && i.push(new mt(a, o));
			}
			return i;
		}
		constructor(t, n) {
		...
		toRangeMapping() {
			...
			if (t && n) return new Ne(t, n);
			if (this.original.startLineNumber === 1 || this.modified.startLineNumber === 1) {
				if (!(this.modified.startLineNumber === 1 && this.original.startLineNumber === 1)) throw new oe("not a valid diff");
				return new Ne(new V(this.original.startLineNumber, 1, this.original.endLineNumberExclusive, 1), new V(this.modified.startLineNumber, 1, this.modified.endLineNumberExclusive, 1));
			} else return new Ne(new V(this.original.startLineNumber - 1, Number.MAX_SAFE_INTEGER, this.original.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), new V(this.modified.startLineNumber - 1, Number.MAX_SAFE_INTEGER, this.modified.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER));
		}
		toRangeMapping2(t, n) {
			if (Zs(this.original.endLineNumberExclusive, t) && Zs(this.modified.endLineNumberExclusive, n)) return new Ne(new V(this.original.startLineNumber, 1, this.original.endLineNumberExclusive, 1), new V(this.modified.startLineNumber, 1, this.modified.endLineNumberExclusive, 1));
			if (!this.original.isEmpty && !this.modified.isEmpty) return new Ne(V.fromPositions(new Q(this.original.startLineNumber, 1), st(new Q(this.original.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), t)), V.fromPositions(new Q(this.modified.startLineNumber, 1), st(new Q(this.modified.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), n)));
			if (this.original.startLineNumber > 1 && this.modified.startLineNumber > 1) return new Ne(V.fromPositions(st(new Q(this.original.startLineNumber - 1, Number.MAX_SAFE_INTEGER), t), st(new Q(this.original.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), t)), V.fromPositions(st(new Q(this.modified.startLineNumber - 1, Number.MAX_SAFE_INTEGER), n), st(new Q(this.modified.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), n)));
			throw new oe();
		}
	};
	function st(e, t) {
		if (e.lineNumber < 1) return new Q(1, 1);
		if (e.lineNumber > t.length) return new Q(t.length, t[t.length - 1].length + 1);
	...
	var yt = class vn extends rt {
		static fromRangeMappings(t) {
			return new vn(G.join(t.map((n) => G.fromRangeInclusive(n.originalRange))), G.join(t.map((n) => G.fromRangeInclusive(n.modifiedRange))), t);
		}
		constructor(t, n, r) {
		...
		static fromEdit(t) {
			...
			return t.replacements.map((r, i) => new qi(r.range, n[i]));
		}
		...
		flip() {
			return new qi(this.modifiedRange, this.originalRange);
		}
		toTextEdit(t) {
			const n = t.getValueOfRange(this.modifiedRange);
			return new ph(this.originalRange, n);
		}
	};
	function Ks(e, t, n, r = !1) {
		const i = [];
		for (const s of rh(e.map((a) => gh(a, t, n)), (a, o) => a.original.intersectsOrTouches(o.original) || a.modified.intersectsOrTouches(o.modified))) {
			const a = s[0], o = s[s.length - 1];
			i.push(new yt(a.original.join(o.original), a.modified.join(o.modified), s.map((l) => l.innerChanges[0])));
		}
		return Ut(() => !r && i.length > 0 && (i[0].modified.startLineNumber !== i[0].original.startLineNumber || n.length.lineCount - i[i.length - 1].modified.endLineNumberExclusive !== t.length.lineCount - i[i.length - 1].original.endLineNumberExclusive) ? !1 : $i(i, (s, a) => a.original.startLineNumber - s.original.endLineNumberExclusive === a.modified.startLineNumber - s.modified.endLineNumberExclusive && s.original.endLineNumberExclusive < a.original.startLineNumber && s.modified.endLineNumberExclusive < a.modified.startLineNumber)), i;
	}
	function gh(e, t, n) {
		let r = 0, i = 0;
		return e.modifiedRange.endColumn === 1 && e.originalRange.endColumn === 1 && e.originalRange.startLineNumber + r <= e.originalRange.endLineNumber && e.modifiedRange.startLineNumber + r <= e.modifiedRange.endLineNumber && (i = -1), e.modifiedRange.startColumn - 1 >= n.getLineLength(e.modifiedRange.startLineNumber) && e.originalRange.startColumn - 1 >= t.getLineLength(e.originalRange.startLineNumber) && e.originalRange.startLineNumber <= e.originalRange.endLineNumber + i && e.modifiedRange.startLineNumber <= e.modifiedRange.endLineNumber + i && (r = 1), new yt(new G(e.originalRange.startLineNumber + r, e.originalRange.endLineNumber + 1 + i), new G(e.modifiedRange.startLineNumber + r, e.modifiedRange.endLineNumber + 1 + i), [e]);
	}
	const bh = 3;
	var wh = class {
		computeDiff(e, t, n) {
			...
			for (const a of r.changes) {
				let o;
				a.originalEndLineNumber === 0 ? o = new G(a.originalStartLineNumber + 1, a.originalStartLineNumber + 1) : o = new G(a.originalStartLineNumber, a.originalEndLineNumber + 1);
				let l;
				a.modifiedEndLineNumber === 0 ? l = new G(a.modifiedStartLineNumber + 1, a.modifiedStartLineNumber + 1) : l = new G(a.modifiedStartLineNumber, a.modifiedEndLineNumber + 1);
				let c = new yt(o, l, a.charChanges?.map((u) => new Ne(new V(u.originalStartLineNumber, u.originalStartColumn, u.originalEndLineNumber, u.originalEndColumn), new V(u.modifiedStartLineNumber, u.modifiedStartColumn, u.modifiedEndLineNumber, u.modifiedEndColumn))));
				s && (s.modified.endLineNumberExclusive === c.modified.startLineNumber || s.original.endLineNumberExclusive === c.original.startLineNumber) && (c = new yt(s.original.join(c.original), s.modified.join(c.modified), s.innerChanges && c.innerChanges ? s.innerChanges.concat(c.innerChanges) : void 0), i.pop()), i.push(c), s = c;
			}
			return Ut(() => $i(i, (a, o) => o.original.startLineNumber - a.original.endLineNumberExclusive === o.modified.startLineNumber - a.modified.endLineNumberExclusive && a.original.endLineNumberExclusive < o.original.startLineNumber && a.modified.endLineNumberExclusive < o.modified.startLineNumber)), new Zt(i, [], r.quitEarly);
		}
	};
	function ea(e, t, n, r) {
		return new br(e, t, n).ComputeDiff(r);
	}
	var ta = class {
		constructor(e) {
			const t = [], n = [];
			for (let r = 0, i = e.length; r < i; r++) t[r] = ii(e[r], 1), n[r] = ri(e[r], 1);
	...
	function vh(e) {
		...
		for (let r = 1, i = e.length; r < i; r++) {
			...
			Math.min(a, o) < bh ? (n.originalLength = s.originalStart + s.originalLength - n.originalStart, n.modifiedLength = s.modifiedStart + s.modifiedLength - n.modifiedStart) : (t.push(s), n = s);
		}
		return t;
	}
	var ni = class Jo {
		...
		static createFromDiffResult(t, n, r, i, s, a, o) {
			...
			if (n.originalLength === 0 ? (l = r.getStartLineNumber(n.originalStart) - 1, c = 0) : (l = r.getStartLineNumber(n.originalStart), c = r.getEndLineNumber(n.originalStart + n.originalLength - 1)), n.modifiedLength === 0 ? (u = i.getStartLineNumber(n.modifiedStart) - 1, d = 0) : (u = i.getStartLineNumber(n.modifiedStart), d = i.getEndLineNumber(n.modifiedStart + n.modifiedLength - 1)), a && n.originalLength > 0 && n.originalLength < 20 && n.modifiedLength > 0 && n.modifiedLength < 20 && s()) {
				...
				if (p.getElements().length > 0 && b.getElements().length > 0) {
					let g = ea(p, b, s, !0).changes;
					o && (g = vh(g)), m = [];
					for (let v = 0, T = g.length; v < T; v++) m.push(nn.createFromDiffChange(g[v], p, b));
				}
			}
			return new Jo(l, c, u, d, m);
		}
	}, yh = class {
		constructor(e, t, n) {
			this.shouldComputeCharChanges = n.shouldComputeCharChanges, this.shouldPostProcessCharChanges = n.shouldPostProcessCharChanges, this.shouldIgnoreTrimWhitespace = n.shouldIgnoreTrimWhitespace, this.shouldMakePrettyDiff = n.shouldMakePrettyDiff, this.originalLines = e, this.modifiedLines = t, this.original = new ta(e), this.modified = new ta(t), this.continueLineDiff = na(n.maxComputationTime), this.continueCharDiff = na(n.maxComputationTime === 0 ? 0 : Math.min(n.maxComputationTime, 5e3));
		}
		computeDiff() {
			...
			const e = ea(this.original, this.modified, this.continueLineDiff, this.shouldMakePrettyDiff), t = e.changes, n = e.quitEarly;
			if (this.shouldIgnoreTrimWhitespace) {
				const a = [];
				for (let o = 0, l = t.length; o < l; o++) a.push(ni.createFromDiffResult(this.shouldIgnoreTrimWhitespace, t[o], this.original, this.modified, this.continueCharDiff, this.shouldComputeCharChanges, this.shouldPostProcessCharChanges));
				return {
					quitEarly: n,
					changes: a
				};
			...
			for (let a = -1, o = t.length; a < o; a++) {
				...
				l && (r.push(ni.createFromDiffResult(this.shouldIgnoreTrimWhitespace, l, this.original, this.modified, this.continueCharDiff, this.shouldComputeCharChanges, this.shouldPostProcessCharChanges)), i += l.originalLength, s += l.modifiedLength);
			}
			return {
				quitEarly: n,
				changes: r
		...
		_pushTrimWhitespaceCharChange(e, t, n, r, i, s, a) {
			...
			let o;
			this.shouldComputeCharChanges && (o = [new nn(t, n, t, r, i, s, i, a)]), e.push(new ni(t, t, i, i, o));
		}
		_mergeTrimWhitespaceCharChange(e, t, n, r, i, s, a) {
			const o = e.length;
			if (o === 0) return !1;
			const l = e[o - 1];
			return l.originalEndLineNumber === 0 || l.modifiedEndLineNumber === 0 ? !1 : l.originalEndLineNumber === t && l.modifiedEndLineNumber === i ? (this.shouldComputeCharChanges && l.charChanges && l.charChanges.push(new nn(t, n, t, r, i, s, i, a)), !0) : l.originalEndLineNumber + 1 === t && l.modifiedEndLineNumber + 1 === i ? (l.originalEndLineNumber = t, l.modifiedEndLineNumber = i, this.shouldComputeCharChanges && l.charChanges && l.charChanges.push(new nn(t, n, t, r, i, s, i, a)), !0) : !1;
		}
	};
	function ii(e, t) {
		const n = zl(e);
	...
	var Tt, si, at = class Oi {
		static trivial(t, n) {
			return new Oi([new de(Y.ofLength(t.length), Y.ofLength(n.length))], !1);
		}
		static trivialTimedOut(t, n) {
			return new Oi([new de(Y.ofLength(t.length), Y.ofLength(n.length))], !0);
		}
		constructor(t, n) {
		...
		static invert(t, n) {
			const r = [];
			return sh(t, (i, s) => {
				r.push(ze.fromOffsetPairs(i ? i.getEndExclusives() : De.zero, s ? s.getStarts() : new De(n, (i ? i.seq2Range.endExclusive - i.seq1Range.endExclusive : 0) + n)));
			}), r;
		}
		static fromOffsetPairs(t, n) {
			return new ze(new Y(t.offset1, n.offset1), new Y(t.offset2, n.offset2));
		}
		static assertSorted(t) {
			let n;
			for (const r of t) {
	...
	Tt = De, Tt.zero = new Tt(0, 0), Tt.max = new Tt(Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER);
	var rn = class {
		isValid() {
			return !0;
		}
	};
	si = rn, si.instance = new si();
	var Th = class {
		constructor(e) {
...
`);
	...
	ia = li, ia.chrKeys = /* @__PURE__ */ new Map();
	var kh = class {
		compute(e, t, n = rn.instance, r) {
			if (e.length === 0 || t.length === 0) return at.trivial(e, t);
			const i = new ai(e.length, t.length), s = new ai(e.length, t.length), a = new ai(e.length, t.length);
			...
			function u(p, b) {
				(p + 1 !== l || b + 1 !== c) && o.push(new de(new Y(p + 1, l), new Y(b + 1, c))), l = p, c = b;
			}
		...
		compute(e, t, n = rn.instance) {
			...
			o.set(0, s(0, 0));
			const l = new Lh();
			l.set(0, o.get(0) === 0 ? null : new sa(null, 0, 0, o.get(0)));
			let c = 0;
			e: for (;;) {
				...
				for (c = b; c <= g; c += 2) {
					...
					if (l.set(c, I !== k ? new sa(D, k, E, I - k) : D), o.get(c) === r.length && o.get(c) - c === i.length) break e;
				}
			}
			let u = l.get(c);
			const d = [];
			...
			for (;;) {
				...
				if ((b !== m || g !== p) && d.push(new de(new Y(b, m), new Y(g, p))), !u) break;
		...
		getBoundaryScore(e) {
			const t = la(e > 0 ? this.elements[e - 1] : -1), n = la(e < this.elements.length ? this.elements[e] : -1);
			if (t === 7 && n === 8) return 0;
			if (t === 8) return 150;
			let r = 0;
			return t !== n && (r += 10, t === 0 && n === 1 && (r += 1)), r += oa(t), r += oa(n), r;
		}
		translateOffset(e, t = "right") {
			const n = it(this.firstElementOffsetByLineIdx, (i) => i <= e), r = e - this.firstElementOffsetByLineIdx[n];
			return new Q(this.range.startLineNumber + n, 1 + this.lineStartOffsets[n] + r + (r === 0 && t === "left" ? 0 : this.trimmedWsLengthsByLineIdx[n]));
		}
		translateRange(e) {
		...
		findSubWordContaining(e) {
			if (e < 0 || e >= this.elements.length || !ot(this.elements[e])) return;
			let t = e;
			for (; t > 0 && ot(this.elements[t - 1]) && !aa(this.elements[t]);) t--;
			let n = e;
			for (; n < this.elements.length && ot(this.elements[n]) && !aa(this.elements[n]);) n++;
			return new Y(t, n);
		}
		countLinesIn(e) {
		...
		extendToFullLines(e) {
			return new Y(nt(this.firstElementOffsetByLineIdx, (t) => t <= e.start) ?? 0, hh(this.firstElementOffsetByLineIdx, (t) => e.endExclusive <= t) ?? this.elements.length);
		}
	};
	function ot(e) {
	...
	function Ah(e, t, n, r, i, s) {
		let { moves: a, excludedChanges: o } = Mh(e, t, n, s);
		if (!s.isValid()) return [];
		const l = Eh(e.filter((c) => !o.has(c)), r, i, t, n, s);
		return oh(a, l), a = Nh(a), a = a.filter((c) => {
			const u = c.original.toOffsetRange().slice(t).map((d) => d.trim());
			return u.join(`
`).length >= 15 && Rh(u, (d) => d.length >= 2) >= 2;
		}), a = Ch(e, a), a;
	}
	function Rh(e, t) {
		let n = 0;
	...
	function Mh(e, t, n, r) {
		const i = [], s = e.filter((l) => l.modified.isEmpty && l.original.length >= 3).map((l) => new li(l.original, t, l)), a = new Set(e.filter((l) => l.original.isEmpty && l.modified.length >= 3).map((l) => new li(l.modified, n, l))), o = /* @__PURE__ */ new Set();
		for (const l of s) {
			let c = -1, u;
			for (const d of a) {
			...
			if (c > .9 && u && (a.delete(u), i.push(new rt(l.range, u.range)), o.add(l.source), o.add(u.source)), !r.isValid()) return {
				moves: i,
				excludedChanges: o
			};
		}
	...
	function Eh(e, t, n, r, i, s) {
		...
		for (const m of e) for (let p = m.original.startLineNumber; p < m.original.endLineNumberExclusive - 2; p++) {
			...
			o.add(b, { range: new G(p, p + 3) });
		}
		const l = [];
		e.sort(wt((m) => m.modified.startLineNumber, _t));
		for (const m of e) {
			let p = [];
			for (let b = m.modified.startLineNumber; b < m.modified.endLineNumberExclusive - 2; b++) {
				const g = `${n[b - 1]}:${n[b + 1 - 1]}:${n[b + 2 - 1]}`, v = new G(b, b + 3), T = [];
				o.forEach(g, ({ range: k }) => {
					for (const I of p) if (I.originalLineRange.endLineNumberExclusive + 1 === k.endLineNumberExclusive && I.modifiedLineRange.endLineNumberExclusive + 1 === v.endLineNumberExclusive) {
						I.originalLineRange = new G(I.originalLineRange.startLineNumber, k.endLineNumberExclusive), I.modifiedLineRange = new G(I.modifiedLineRange.startLineNumber, v.endLineNumberExclusive), T.push(I);
						return;
					}
					const E = {
						modifiedLineRange: v,
		...
		l.sort(lh(wt((m) => m.modifiedLineRange.length, _t)));
		const c = new Kt(), u = new Kt();
		for (const m of l) {
			...
			for (const T of v.ranges) {
				...
				a.push(new rt(E, k)), c.addRange(k), u.addRange(E);
			}
		}
		a.sort(wt((m) => m.original.startLineNumber, _t));
		const d = new ei(e);
		for (let m = 0; m < a.length; m++) {
			...
			for (I = 0; I < v; I++) {
				...
				if (w > r.length || f > i.length || c.contains(f) || u.contains(w) || !ca(r[w - 1], i[f - 1], s)) break;
			}
			I > 0 && (u.addRange(new G(p.original.startLineNumber - I, p.original.startLineNumber)), c.addRange(new G(p.modified.startLineNumber - I, p.modified.startLineNumber)));
			let D;
			for (D = 0; D < E; D++) {
				const w = p.original.endLineNumberExclusive + D, f = p.modified.endLineNumberExclusive + D;
				if (w > r.length || f > i.length || c.contains(f) || u.contains(w) || !ca(r[w - 1], i[f - 1], s)) break;
			}
			D > 0 && (u.addRange(new G(p.original.endLineNumberExclusive, p.original.endLineNumberExclusive + D)), c.addRange(new G(p.modified.endLineNumberExclusive, p.modified.endLineNumberExclusive + D))), (I > 0 || D > 0) && (a[m] = new rt(new G(p.original.startLineNumber - I, p.original.endLineNumberExclusive + D), new G(p.modified.startLineNumber - I, p.modified.endLineNumberExclusive + D)));
		}
		return a;
	}
	function ca(e, t, n) {
		...
		const r = new ra().compute(new sn([e], new V(1, 1, 1, e.length), !1), new sn([t], new V(1, 1, 1, t.length), !1), n);
		let i = 0;
		...
		function a(l) {
			let c = 0;
			for (let u = 0; u < e.length; u++) oi(l.charCodeAt(u)) || c++;
			return c;
		}
	...
	function Nh(e) {
		...
		e.sort(wt((n) => n.original.startLineNumber, _t));
		const t = [e[0]];
	...
	function Ch(e, t) {
		const n = new ei(e);
		return t = t.filter((r) => (n.findLastMonotonous((i) => i.original.startLineNumber < r.original.endLineNumberExclusive) || new rt(new G(1, 1), new G(1, 1))) !== nt(e, (i) => i.modified.startLineNumber < r.modified.endLineNumberExclusive)), t;
	}
	function ha(e, t, n) {
		let r = n;
		return r = ua(e, t, r), r = ua(e, t, r), r = Ih(e, t, r), r;
	}
	function ua(e, t, n) {
		...
		for (let s = 1; s < n.length; s++) {
			...
			if (o.seq1Range.isEmpty || o.seq2Range.isEmpty) {
				...
				if (c--, c === l) {
					r[r.length - 1] = new de(new Y(a.seq1Range.start, o.seq1Range.endExclusive - l), new Y(a.seq2Range.start, o.seq2Range.endExclusive - l));
					continue;
				}
				o = o.delta(-c);
			}
		...
		for (let s = 0; s < r.length - 1; s++) {
			...
			if (o.seq1Range.isEmpty || o.seq2Range.isEmpty) {
				...
				if (c === l) {
					r[s + 1] = new de(new Y(o.seq1Range.start + l, a.seq1Range.endExclusive), new Y(o.seq2Range.start + l, a.seq2Range.endExclusive));
					continue;
				}
				c > 0 && (o = o.delta(c));
			}
	...
	function Ih(e, t, n) {
		...
		for (let r = 0; r < n.length; r++) {
			const i = r > 0 ? n[r - 1] : void 0, s = n[r], a = r + 1 < n.length ? n[r + 1] : void 0, o = new Y(i ? i.seq1Range.endExclusive + 1 : 0, a ? a.seq1Range.start - 1 : e.length), l = new Y(i ? i.seq2Range.endExclusive + 1 : 0, a ? a.seq2Range.start - 1 : t.length);
			s.seq1Range.isEmpty ? n[r] = da(s, e, t, o, l) : s.seq2Range.isEmpty && (n[r] = da(s.swap(), t, e, l, o).swap());
		}
		return n;
	}
	...
	function Dh(e, t, n) {
		...
		do {
			...
			for (let c = 1; c < r.length; c++) {
				let m = function(p, b) {
					const g = new Y(d.seq1Range.endExclusive, u.seq1Range.start);
		...
		return ah(r, (l, c, u) => {
			...
			const g = e.getText(new Y(c.seq1Range.endExclusive, p.endExclusive));
			m(g) && (d = d.deltaEnd(g.length));
			const v = de.fromOffsetPairs(l ? l.getEndExclusives() : De.zero, u ? u.getStarts() : De.max), T = d.intersect(v);
...
`);
	...
	var Wh = class {
		constructor() {
			this.dynamicProgrammingDiffing = new kh(), this.myersDiffingAlgorithm = new ra();
		}
		computeDiff(e, t, n) {
			if (e.length <= 1 && ih(e, t, (I, D) => I === D)) return new Zt([], [], !1);
			if (e.length === 1 && e[0].length === 0 || t.length === 1 && t[0].length === 0) return new Zt([new yt(new G(1, e.length + 1), new G(1, t.length + 1), [new Ne(new V(1, 1, e.length, e[e.length - 1].length + 1), new V(1, 1, t.length, t[t.length - 1].length + 1))])], [], !1);
			const r = n.maxComputationTimeMs === 0 ? rn.instance : new Th(n.maxComputationTimeMs), i = !n.ignoreTrimWhitespace, s = /* @__PURE__ */ new Map();
			function a(I) {
				let D = s.get(I);
				return D === void 0 && (D = s.size, s.set(I, D)), D;
			}
			const o = e.map((I) => a(I.trim())), l = t.map((I) => a(I.trim())), c = new fa(o, e), u = new fa(l, t), d = c.length + u.length < 1700 ? this.dynamicProgrammingDiffing.compute(c, u, r, (I, D) => e[I] === t[D] ? t[D].length === 0 ? .1 : 1 + Math.log(1 + t[D].length) : .99) : this.myersDiffingAlgorithm.compute(c, u, r);
			let m = d.diffs, p = d.hitTimeout;
			m = ha(c, u, m), m = Uh(c, u, m);
			const b = [], g = (I) => {
				if (i) for (let D = 0; D < I; D++) {
					const w = v + D, f = T + D;
					if (e[w] !== t[f]) {
						const _ = this.refineDiff(e, t, new de(new Y(w, w + 1), new Y(f, f + 1)), r, i, n);
			...
			let v = 0, T = 0;
			for (const I of m) {
				Ut(() => I.seq1Range.start - v === I.seq2Range.start - T), g(I.seq1Range.start - v), v = I.seq1Range.endExclusive, T = I.seq2Range.endExclusive;
				const D = this.refineDiff(e, t, I, r, i, n);
				D.hitTimeout && (p = !0);
				for (const w of D.mappings) b.push(w);
			}
			g(e.length - v);
			const k = Ks(b, new tn(e), new tn(t));
			let E = [];
			return n.computeMoves && (E = this.computeMoves(k, e, t, o, l, r, i, n)), Ut(() => {
				function I(w, f) {
		...
		computeMoves(e, t, n, r, i, s, a, o) {
			return Ah(e, t, n, r, i, s).map((l) => new nh(l, Ks(this.refineDiff(t, n, new de(l.original.toOffsetRange(), l.modified.toOffsetRange()), s, a, o).mappings, new tn(t), new tn(n), !0)));
		}
		refineDiff(e, t, n, r, i, s) {
			const a = Ph(n).toRangeMapping2(e, t), o = new sn(e, a.originalRange, i), l = new sn(t, a.modifiedRange, i), c = o.length + l.length < 500 ? this.dynamicProgrammingDiffing.compute(o, l, r) : this.myersDiffingAlgorithm.compute(o, l, r);
			let u = c.diffs;
			return u = ha(o, l, u), u = ma(o, l, u, (d, m) => d.findWordContaining(m)), s.extendToSubwords && (u = ma(o, l, u, (d, m) => d.findSubWordContaining(m), !0)), u = zh(o, l, u), u = Dh(o, l, u), {
				mappings: u.map((d) => new Ne(o.translateRange(d.seq1Range), l.translateRange(d.seq2Range))),
				hitTimeout: c.hitTimeout
			};
		}
	};
	function Ph(e) {
		return new rt(new G(e.seq1Range.start + 1, e.seq1Range.endExclusive + 1), new G(e.seq2Range.start + 1, e.seq2Range.endExclusive + 1));
	}
	const ga = {
		getLegacy: () => new wh(),
		getDefault: () => new Wh()
	...
	var x = class {
		...
		static toRGBA(t) {
			...
			if (r === 0) a = o = l = i;
				...
				a = zt._hue2rgb(u, c, n + 1 / 3), o = zt._hue2rgb(u, c, n), l = zt._hue2rgb(u, c, n - 1 / 3);
			}
		...
		get hsva() {
			return this._hsva ? this._hsva : an.fromRGBA(this.rgba);
		}
		constructor(t) {
			if (t) if (t instanceof x) this.rgba = t;
			else if (t instanceof je) this._hsla = t, this.rgba = je.toRGBA(t);
			else if (t instanceof an) this._hsva = t, this.rgba = an.toRGBA(t);
			else throw new Error("Invalid color ctor argument");
			else throw new Error("Color needs a value");
		}
		equals(t) {
			return !!t && x.equals(this.rgba, t.rgba) && je.equals(this.hsla, t.hsla) && an.equals(this.hsva, t.hsva);
		}
		getRelativeLuminance() {
			const t = be._relativeLuminanceForComponent(this.rgba.r), n = be._relativeLuminanceForComponent(this.rgba.g), r = be._relativeLuminanceForComponent(this.rgba.b);
		...
		lighten(t) {
			return new be(new je(this.hsla.h, this.hsla.s, this.hsla.l + this.hsla.l * t, this.hsla.a));
		}
		darken(t) {
			return new be(new je(this.hsla.h, this.hsla.s, this.hsla.l - this.hsla.l * t, this.hsla.a));
		}
		...
		opposite() {
			return new be(new x(255 - this.rgba.r, 255 - this.rgba.g, 255 - this.rgba.b, this.rgba.a));
		}
		mix(t, n = .5) {
			const r = Math.min(Math.max(n, 0), 1), i = this.rgba, s = t.rgba;
			return new be(new x(i.r + (s.r - i.r) * r, i.g + (s.g - i.g) * r, i.b + (s.b - i.b) * r, i.a + (s.a - i.a) * r));
		}
		makeOpaque(t) {
			if (this.isOpaque() || t.rgba.a !== 1) return this;
			const { r: n, g: r, b: i, a: s } = this.rgba;
			return new be(new x(t.rgba.r - s * (t.rgba.r - n), t.rgba.g - s * (t.rgba.g - r), t.rgba.b - s * (t.rgba.b - i), 1));
		}
		toString() {
			return this._toString || (this._toString = be.Format.CSS.format(this)), this._toString;
		}
		toNumber32Bit() {
	...
	se = kt, se.white = new se(new x(255, 255, 255, 1)), se.black = new se(new x(0, 0, 0, 1)), se.red = new se(new x(255, 0, 0, 1)), se.blue = new se(new x(0, 0, 255, 1)), se.green = new se(new x(0, 255, 0, 1)), se.cyan = new se(new x(0, 255, 255, 1)), se.lightgrey = new se(new x(211, 211, 211, 1)), se.transparent = new se(new x(0, 0, 0, 0)), (function(e) {
		(function(t) {
			(function(n) {
				function r(g) {
	...
	function qh(e, t) {
		if (!e) return;
		const n = kt.Format.CSS.parseHex(t);
		if (n) return {
			range: e,
	...
	function wa(e, t, n) {
		...
		const r = ba(t[0].values());
		return {
			range: e,
			color: ci(r[0], r[1], r[2], n ? r[3] : 1)
		};
	}
	function _a(e, t, n) {
		if (!e || t.length !== 1) return;
		const r = ba(t[0].values()), i = new kt(new je(r[0], r[1] / 100, r[2] / 100, n ? r[3] : 1));
		return {
			range: e,
			color: ci(i.rgba.r, i.rgba.g, i.rgba.b, i.rgba.a)
		};
	}
	function Lt(e, t) {
	...
	function Oh(e) {
		...
		if (n.length > 0) for (const r of n) {
			...
			if (!a) continue;
			let o;
			s === "rgb" ? o = wa(St(e, r), Lt(a, /^\(\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*,\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*,\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*\)$/gm), !1) : s === "rgba" ? o = wa(St(e, r), Lt(a, /^\(\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*,\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*,\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*,\s*(0[.][0-9]+|[.][0-9]+|[01][.]|[01])\s*\)$/gm), !0) : s === "hsl" ? o = _a(St(e, r), Lt(a, /^\(\s*((?:360(?:\.0+)?|(?:36[0]|3[0-5][0-9]|[12][0-9][0-9]|[1-9]?[0-9])(?:\.\d+)?))\s*[\s,]\s*(100|\d{1,2}[.]\d*|\d{1,2})%\s*[\s,]\s*(100|\d{1,2}[.]\d*|\d{1,2})%\s*\)$/gm), !1) : s === "hsla" ? o = _a(St(e, r), Lt(a, /^\(\s*((?:360(?:\.0+)?|(?:36[0]|3[0-5][0-9]|[12][0-9][0-9]|[1-9]?[0-9])(?:\.\d+)?))\s*[\s,]\s*(100|\d{1,2}[.]\d*|\d{1,2})%\s*[\s,]\s*(100|\d{1,2}[.]\d*|\d{1,2})%\s*[\s,]\s*(0[.][0-9]+|[.][0-9]+|[01][.]0*|[01])\s*\)$/gm), !0) : s === "#" && (o = qh(St(e, r), s + a)), o && t.push(o);
		}
		return t;
	}
	function Fh(e) {
	...
	function Gh(e, t) {
		...
		if (Cl(s)) return n;
		for (let a = 1; a <= r; a += va - Vh) {
			const o = Math.min(a + va - 1, r), l = [];
...
`) + 1, k = u.index - T + 1, E = b[b.length - 1], I = {
	...
	ya = Symbol.asyncIterator;
	hi = class ke {
		constructor(t, n) {
			this._onReturn = n, this._producerConsumer = new Jh(), this._iterator = {
	...
	var Kh = class {
		...
		onEvents(e) {
			...
			const t = e.changes;
			for (const n of t) this._acceptDeleteRange(n.range), this._acceptInsertText(new Q(n.range.startLineNumber, n.range.startColumn), n.text);
		...
		$acceptNewModel(e) {
			this._models[e.url] = new iu(Fn.parse(e.url), e.lines, e.EOL, e.versionId);
		}
		...
		findMatches(e) {
			...
			for (let n = 0; n < this._lines.length; n++) {
				const r = this._lines[n], i = this.offsetAt(new Q(n + 1, 1)), s = r.matchAll(e);
		...
		getWordAtPosition(e, t) {
			const n = Yn(e.column, js(t), this._lines[e.lineNumber - 1], 0);
		...
		constructor(t = null) {
			this._foreignModule = t, this._requestHandlerBrand = void 0, this._workerTextModelSyncServer = new nu();
		}
		dispose() {}
		async $ping() {
			return "pong";
		...
		static computeDiff(t, n, r, i) {
			const s = i === "advanced" ? ga.getDefault() : ga.getLegacy(), a = t.getLinesContent(), o = n.getLinesContent(), l = s.computeDiff(a, o, r), c = l.changes.length > 0 ? !1 : this._modelsAreIdentical(t, n);
			function u(d) {
				return d.map((m) => [
		...
		async $computeMoreMinimalEdits(t, n, r) {
			...
			for (let { range: l, text: c, eol: u } of n) {
				...
				if (Math.max(c.length, d.length) > yn._diffLimit) {
					s.push({
						range: l,
						text: c
					});
		...
		async $textualSuggest(t, n, r, i) {
			const s = new Yi(), a = new RegExp(r, i), o = /* @__PURE__ */ new Set();
			e: for (const l of t) {
				const c = this._getModel(l);
				if (c) {
					for (const u of c.words(a)) if (!(u === n || !isNaN(Number(u))) && (o.add(u), o.size > yn._suggestionsLimit)) break e;
				}
			}
			return {
				words: Array.from(o),
		...
		async $navigateValueSet(t, n, r, i, s) {
			...
			return _r.INSTANCE.navigateValueSet(n, l, c, u, r);
		}
		$fmr(t, n) {
	...
	di = ka, di._diffLimit = 1e5, di._suggestionsLimit = 1e4, typeof importScripts == "function" && (globalThis.monaco = Pc());
	var Sa, La = class Bi {
		static getChannel(t) {
	...
	Sa = La, Sa.CHANNEL_NAME = "editorWorkerHost";
	function ru(e) {
		let t;
		const n = Yl((r) => {
			const i = La.getChannel(r);
			return t = e({
	...
	var ou = /{([^}]+)}/g;
	function lu(e, t) {
		return Object.keys(t).length === 0 ? e : e.replace(ou, (n, r) => t[r] ?? n);
	}
	var xa;
	(function(e) {
		function t(n) {
	...
	(function(e) {
		function t(r, i) {
			return r === Number.MAX_VALUE && (r = on.MAX_VALUE), i === Number.MAX_VALUE && (i = on.MAX_VALUE), {
				line: r,
				character: i
			};
		}
	...
	(function(e) {
		...
		function n(r) {
			let i = r;
			return M.objectLiteral(i) && ne.is(i.start) && ne.is(i.end);
		}
		e.is = n;
	})($ || ($ = {}));
	var ln;
	...
	(function(e) {
		...
		function n(r) {
			const i = r;
			return M.objectLiteral(i) && $.is(i.range) && fi.is(i.color);
		}
		e.is = n;
	})(Ma || (Ma = {}));
	var Ea;
	(function(e) {
		...
		function n(r) {
			const i = r;
			return M.objectLiteral(i) && M.string(i.label) && (M.undefined(i.textEdit) || re.is(i)) && (M.undefined(i.additionalTextEdits) || M.typedArray(i.additionalTextEdits, re.is));
		}
		e.is = n;
	})(Ea || (Ea = {}));
	var cn;
	...
	(function(e) {
		...
		function n(r) {
			let i = r;
			return M.defined(i) && ln.is(i.location) && M.string(i.message);
		}
		e.is = n;
	})(pi || (pi = {}));
	var Ca;
	...
	(function(e) {
		...
		function n(r) {
			var i;
			let s = r;
			return M.defined(s) && $.is(s.range) && M.string(s.message) && (M.number(s.severity) || M.undefined(s.severity)) && (M.integer(s.code) || M.string(s.code) || M.undefined(s.code)) && (M.undefined(s.codeDescription) || M.string((i = s.codeDescription) === null || i === void 0 ? void 0 : i.href)) && (M.string(s.source) || M.undefined(s.source)) && (M.undefined(s.relatedInformation) || M.typedArray(s.relatedInformation, pi.is));
		}
		e.is = n;
	})(hn || (hn = {}));
	var lt;
	...
	(function(e) {
		...
		function i(s) {
			const a = s;
			return re.is(a) && (gi.is(a.annotationId) || ct.is(a.annotationId));
		}
		e.is = i;
	})(Ha || (Ha = {}));
	var bi;
	(function(e) {
		...
		function n(r) {
			let i = r;
			return M.defined(i) && Ti.is(i.textDocument) && Array.isArray(i.edits);
		}
		e.is = n;
	})(bi || (bi = {}));
	var wi;
	(function(e) {
		...
		function n(r) {
			let i = r;
			return i && i.kind === "create" && M.string(i.uri) && (i.options === void 0 || (i.options.overwrite === void 0 || M.boolean(i.options.overwrite)) && (i.options.ignoreIfExists === void 0 || M.boolean(i.options.ignoreIfExists))) && (i.annotationId === void 0 || ct.is(i.annotationId));
		}
		e.is = n;
	})(wi || (wi = {}));
	var _i;
	(function(e) {
		...
		function n(r) {
			let i = r;
			return i && i.kind === "rename" && M.string(i.oldUri) && M.string(i.newUri) && (i.options === void 0 || (i.options.overwrite === void 0 || M.boolean(i.options.overwrite)) && (i.options.ignoreIfExists === void 0 || M.boolean(i.options.ignoreIfExists))) && (i.annotationId === void 0 || ct.is(i.annotationId));
		}
		e.is = n;
	})(_i || (_i = {}));
	var vi;
	(function(e) {
		...
		function n(r) {
			let i = r;
			return i && i.kind === "delete" && M.string(i.uri) && (i.options === void 0 || (i.options.recursive === void 0 || M.boolean(i.options.recursive)) && (i.options.ignoreIfNotExists === void 0 || M.boolean(i.options.ignoreIfNotExists))) && (i.annotationId === void 0 || ct.is(i.annotationId));
		}
		e.is = n;
	})(vi || (vi = {}));
	var yi;
	(function(e) {
		function t(n) {
			let r = n;
			return r && (r.changes !== void 0 || r.documentChanges !== void 0) && (r.documentChanges === void 0 || r.documentChanges.every((i) => M.string(i.kind) ? wi.is(i) || _i.is(i) || vi.is(i) : bi.is(i)));
		}
		e.is = t;
	})(yi || (yi = {}));
	var Ua;
	...
	(function(e) {
		function t(n) {
			const r = n;
			return M.objectLiteral(n) && Ce.is(r.kind) && M.string(r.value);
		}
		e.is = t;
	})(xt || (xt = {}));
	var me;
	...
	(function(e) {
		function t(n) {
			let r = n;
			return !!r && M.objectLiteral(r) && (xt.is(r.contents) || un.is(r.contents) || M.typedArray(r.contents, un.is)) && (n.range === void 0 || $.is(n.range));
		}
		e.is = t;
	})(ja || (ja = {}));
	var $a;
	...
	(function(e) {
		...
		function n(r) {
			let i = r;
			return M.defined(i) && M.typedArray(i.diagnostics, hn.is) && (i.only === void 0 || M.typedArray(i.only, M.string)) && (i.triggerKind === void 0 || i.triggerKind === mn.Invoked || i.triggerKind === mn.Automatic);
		}
		e.is = n;
	})(Za || (Za = {}));
	var Ka;
	(function(e) {
		...
		function n(r) {
			let i = r;
			return i && M.string(i.title) && (i.diagnostics === void 0 || M.typedArray(i.diagnostics, hn.is)) && (i.kind === void 0 || M.string(i.kind)) && (i.edit !== void 0 || i.command !== void 0) && (i.command === void 0 || lt.is(i.command)) && (i.isPreferred === void 0 || M.boolean(i.isPreferred)) && (i.edit === void 0 || yi.is(i.edit));
		}
		e.is = n;
	})(Ka || (Ka = {}));
	var eo;
	(function(e) {
		...
		function n(r) {
			let i = r;
			return M.defined(i) && $.is(i.range) && (M.undefined(i.command) || lt.is(i.command));
		}
		e.is = n;
	})(eo || (eo = {}));
	var to;
	...
	(function(e) {
		...
		function n(r) {
			const i = r;
			return M.objectLiteral(i) && (i.tooltip === void 0 || M.string(i.tooltip) || xt.is(i.tooltip)) && (i.location === void 0 || ln.is(i.location)) && (i.command === void 0 || lt.is(i.command));
		}
		e.is = n;
	})(Ai || (Ai = {}));
	var ho;
	(function(e) {
		...
		function n(r) {
			const i = r;
			return M.objectLiteral(i) && ne.is(i.position) && (M.string(i.label) || M.typedArray(i.label, Ai.is)) && (i.kind === void 0 || xi.is(i.kind)) && i.textEdits === void 0 || M.typedArray(i.textEdits, re.is) && (i.tooltip === void 0 || M.string(i.tooltip) || xt.is(i.tooltip)) && (i.paddingLeft === void 0 || M.boolean(i.paddingLeft)) && (i.paddingRight === void 0 || M.boolean(i.paddingRight));
		}
		e.is = n;
	})(ho || (ho = {}));
	var uo;
	...
	(function(e) {
		function t(n) {
			const r = n;
			return M.objectLiteral(r) && mi.is(r.uri) && M.string(r.name);
		}
		e.is = t;
	})(wo || (wo = {}));
	var _o;
...
` && r++;
	...
	var vo = class Vi {
		...
		getLineOffsets() {
			return this._lineOffsets === void 0 && (this._lineOffsets = yo(this._content, !0)), this._lineOffsets;
		}
		positionAt(t) {
	...
	(function(e) {
		function t(i, s, a, o) {
			return new vo(i, s, a, o);
		}
		e.create = t;
		function n(i, s, a) {
			if (i instanceof vo) return i.update(s, a), i;
			throw new Error("TextDocument.update: document must be created by TextDocument.create");
		}
		e.update = n;
		function r(i, s) {
			let a = i.getText(), o = Mi(s.map(hu), (u, d) => {
				let m = u.range.start.line - d.range.start.line;
				return m === 0 ? u.range.start.character - d.range.start.character : m;
			}), l = 0;
			const c = [];
	...
	var uu = class {
		...
		skipWhitespace() {
			return this.advanceWhileChar((e) => e === wu || e === _u || e === pu || e === bu || e === gu) > 0;
		}
		advanceWhileChar(e) {
	...
	const So = 33, ht = 45, pn = 60, xe = 62, Ni = 47, du = 61, mu = 34, fu = 39, pu = 10, gu = 13, bu = 12, wu = 32, _u = 9, vu = {
	...
	function fe(e, t = 0, n = B.WithinContent, r = !1) {
		...
		function T() {
			...
			switch (s) {
				...
				case B.WithinDoctype: return i.advanceIfChar(xe) ? (s = B.WithinContent, g(k, U.EndDoctypeTag)) : (i.advanceUntilChar(xe), g(k, U.Doctype));
				case B.WithinContent:
					if (i.advanceIfChar(pn)) {
						if (!i.eos() && i.peekChar() === So) {
							if (i.advanceIfChars([
								So,
								ht,
								ht
						...
						return i.advanceIfChar(Ni) ? (s = B.AfterOpeningEndTag, g(k, U.EndTagOpen)) : (s = B.AfterOpeningStartTag, g(k, U.StartTagOpen));
					}
					return i.advanceUntilChar(pn), g(k, U.Content);
				case B.AfterOpeningEndTag: return p().length > 0 ? (s = B.WithinEndTag, g(k, U.EndTag)) : i.skipWhitespace() ? g(k, U.Whitespace, ve("Tag name must directly follow the open bracket.")) : (s = B.WithinEndTag, i.advanceUntilChar(xe), k < i.pos() ? g(k, U.Unknown, ve("End tag name expected.")) : T());
				case B.WithinEndTag:
					if (i.skipWhitespace()) return g(k, U.Whitespace);
					if (i.advanceIfChar(xe)) return s = B.WithinContent, g(k, U.EndTagClose);
					if (r && i.peekChar() === pn) return s = B.WithinContent, g(k, U.EndTagClose, ve("Closing bracket missing."));
					E = ve("Closing bracket expected.");
					break;
				case B.AfterOpeningStartTag: return u = p(), m = void 0, d = void 0, u.length > 0 ? (c = !1, s = B.WithinTag, g(k, U.StartTag)) : i.skipWhitespace() ? g(k, U.Whitespace, ve("Tag name must directly follow the open bracket.")) : (s = B.WithinTag, i.advanceUntilChar(xe), k < i.pos() ? g(k, U.Unknown, ve("Start tag name expected.")) : T());
				case B.WithinTag: return i.skipWhitespace() ? (c = !0, g(k, U.Whitespace)) : c && (d = b(), d.length > 0) ? (s = B.AfterAttributeName, c = !1, g(k, U.AttributeName)) : i.advanceIfChars([Ni, xe]) ? (s = B.WithinContent, g(k, U.StartTagSelfClose)) : i.advanceIfChar(xe) ? (u === "script" ? m && vu[m] ? s = B.WithinContent : s = B.WithinScriptContent : u === "style" ? s = B.WithinStyleContent : s = B.WithinContent, g(k, U.StartTagClose)) : r && i.peekChar() === pn ? (s = B.WithinContent, g(k, U.StartTagClose, ve("Closing bracket missing."))) : (i.advance(1), g(k, U.Unknown, ve("Unexpected character in tag.")));
				case B.AfterAttributeName: return i.skipWhitespace() ? (c = !0, g(k, U.Whitespace)) : i.advanceIfChar(du) ? (s = B.BeforeAttributeValue, g(k, U.DelimiterAssign)) : (s = B.WithinTag, T());
				case B.BeforeAttributeValue:
					if (i.skipWhitespace()) return g(k, U.Whitespace);
					let I = i.advanceIfRegExp(/^[^\s"'`=<>]+/);
					if (I.length > 0 && (i.peekChar() === xe && i.peekChar(-1) === Ni && (i.goBack(1), I = I.substring(0, I.length - 1)), d === "type" && (m = I), I.length > 0)) return s = B.WithinTag, c = !1, g(k, U.AttributeValue);
					const D = i.peekChar();
					return D === fu || D === mu ? (i.advance(1), i.advanceUntilChar(D) && i.advance(1), d === "type" && (m = i.getSource().substring(k + 1, i.pos() - 1)), s = B.WithinTag, c = !1, g(k, U.AttributeValue)) : (s = B.WithinTag, c = !1, T());
	...
	var xo = class {
		...
		findNodeBefore(e) {
			const t = Lo(this.children, (n) => e <= n.start) - 1;
			if (t >= 0) {
		...
		parse(e, t) {
			const n = fe(e, void 0, void 0, !0), r = new xo(0, e.length, [], void 0);
			let i = r, s = -1, a, o = null, l = n.scan();
			for (; l !== U.EOS;) {
				switch (l) {
					case U.StartTagOpen:
						const c = new xo(n.getTokenOffset(), e.length, [], i);
...
`,
	...
	const Su = 97, Lu = 122, xu = 65, Au = 90, Ru = 48, Mu = 57;
	function Rt(e, t) {
		const n = e.charCodeAt(t);
		return Su <= n && n <= Lu || xu <= n && n <= Au || Ru <= n && n <= Mu;
	}
	function gn(e) {
		return typeof e < "u";
	}
...
`)), r.value !== "") return r;
	...
	var Nu = class {
		...
		async computeCompletions(e, t) {
			...
			for (const r of this.atributeCompletions) {
				const i = Iu(e.getText(r.range));
				if (zu(i)) if (i === "." || i === "..") n.isIncomplete = !0;
				else {
					const s = Hu(r.value, i, r.range), a = await this.providePathSuggestions(r.value, s, e, t);
		...
		async providePathSuggestions(e, t, n, r) {
			...
			if (s) try {
				...
				for (const [l, c] of o) l.charCodeAt(0) !== Cu && a.push(Uu(l, c === Ei.Directory, t));
				return a;
			} catch {}
			return [];
		}
	...
	function Hu(e, t, n) {
		...
		if (i === -1) r = Du(n, 1, -1);
			...
			let l;
			o !== -1 ? l = Mt(a, o) : l = Mt(n.end, -1), r = $.create(a, l);
		}
		return r;
	}
	function Uu(e, t, n) {
				...
				return a.forEach((H) => {
					H.provideTags().forEach((q) => {
						i.items.push({
							label: q.name,
							kind: me.Property,
							documentation: $e(q, void 0, l),
							textEdit: re.replace(N, q.name),
...
\r`.indexOf(N) >= 0) return c.substring(R, y);
			...
			function k(y, R, N = u) {
				const H = g(y, N), q = Mo(c, N, B.WithinEndTag, U.EndTagClose) ? "" : ">";
				let P = d;
				for (R && (P = P.parent); P;) {
					const W = P.tag;
					if (W && (!P.closed || P.endTagStart && P.endTagStart > u)) {
						const X = {
							...
							kind: me.Property,
							filterText: "/" + W,
							textEdit: re.replace(H, "/" + W + q),
							insertTextFormat: ye.PlainText
						}, K = T(P.start), ae = T(y - 1);
						if (K !== null && ae !== null && K !== ae) {
							const Ie = K + "</" + W + q;
							X.textEdit = re.replace(g(y - 1 - ae.length), Ie), X.filterText = ae + "</" + W;
						}
						return i.items.push(X), i;
					}
					P = P.parent;
				}
				return R || a.forEach((W) => {
					W.provideTags().forEach((X) => {
						i.items.push({
							label: "/" + X.name,
							kind: me.Property,
							documentation: $e(X, void 0, l),
							filterText: "/" + X.name + q,
							textEdit: re.replace(H, "/" + X.name + q),
							insertTextFormat: ye.PlainText
						});
					});
				}), i;
			...
			function w(y, R = u) {
				...
				let P = "";
				if (!Mo(c, R, B.AfterAttributeName, U.DelimiterAssign)) {
					const X = r?.attributeDefaultValue ?? "doublequotes";
					X === "empty" ? P = "=$1" : X === "singlequotes" ? P = "='$1'" : P = "=\"$1\"";
				}
				const W = D();
				return W[H] = !1, a.forEach((X) => {
					X.provideAttributes(p).forEach((K) => {
						if (W[K.name]) return;
						W[K.name] = !0;
						let ae = K.name, Ie;
						K.valueSet !== "v" && P.length && (ae = ae + P, (K.valueSet || K.name === "style") && (Ie = {
							title: "Suggest",
							command: "editor.action.triggerSuggest"
						})), i.items.push({
							label: K.name,
							kind: K.valueSet === "handler" ? me.Function : me.Value,
							documentation: $e(K, void 0, l),
							textEdit: re.replace(q, ae),
							insertTextFormat: ye.Snippet,
							command: Ie
						});
					});
			...
			function _(y, R = u) {
				...
				if (u > y && u <= R && Pu(c[y])) {
					const P = y + 1;
					let W = R;
					R > y && c[R - 1] === c[y] && W--, N = g(qu(c, u, P), Ou(c, u, W)), q = u >= P && u <= W ? c.substring(P, u) : "", H = !1;
				} else N = g(y, R), q = c.substring(y, u), H = !0;
				if (s.length > 0) {
					const P = p.toLowerCase(), W = b.toLowerCase(), X = g(y, R);
					for (const K of s) K.onHtmlAttributeValue && K.onHtmlAttributeValue({
						document: e,
						position: t,
						tag: P,
						attribute: W,
				...
				return a.forEach((P) => {
					P.provideValues(p, b).forEach((W) => {
						const X = H ? "\"" + W.name + "\"" : W.name;
						i.items.push({
							label: W.name,
							filterText: X,
							kind: me.Unit,
							documentation: $e(W, void 0, l),
							textEdit: re.replace(N, X),
							insertTextFormat: ye.PlainText
						});
					});
				}), L(), i;
			...
			function L() {
				...
				for (; y >= 0 && Rt(c, y);) y--, R--;
				if (y >= 0 && c[y] === "&") {
					const N = $.create(ne.create(t.line, R - 1), t);
					for (const H in At) if (ku(H, ";")) {
						const q = "&" + H;
						i.items.push({
							label: q,
							kind: me.Keyword,
							documentation: ve("Character entity representing '{0}'", At[H]),
							textEdit: re.replace(N, q),
							insertTextFormat: ye.PlainText
						});
					}
				}
			...
			function S(y, R) {
				...
				i.items.push({
					...
					textEdit: re.replace(N, "!DOCTYPE html>"),
					insertTextFormat: ye.PlainText
				});
			}
			let A = m.scan();
		...
		doesSupportMarkdown() {
			if (!gn(this.supportsMarkdown)) {
				if (!gn(this.lsOptions.clientCapabilities)) return this.supportsMarkdown = !0, this.supportsMarkdown;
		...
		doHover(e, t, n, r) {
			...
			function p(w, f) {
				...
				for (const C in At) {
					let z = null;
					if (_ === "&" + C) {
						let L = At[C].charCodeAt(0).toString(16).toUpperCase(), S = "U+";
						if (L.length < 4) {
							const y = 4 - L.length;
							let R = 0;
							for (; R < y;) S += "0", R += 1;
						...
						S += L;
						const A = ve("Character entity representing '{0}', unicode equivalent '{1}'", At[C], S);
						A ? z = {
							contents: A,
							range: f
						} : z = null;
			...
			function g() {
				...
				if (w >= 0 && l[w] === "&") {
					let z = null;
					return l[_] === ";" ? z = $.create(ne.create(t.line, f), ne.create(t.line, C + 1)) : z = $.create(ne.create(t.line, f), ne.create(t.line, C)), z;
				}
				return null;
			}
			function v(w) {
			...
			if (D) {
				const w = o.tag, f = Bu(e.getText(D)), _ = I(o.start, e.offsetAt(D.start));
...
`);
					...
					for (var D = 0, w = !1, f = !1, _ = !1, C = !1, z = !1, L = this._ch, S = !1, A, y, R; A = this._input.read(b), y = A !== "", R = L, this._ch = this._input.next(), this._ch === "\\" && this._input.hasNext() && (this._ch += this._input.next()), L = this._ch, this._ch;) if (this._ch === "/" && this._input.peek() === "*") {
						...
						var X = R === "\"" || R === "'";
						this.preserveSingleSpace(X || y), this.print_string(this._ch + this.eatString(this._ch)), this.eatWhitespace(!0);
						...
						var K = !1;
						this._input.lookBack("with") && (K = !0), this.preserveSingleSpace(y || K), this.print_string(this._ch), f && R === "$" && this._options.selector_separator_newline ? (this._output.add_new_line(), z = !0) : (this.eatWhitespace(), D++, this.indent());
					}
...
`);
					...
					if (f.text !== "") {
						...
						_.custom_beautifier_name === "javascript" && typeof this._js_beautify == "function" ? L = this._js_beautify : _.custom_beautifier_name === "css" && typeof this._css_beautify == "function" ? L = this._css_beautify : _.custom_beautifier_name === "html" && (L = function(W, X) {
							return new k(W, X, C._js_beautify, C._css_beautify).beautify();
...
`) === -1 || (_.is_inline_element || _.is_unformatted || w.print_newline(!1), this._calcluate_parent_multiline(w, _));
	...
	function Gu(e, t, n) {
		...
		if (t) {
			...
			for (; u > 0 && Io(r, u - 1);) u--;
			u === 0 || Co(r, u - 1) ? c = u : u < c && (c = u + 1);
			let d = e.offsetAt(t.end), m = d;
			for (; m < r.length && Io(r, m);) m++;
			(m === r.length || Co(r, m)) && (d = m), t = $.create(e.positionAt(c), e.positionAt(d));
			...
			if (i = d === r.length, r = r.substring(c, d), c !== 0) {
				const b = e.offsetAt(ne.create(t.start.line, 0));
				s = Yu(e.getText(), b, n);
			}
		} else t = $.create(ne.create(0, 0), e.positionAt(r.length));
		const o = {
			indent_size: a,
			indent_char: n.insertSpaces ? " " : "	",
			indent_empty_lines: pe(n, "indentEmptyLines", !1),
			wrap_line_length: pe(n, "wrapLineLength", 120),
			unformatted: Ci(n, "unformatted", void 0),
			content_unformatted: Ci(n, "contentUnformatted", void 0),
			...
			extra_liners: Ci(n, "extraLiners", void 0),
...
`,
		...
		let l = $u(Xu(r), o);
		if (s > 0) {
			const c = n.insertSpaces ? Ao(" ", a * s) : Ao("	", s);
			l = l.split(`
`).join(`
...
`.indexOf(e.charAt(t)) !== -1;
		...
		(() => {
			...
			class m {
				...
				constructor(S, A, y, R, N, H = !1) {
					Ge(this, "scheme", void 0), Ge(this, "authority", void 0), Ge(this, "path", void 0), Ge(this, "query", void 0), Ge(this, "fragment", void 0), typeof S == "object" ? (this.scheme = S.scheme || c, this.authority = S.authority || c, this.path = S.path || c, this.query = S.query || c, this.fragment = S.fragment || c) : (this.scheme = (function(q, P) {
						return q || P ? q : "file";
			...
			class b extends m {
				constructor(...S) {
					super(...S), Ge(this, "_formatted", null), Ge(this, "_fsPath", null);
				}
				get fsPath() {
			...
			function E(L, S) {
				...
				if (R && (y += R, y += ":"), (N || R === "file") && (y += u, y += u), N) {
					let W = N.indexOf("@");
					if (W !== -1) {
						const X = N.substr(0, W);
						N = N.substr(W + 1), W = X.lastIndexOf(":"), W === -1 ? y += A(X, !1, !1) : (y += A(X.substr(0, W), !1, !1), y += ":", y += A(X.substr(W + 1), !1, !0)), y += "@";
					}
	...
	function td(e, t, n, r) {
		...
		return /^\#/i.test(t) ? e + t : /^\/\//i.test(t) ? (Pe(e, "https://") ? "https" : "http") + ":" + t.replace(/^\s*/g, "") : n ? n.resolveReference(t, r || e) : t;
	}
	function nd(e, t, n, r, i, s) {
		const a = Ii(n);
		if (!ed(a, e.languageId)) return;
		a.length < n.length && (r++, i--);
		const o = td(e.uri, a, t, s);
		if (!o) return;
		const l = rd(o, e);
		return {
	...
	function rd(e, t) {
		try {
			let n = Ku.parse(e);
			return n.scheme === "file" && n.query && (n = n.with({ query: null }), e = n.toString(!0)), n.scheme === "file" && n.fragment && !(e.startsWith(t.uri) && e.charCodeAt(t.uri.length) === id) ? n.with({ fragment: null }).toString(!0) : e;
		} catch {
			return;
		}
	}
				...
				switch (i) {
					...
					case U.AttributeValue:
						if (a && s && this.dataManager.isPathAttribute(a, s)) {
							...
							if (!o) {
								const d = nd(e, t, u, r.getTokenOffset(), r.getTokenEnd(), l);
								d && n.push(d);
							}
							o && typeof l > "u" && (l = Ii(u), l && t && (l = t.resolveReference(l, e.uri))), o = !1, s = void 0;
						} else if (s === "id") {
							const u = Ii(r.getTokenText());
	...
	function ad(e, t, n) {
		...
		if (!i.tag) return [];
		const s = [], a = Do(U.StartTag, e, i.start), o = typeof i.endTagStart == "number" && Do(U.EndTag, e, i.endTagStart);
		return (a && Uo(a, t) || o && Uo(o, t)) && (a && s.push({
			kind: dn.Read,
			range: a
		}), o && s.push({
			kind: dn.Read,
	...
	function Uo(e, t) {
		return Ho(e.start, t) && Ho(t, e.end);
	}
	function Do(e, t, n) {
	...
	function od(e, t) {
		...
		return n;
		function i(s, a) {
			const o = Si.create(s.name, s.kind, s.range, e.uri, a?.name);
	...
	function Po(e, t, n) {
		const r = ld(t), i = $.create(e.positionAt(t.start), e.positionAt(t.end)), s = Li.create(r, void 0, ki.Field, i, i);
	...
	function cd(e, t, n, r) {
		...
		if (!s.tag || !hd(s, i, s.tag)) return null;
		const a = [], o = {
	...
	var dd = class {
		...
		getSelectionRange(e, t, n) {
			...
			for (let a = r.length - 1; a >= 0; a--) {
				const o = r[a];
				(!i || o[0] !== i[0] || o[1] !== i[1]) && (s = fn.create($.create(t.positionAt(r[a][0]), t.positionAt(r[a][1])), s)), i = o;
			}
			return s || (s = fn.create($.create(e, e))), s;
		}
...
*   a _positive value_ means that the element should be focusable and reachable via sequential keyboard navigation; the order in which the elements are focused is the increasing value of the [**tabindex**](#attr-tabindex). If several elements share the same tabindex, their relative order follows their relative positions in the document.`
	...
	var pd = class {
		...
		setDataProviders(e, t) {
			this.dataProviders = [], e && this.dataProviders.push(new Ro("html5", fd)), this.dataProviders.push(...t);
		}
		getDataProviders() {
			return this.dataProviders;
		}
		isVoidElement(e, t) {
			return !!e && yu(t, e.toLowerCase(), (n, r) => n.localeCompare(r)) >= 0;
		}
		getVoidElements(e) {
	...
	function wd(e = bd) {
		const t = new pd(e), n = new Fu(e, t), r = new Wu(e, t), i = new Tu(t), s = new md(i), a = new dd(t), o = new sd(t);
		return {
			...
			format: Gu,
			findDocumentHighlights: ad,
			findDocumentLinks: o.findDocumentLinks.bind(o),
			findDocumentSymbols: od,
			findDocumentSymbols2: Wo,
			...
			doRename: cd,
			findMatchingTagPosition: ud,
			findOnTypeRenameRanges: qo,
			findLinkedEditingRanges: qo
		};
	}
	function _d(e, t) {
		return new Ro(e, t);
	...
	var vd = class {
		constructor(e, t) {
			...
			if (n?.dataProviders) for (const s in n.dataProviders) i.push(_d(s, n.dataProviders[s]));
			this._languageService = wd({
		...
		_getTextDocument(e) {
			...
			for (let n of t) if (n.uri.toString() === e) return Ri.create(e, this._languageId, n.version, n.getValue());
			return null;
		}
	};
	self.onmessage = () => {
