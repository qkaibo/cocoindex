	...
	var Is = class {
		constructor() {
			this.listeners = [], this.unexpectedErrorHandler = function(e) {
				setTimeout(() => {
					throw e.stack ? Ht.isErrorNoTelemetry(e) ? new Ht(e.message + `
...
` + e.stack) : e;
	...
	const Bs = "Canceled";
	function qs(e) {
		return e instanceof H1 ? !0 : e instanceof Error && e.name === "Canceled" && e.message === "Canceled";
	}
	var H1 = class extends Error {
		constructor() {
			super(Bs), this.name = this.message;
		}
	}, Ht = class E1 extends Error {
		constructor(t) {
			super(t), this.name = "CodeExpectedError";
		}
		static fromError(t) {
			if (t instanceof E1) return t;
			const n = new E1();
	...
	function Hs(e, t = "unexpected state") {
		if (!e) throw typeof t == "string" ? new Z(`Assertion Failed: ${t}`) : t;
	}
	function ct(e) {
		if (!e()) {
			debugger;
			e(), ut(new Z("Assertion Failed"));
		}
	}
	function W1(e, t) {
		let n = 0;
	...
	(function(e) {
		...
		e.flatMap = p;
		function* y(...w) {
			for (const C of w) $s(C) ? yield* C : yield C;
		}
		e.concat = y;
		function v(w, C, T) {
			let U = T;
	...
	$1 = ft, $1.DISABLE_DISPOSED_WARNING = !1;
	var Ee = class {
		constructor() {
	...
	z1 = Ee, z1.None = Object.freeze({ dispose() {} });
	var Wt, O = class P1 {
		constructor(t) {
			this.element = t, this.next = P1.Undefined, this.prev = P1.Undefined;
		}
	};
	Wt = O, Wt.Undefined = new Wt(void 0);
	var Gs = class {
		constructor() {
	...
	var G1 = class Rs {
		...
		constructor(t) {
			this._now = t === !1 ? Date.now : js, this._startTime = this._now(), this._stopTime = -1;
		}
		stop() {
	...
	(function(e) {
		...
		e.signal = o;
		function u(...R) {
			return (S, E = null, A) => h(zs(...R.map((F) => F((B) => S.call(E, B)))), A);
		}
		e.any = u;
		function c(R, S, E, A) {
			let F = E;
		...
		function d(R, S, E = 100, A = !1, F = !1, B, H) {
			...
			const Ft = new ce({
				...
				onWillAddFirstListener() {
					Q = R(($0) => {
						Tt++, J = S(J, $0), A && !xe && (Ft.fire(J), J = void 0), st = () => {
							const z0 = J;
							J = void 0, xe = void 0, (!A || Tt > 1) && Ft.fire(z0), Tt = 0;
						}, typeof E == "number" ? (xe && clearTimeout(xe), xe = setTimeout(st, E)) : xe === void 0 && (xe = null, queueMicrotask(st));
					});
				},
		...
		function y(R, S = !1, E = [], A) {
			...
			A && A.add(B);
			const H = () => {
				F?.forEach((J) => Q.fire(J)), F = null;
			}, Q = new ce({
				onWillAddFirstListener() {
					B || (B = R((J) => Q.fire(J)), A && A.add(B));
				},
		...
		function N(R, S, E = (A) => A) {
			const A = (...Q) => H.fire(E(...Q)), F = () => R.on(S, A), B = () => R.removeListener(S, A), H = new ce({
		...
		function k(R, S, E = (A) => A) {
			const A = (...Q) => H.fire(E(...Q)), F = () => R.addEventListener(S, A), B = () => R.removeEventListener(S, A), H = new ce({
				onWillAddFirstListener: F,
				onDidRemoveLastListener: B
			});
			return H.event;
		...
		class T {
			constructor(S, E) {
				...
				this.emitter = new ce(A), E && E.add(this.emitter);
			}
			beginUpdate(S) {
				this._counter++;
			}
		...
		function x(R) {
			return (S, E, A) => {
				...
				return A instanceof ft ? A.add(Q) : Array.isArray(A) && A.push(Q), Q;
			};
		}
	...
	var X1 = class D1 {
		constructor(t) {
			this.listenerCount = 0, this.invocationCount = 0, this.elapsedOverall = 0, this.durations = [], this.name = `${t}_${D1._idPool++}`, D1.all.add(this);
		}
		start(t) {
			this._stopWatch = new G1(), this.listenerCount = t;
		}
		stop() {
			if (this._stopWatch) {
	...
	$t = X1, $t.all = /* @__PURE__ */ new Set(), $t._idPool = 0;
	let Xs = -1;
	var Q1 = class As {
		constructor(t, n, r = (As._idPool++).toString(16).padStart(3, "0")) {
...
`));
	...
	var ce = class {
		constructor(e) {
			this._size = 0, this._options = e, this._leakageMon = this._options?.leakWarningThreshold ? new Q1(e?.onListenerError ?? ut, this._options?.leakWarningThreshold ?? Xs) : void 0, this._perfMon = this._options?._profName ? new X1(this._options._profName) : void 0, this._deliveryQueue = this._options?.deliveryQueue;
		}
		dispose() {
		...
		get event() {
			return this._event ?? (this._event = (e, t, n) => {
				if (this._leakageMon && this._size > this._leakageMon.threshold ** 2) {
					...
					console.warn(l);
					const o = this._leakageMon.getMostFrequentStack() ?? ["UNKNOWN stack", -1], u = new Js(`${l}. HINT: Stack shows most frequent listener (${o[1]}-times)`, o[0]);
					return (this._options?.onListenerError || ut)(u), Ee.None;
				}
				if (this._disposed) return Ee.None;
				t && (e = e.bind(t));
				const r = new Ot(e);
				let s;
				this._leakageMon && this._size >= Math.ceil(this._leakageMon.threshold * .2) && (r.stack = Qs.create(), s = this._leakageMon.check(r.stack, this._size + 1)), this._listeners ? this._listeners instanceof Ot ? (this._deliveryQueue ?? (this._deliveryQueue = new Ks()), this._listeners = [this._listeners, r]) : this._listeners.push(r) : (this._options?.onWillAddFirstListener?.(this), this._listeners = r, this._options?.onDidAddFirstListener?.(this)), this._options?.onDidAddListener?.(this), this._size++;
				const a = mt(() => {
					s?.(), this._removeListener(r);
				});
				return n instanceof ft ? n.add(a) : Array.isArray(n) && n.push(a), a;
			}), this._event;
		}
		_removeListener(e) {
		...
		_deliver(e, t) {
			if (!e) return;
			const n = this._options?.onListenerError || ut;
			if (!n) {
				e.value(t);
				return;
			}
		...
		fire(e) {
			if (this._deliveryQueue?.current && (this._deliverQueue(this._deliveryQueue), this._perfMon?.stop()), this._perfMon?.start(this._size), this._listeners) if (this._listeners instanceof Ot) this._deliver(this._listeners, e);
			else {
	...
	const ti = Y1() === "pseudo" || typeof document < "u" && document.location && typeof document.location.hash == "string" && document.location.hash.indexOf("pseudo=true") >= 0;
	function J1(e, t) {
		let n;
		...
		}), ti && (n = "［" + n.replace(/[aouei]/g, "$&$&") + "］"), n;
	}
	function D(e, t, ...n) {
		return J1(typeof e == "number" ? ni(e, t) : t, n);
	}
	function ni(e, t) {
		const n = ei()?.[e];
		if (typeof n != "string") {
	...
	let gt = !1, dt = !1, pt = !1, Z1 = !1, me;
	const fe = globalThis;
	let ee;
	typeof fe.vscode < "u" && typeof fe.vscode.process < "u" ? ee = fe.vscode.process : typeof process < "u" && typeof process?.versions?.node == "string" && (ee = process);
	const ai = typeof ee?.versions?.electron == "string" && ee?.type === "renderer";
	if (typeof ee == "object") {
		gt = ee.platform === "win32", dt = ee.platform === "darwin", pt = ee.platform === "linux", pt && ee.env.SNAP && ee.env.SNAP_REVISION, ee.env.CI || ee.env.BUILD_ARTIFACTSTAGINGDIRECTORY || ee.env.GITHUB_WORKSPACE;
		const e = ee.env.VSCODE_NLS_CONFIG;
		if (e) try {
			const t = JSON.parse(e);
			t.userLocale, t.osLocale, t.resolvedLanguage, t.languagePack?.translationsConfigFile;
		} catch {}
	} else typeof navigator == "object" && !ai ? (me = navigator.userAgent, gt = me.indexOf("Windows") >= 0, dt = me.indexOf("Macintosh") >= 0, (me.indexOf("Macintosh") >= 0 || me.indexOf("iPad") >= 0 || me.indexOf("iPhone") >= 0) && navigator.maxTouchPoints && navigator.maxTouchPoints, pt = me.indexOf("Linux") >= 0, me?.indexOf("Mobi"), Z1 = !0, Y1(), navigator.language.toLowerCase()) : console.error("Unable to resolve platform.");
	const Ge = gt, li = dt;
	Z1 && typeof fe.importScripts == "function" && fe.origin;
	const he = me, oi = typeof fe.postMessage == "function" && !fe.importScripts;
	(() => {
		if (oi) {
			const e = [];
	...
	const ci = !!(he && he.indexOf("Chrome") >= 0);
	he && he.indexOf("Firefox");
	!ci && he && he.indexOf("Safari");
	he && he.indexOf("Edg/");
	he && he.indexOf("Android");
	function hi(e) {
		return e;
	}
	var mi = class {
		constructor(e, t) {
			this.lastCache = void 0, this.lastArgKey = void 0, typeof e == "function" ? (this._fn = e, this._computeKey = hi) : (this._fn = t, this._computeKey = e.getCacheKey);
		}
		get(e) {
	...
	var Xt = class {
		constructor(e) {
			this.executor = e, this._state = Ne.Uninitialized;
		}
		get value() {
			if (this._state === Ne.Uninitialized) {
				this._state = Ne.Running;
				try {
					this._value = this.executor();
				} catch (e) {
					this._error = e;
				} finally {
					this._state = Ne.Completed;
				}
			} else if (this._state === Ne.Running) throw new Error("Cannot read the value of a lazy that is being initialized");
	...
	K1 = class it {
		static getInstance() {
			return it._INSTANCE || (it._INSTANCE = new it()), it._INSTANCE;
		}
		constructor() {
			this._data = Si();
		}
	...
	var wt = class T1 {
		static getInstance(t) {
			return T1.cache.get(Array.from(t));
		}
		static getLocales() {
			return T1._locales.value;
		}
		constructor(t) {
	...
	Se = wt, Se.ambiguousCharacterData = new Xt(() => JSON.parse("{\"_common\":[8232,32,8233,32,5760,32,8192,32,8193,32,8194,32,8195,32,8196,32,8197,32,8198,32,8200,32,8201,32,8202,32,8287,32,8199,32,8239,32,2042,95,65101,95,65102,95,65103,95,8208,45,8209,45,8210,45,65112,45,1748,45,8259,45,727,45,8722,45,10134,45,11450,45,1549,44,1643,44,184,44,42233,44,894,59,2307,58,2691,58,1417,58,1795,58,1796,58,5868,58,65072,58,6147,58,6153,58,8282,58,1475,58,760,58,42889,58,8758,58,720,58,42237,58,451,33,11601,33,660,63,577,63,2429,63,5038,63,42731,63,119149,46,8228,46,1793,46,1794,46,42510,46,68176,46,1632,46,1776,46,42232,46,1373,96,65287,96,8219,96,1523,96,8242,96,1370,96,8175,96,65344,96,900,96,8189,96,8125,96,8127,96,8190,96,697,96,884,96,712,96,714,96,715,96,756,96,699,96,701,96,700,96,702,96,42892,96,1497,96,2036,96,2037,96,5194,96,5836,96,94033,96,94034,96,65339,91,10088,40,10098,40,12308,40,64830,40,65341,93,10089,41,10099,41,12309,41,64831,41,10100,123,119060,123,10101,125,65342,94,8270,42,1645,42,8727,42,66335,42,5941,47,8257,47,8725,47,8260,47,9585,47,10187,47,10744,47,119354,47,12755,47,12339,47,11462,47,20031,47,12035,47,65340,92,65128,92,8726,92,10189,92,10741,92,10745,92,119311,92,119355,92,12756,92,20022,92,12034,92,42872,38,708,94,710,94,5869,43,10133,43,66203,43,8249,60,10094,60,706,60,119350,60,5176,60,5810,60,5120,61,11840,61,12448,61,42239,61,8250,62,10095,62,707,62,119351,62,5171,62,94015,62,8275,126,732,126,8128,126,8764,126,65372,124,65293,45,118002,50,120784,50,120794,50,120804,50,120814,50,120824,50,130034,50,42842,50,423,50,1000,50,42564,50,5311,50,42735,50,119302,51,118003,51,120785,51,120795,51,120805,51,120815,51,120825,51,130035,51,42923,51,540,51,439,51,42858,51,11468,51,1248,51,94011,51,71882,51,118004,52,120786,52,120796,52,120806,52,120816,52,120826,52,130036,52,5070,52,71855,52,118005,53,120787,53,120797,53,120807,53,120817,53,120827,53,130037,53,444,53,71867,53,118006,54,120788,54,120798,54,120808,54,120818,54,120828,54,130038,54,11474,54,5102,54,71893,54,119314,55,118007,55,120789,55,120799,55,120809,55,120819,55,120829,55,130039,55,66770,55,71878,55,2819,56,2538,56,2666,56,125131,56,118008,56,120790,56,120800,56,120810,56,120820,56,120830,56,130040,56,547,56,546,56,66330,56,2663,57,2920,57,2541,57,3437,57,118009,57,120791,57,120801,57,120811,57,120821,57,120831,57,130041,57,42862,57,11466,57,71884,57,71852,57,71894,57,9082,97,65345,97,119834,97,119886,97,119938,97,119990,97,120042,97,120094,97,120146,97,120198,97,120250,97,120302,97,120354,97,120406,97,120458,97,593,97,945,97,120514,97,120572,97,120630,97,120688,97,120746,97,65313,65,117974,65,119808,65,119860,65,119912,65,119964,65,120016,65,120068,65,120120,65,120172,65,120224,65,120276,65,120328,65,120380,65,120432,65,913,65,120488,65,120546,65,120604,65,120662,65,120720,65,5034,65,5573,65,42222,65,94016,65,66208,65,119835,98,119887,98,119939,98,119991,98,120043,98,120095,98,120147,98,120199,98,120251,98,120303,98,120355,98,120407,98,120459,98,388,98,5071,98,5234,98,5551,98,65314,66,8492,66,117975,66,119809,66,119861,66,119913,66,120017,66,120069,66,120121,66,120173,66,120225,66,120277,66,120329,66,120381,66,120433,66,42932,66,914,66,120489,66,120547,66,120605,66,120663,66,120721,66,5108,66,5623,66,42192,66,66178,66,66209,66,66305,66,65347,99,8573,99,119836,99,119888,99,119940,99,119992,99,120044,99,120096,99,120148,99,120200,99,120252,99,120304,99,120356,99,120408,99,120460,99,7428,99,1010,99,11429,99,43951,99,66621,99,128844,67,71913,67,71922,67,65315,67,8557,67,8450,67,8493,67,117976,67,119810,67,119862,67,119914,67,119966,67,120018,67,120174,67,120226,67,120278,67,120330,67,120382,67,120434,67,1017,67,11428,67,5087,67,42202,67,66210,67,66306,67,66581,67,66844,67,8574,100,8518,100,119837,100,119889,100,119941,100,119993,100,120045,100,120097,100,120149,100,120201,100,120253,100,120305,100,120357,100,120409,100,120461,100,1281,100,5095,100,5231,100,42194,100,8558,68,8517,68,117977,68,119811,68,119863,68,119915,68,119967,68,120019,68,120071,68,120123,68,120175,68,120227,68,120279,68,120331,68,120383,68,120435,68,5024,68,5598,68,5610,68,42195,68,8494,101,65349,101,8495,101,8519,101,119838,101,119890,101,119942,101,120046,101,120098,101,120150,101,120202,101,120254,101,120306,101,120358,101,120410,101,120462,101,43826,101,1213,101,8959,69,65317,69,8496,69,117978,69,119812,69,119864,69,119916,69,120020,69,120072,69,120124,69,120176,69,120228,69,120280,69,120332,69,120384,69,120436,69,917,69,120492,69,120550,69,120608,69,120666,69,120724,69,11577,69,5036,69,42224,69,71846,69,71854,69,66182,69,119839,102,119891,102,119943,102,119995,102,120047,102,120099,102,120151,102,120203,102,120255,102,120307,102,120359,102,120411,102,120463,102,43829,102,42905,102,383,102,7837,102,1412,102,119315,70,8497,70,117979,70,119813,70,119865,70,119917,70,120021,70,120073,70,120125,70,120177,70,120229,70,120281,70,120333,70,120385,70,120437,70,42904,70,988,70,120778,70,5556,70,42205,70,71874,70,71842,70,66183,70,66213,70,66853,70,65351,103,8458,103,119840,103,119892,103,119944,103,120048,103,120100,103,120152,103,120204,103,120256,103,120308,103,120360,103,120412,103,120464,103,609,103,7555,103,397,103,1409,103,117980,71,119814,71,119866,71,119918,71,119970,71,120022,71,120074,71,120126,71,120178,71,120230,71,120282,71,120334,71,120386,71,120438,71,1292,71,5056,71,5107,71,42198,71,65352,104,8462,104,119841,104,119945,104,119997,104,120049,104,120101,104,120153,104,120205,104,120257,104,120309,104,120361,104,120413,104,120465,104,1211,104,1392,104,5058,104,65320,72,8459,72,8460,72,8461,72,117981,72,119815,72,119867,72,119919,72,120023,72,120179,72,120231,72,120283,72,120335,72,120387,72,120439,72,919,72,120494,72,120552,72,120610,72,120668,72,120726,72,11406,72,5051,72,5500,72,42215,72,66255,72,731,105,9075,105,65353,105,8560,105,8505,105,8520,105,119842,105,119894,105,119946,105,119998,105,120050,105,120102,105,120154,105,120206,105,120258,105,120310,105,120362,105,120414,105,120466,105,120484,105,618,105,617,105,953,105,8126,105,890,105,120522,105,120580,105,120638,105,120696,105,120754,105,1110,105,42567,105,1231,105,43893,105,5029,105,71875,105,65354,106,8521,106,119843,106,119895,106,119947,106,119999,106,120051,106,120103,106,120155,106,120207,106,120259,106,120311,106,120363,106,120415,106,120467,106,1011,106,1112,106,65322,74,117983,74,119817,74,119869,74,119921,74,119973,74,120025,74,120077,74,120129,74,120181,74,120233,74,120285,74,120337,74,120389,74,120441,74,42930,74,895,74,1032,74,5035,74,5261,74,42201,74,119844,107,119896,107,119948,107,120000,107,120052,107,120104,107,120156,107,120208,107,120260,107,120312,107,120364,107,120416,107,120468,107,8490,75,65323,75,117984,75,119818,75,119870,75,119922,75,119974,75,120026,75,120078,75,120130,75,120182,75,120234,75,120286,75,120338,75,120390,75,120442,75,922,75,120497,75,120555,75,120613,75,120671,75,120729,75,11412,75,5094,75,5845,75,42199,75,66840,75,1472,108,8739,73,9213,73,65512,73,1633,108,1777,73,66336,108,125127,108,118001,108,120783,73,120793,73,120803,73,120813,73,120823,73,130033,73,65321,73,8544,73,8464,73,8465,73,117982,108,119816,73,119868,73,119920,73,120024,73,120128,73,120180,73,120232,73,120284,73,120336,73,120388,73,120440,73,65356,108,8572,73,8467,108,119845,108,119897,108,119949,108,120001,108,120053,108,120105,73,120157,73,120209,73,120261,73,120313,73,120365,73,120417,73,120469,73,448,73,120496,73,120554,73,120612,73,120670,73,120728,73,11410,73,1030,73,1216,73,1493,108,1503,108,1575,108,126464,108,126592,108,65166,108,65165,108,1994,108,11599,73,5825,73,42226,73,93992,73,66186,124,66313,124,119338,76,8556,76,8466,76,117985,76,119819,76,119871,76,119923,76,120027,76,120079,76,120131,76,120183,76,120235,76,120287,76,120339,76,120391,76,120443,76,11472,76,5086,76,5290,76,42209,76,93974,76,71843,76,71858,76,66587,76,66854,76,65325,77,8559,77,8499,77,117986,77,119820,77,119872,77,119924,77,120028,77,120080,77,120132,77,120184,77,120236,77,120288,77,120340,77,120392,77,120444,77,924,77,120499,77,120557,77,120615,77,120673,77,120731,77,1018,77,11416,77,5047,77,5616,77,5846,77,42207,77,66224,77,66321,77,119847,110,119899,110,119951,110,120003,110,120055,110,120107,110,120159,110,120211,110,120263,110,120315,110,120367,110,120419,110,120471,110,1400,110,1404,110,65326,78,8469,78,117987,78,119821,78,119873,78,119925,78,119977,78,120029,78,120081,78,120185,78,120237,78,120289,78,120341,78,120393,78,120445,78,925,78,120500,78,120558,78,120616,78,120674,78,120732,78,11418,78,42208,78,66835,78,3074,111,3202,111,3330,111,3458,111,2406,111,2662,111,2790,111,3046,111,3174,111,3302,111,3430,111,3664,111,3792,111,4160,111,1637,111,1781,111,65359,111,8500,111,119848,111,119900,111,119952,111,120056,111,120108,111,120160,111,120212,111,120264,111,120316,111,120368,111,120420,111,120472,111,7439,111,7441,111,43837,111,959,111,120528,111,120586,111,120644,111,120702,111,120760,111,963,111,120532,111,120590,111,120648,111,120706,111,120764,111,11423,111,4351,111,1413,111,1505,111,1607,111,126500,111,126564,111,126596,111,65259,111,65260,111,65258,111,65257,111,1726,111,64428,111,64429,111,64427,111,64426,111,1729,111,64424,111,64425,111,64423,111,64422,111,1749,111,3360,111,4125,111,66794,111,71880,111,71895,111,66604,111,1984,79,2534,79,2918,79,12295,79,70864,79,71904,79,118000,79,120782,79,120792,79,120802,79,120812,79,120822,79,130032,79,65327,79,117988,79,119822,79,119874,79,119926,79,119978,79,120030,79,120082,79,120134,79,120186,79,120238,79,120290,79,120342,79,120394,79,120446,79,927,79,120502,79,120560,79,120618,79,120676,79,120734,79,11422,79,1365,79,11604,79,4816,79,2848,79,66754,79,42227,79,71861,79,66194,79,66219,79,66564,79,66838,79,9076,112,65360,112,119849,112,119901,112,119953,112,120005,112,120057,112,120109,112,120161,112,120213,112,120265,112,120317,112,120369,112,120421,112,120473,112,961,112,120530,112,120544,112,120588,112,120602,112,120646,112,120660,112,120704,112,120718,112,120762,112,120776,112,11427,112,65328,80,8473,80,117989,80,119823,80,119875,80,119927,80,119979,80,120031,80,120083,80,120187,80,120239,80,120291,80,120343,80,120395,80,120447,80,929,80,120504,80,120562,80,120620,80,120678,80,120736,80,11426,80,5090,80,5229,80,42193,80,66197,80,119850,113,119902,113,119954,113,120006,113,120058,113,120110,113,120162,113,120214,113,120266,113,120318,113,120370,113,120422,113,120474,113,1307,113,1379,113,1382,113,8474,81,117990,81,119824,81,119876,81,119928,81,119980,81,120032,81,120084,81,120188,81,120240,81,120292,81,120344,81,120396,81,120448,81,11605,81,119851,114,119903,114,119955,114,120007,114,120059,114,120111,114,120163,114,120215,114,120267,114,120319,114,120371,114,120423,114,120475,114,43847,114,43848,114,7462,114,11397,114,43905,114,119318,82,8475,82,8476,82,8477,82,117991,82,119825,82,119877,82,119929,82,120033,82,120189,82,120241,82,120293,82,120345,82,120397,82,120449,82,422,82,5025,82,5074,82,66740,82,5511,82,42211,82,94005,82,65363,115,119852,115,119904,115,119956,115,120008,115,120060,115,120112,115,120164,115,120216,115,120268,115,120320,115,120372,115,120424,115,120476,115,42801,115,445,115,1109,115,43946,115,71873,115,66632,115,65331,83,117992,83,119826,83,119878,83,119930,83,119982,83,120034,83,120086,83,120138,83,120190,83,120242,83,120294,83,120346,83,120398,83,120450,83,1029,83,1359,83,5077,83,5082,83,42210,83,94010,83,66198,83,66592,83,119853,116,119905,116,119957,116,120009,116,120061,116,120113,116,120165,116,120217,116,120269,116,120321,116,120373,116,120425,116,120477,116,8868,84,10201,84,128872,84,65332,84,117993,84,119827,84,119879,84,119931,84,119983,84,120035,84,120087,84,120139,84,120191,84,120243,84,120295,84,120347,84,120399,84,120451,84,932,84,120507,84,120565,84,120623,84,120681,84,120739,84,11430,84,5026,84,42196,84,93962,84,71868,84,66199,84,66225,84,66325,84,119854,117,119906,117,119958,117,120010,117,120062,117,120114,117,120166,117,120218,117,120270,117,120322,117,120374,117,120426,117,120478,117,42911,117,7452,117,43854,117,43858,117,651,117,965,117,120534,117,120592,117,120650,117,120708,117,120766,117,1405,117,66806,117,71896,117,8746,85,8899,85,117994,85,119828,85,119880,85,119932,85,119984,85,120036,85,120088,85,120140,85,120192,85,120244,85,120296,85,120348,85,120400,85,120452,85,1357,85,4608,85,66766,85,5196,85,42228,85,94018,85,71864,85,8744,118,8897,118,65366,118,8564,118,119855,118,119907,118,119959,118,120011,118,120063,118,120115,118,120167,118,120219,118,120271,118,120323,118,120375,118,120427,118,120479,118,7456,118,957,118,120526,118,120584,118,120642,118,120700,118,120758,118,1141,118,1496,118,71430,118,43945,118,71872,118,119309,86,1639,86,1783,86,8548,86,117995,86,119829,86,119881,86,119933,86,119985,86,120037,86,120089,86,120141,86,120193,86,120245,86,120297,86,120349,86,120401,86,120453,86,1140,86,11576,86,5081,86,5167,86,42719,86,42214,86,93960,86,71840,86,66845,86,623,119,119856,119,119908,119,119960,119,120012,119,120064,119,120116,119,120168,119,120220,119,120272,119,120324,119,120376,119,120428,119,120480,119,7457,119,1121,119,1309,119,1377,119,71434,119,71438,119,71439,119,43907,119,71910,87,71919,87,117996,87,119830,87,119882,87,119934,87,119986,87,120038,87,120090,87,120142,87,120194,87,120246,87,120298,87,120350,87,120402,87,120454,87,1308,87,5043,87,5076,87,42218,87,5742,120,10539,120,10540,120,10799,120,65368,120,8569,120,119857,120,119909,120,119961,120,120013,120,120065,120,120117,120,120169,120,120221,120,120273,120,120325,120,120377,120,120429,120,120481,120,5441,120,5501,120,5741,88,9587,88,66338,88,71916,88,65336,88,8553,88,117997,88,119831,88,119883,88,119935,88,119987,88,120039,88,120091,88,120143,88,120195,88,120247,88,120299,88,120351,88,120403,88,120455,88,42931,88,935,88,120510,88,120568,88,120626,88,120684,88,120742,88,11436,88,11613,88,5815,88,42219,88,66192,88,66228,88,66327,88,66855,88,611,121,7564,121,65369,121,119858,121,119910,121,119962,121,120014,121,120066,121,120118,121,120170,121,120222,121,120274,121,120326,121,120378,121,120430,121,120482,121,655,121,7935,121,43866,121,947,121,8509,121,120516,121,120574,121,120632,121,120690,121,120748,121,1199,121,4327,121,71900,121,65337,89,117998,89,119832,89,119884,89,119936,89,119988,89,120040,89,120092,89,120144,89,120196,89,120248,89,120300,89,120352,89,120404,89,120456,89,933,89,978,89,120508,89,120566,89,120624,89,120682,89,120740,89,11432,89,1198,89,5033,89,5053,89,42220,89,94019,89,71844,89,66226,89,119859,122,119911,122,119963,122,120015,122,120067,122,120119,122,120171,122,120223,122,120275,122,120327,122,120379,122,120431,122,120483,122,7458,122,43923,122,71876,122,71909,90,66293,90,65338,90,8484,90,8488,90,117999,90,119833,90,119885,90,119937,90,119989,90,120041,90,120197,90,120249,90,120301,90,120353,90,120405,90,120457,90,918,90,120493,90,120551,90,120609,90,120667,90,120725,90,5059,90,42204,90,71849,90,65282,34,65283,35,65284,36,65285,37,65286,38,65290,42,65291,43,65294,46,65295,47,65296,48,65298,50,65299,51,65300,52,65301,53,65302,54,65303,55,65304,56,65305,57,65308,60,65309,61,65310,62,65312,64,65316,68,65318,70,65319,71,65324,76,65329,81,65330,82,65333,85,65334,86,65335,87,65343,95,65346,98,65348,100,65350,102,65355,107,65357,109,65358,110,65361,113,65362,114,65364,116,65365,117,65367,119,65370,122,65371,123,65373,125,119846,109],\"_default\":[160,32,8211,45,65374,126,8218,44,65306,58,65281,33,8216,96,8217,96,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"cs\":[65374,126,8218,44,65306,58,65281,33,8216,96,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"de\":[65374,126,65306,58,65281,33,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"es\":[8211,45,65374,126,8218,44,65306,58,65281,33,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"fr\":[65374,126,8218,44,65306,58,65281,33,8216,96,8245,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"it\":[160,32,8211,45,65374,126,8218,44,65306,58,65281,33,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"ja\":[8211,45,8218,44,65281,33,8216,96,8245,96,180,96,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65292,44,65297,49,65307,59],\"ko\":[8211,45,65374,126,8218,44,65306,58,65281,33,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"pl\":[65374,126,65306,58,65281,33,8216,96,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"pt-BR\":[65374,126,8218,44,65306,58,65281,33,8216,96,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"qps-ploc\":[160,32,8211,45,65374,126,8218,44,65306,58,65281,33,8216,96,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"ru\":[65374,126,8218,44,65306,58,65281,33,8216,96,8245,96,180,96,12494,47,305,105,921,73,1009,112,215,120,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"tr\":[160,32,8211,45,65374,126,8218,44,65306,58,65281,33,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65288,40,65289,41,65292,44,65297,49,65307,59,65311,63],\"zh-hans\":[160,32,65374,126,8218,44,8245,96,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89,65297,49],\"zh-hant\":[8211,45,65374,126,8218,44,180,96,12494,47,1047,51,1073,54,1072,97,1040,65,1068,98,1042,66,1089,99,1057,67,1077,101,1045,69,1053,72,305,105,1050,75,921,73,1052,77,1086,111,1054,79,1009,112,1088,112,1056,80,1075,114,1058,84,215,120,1093,120,1061,88,1091,121,1059,89]}")), Se.cache = new mi({ getCacheKey: JSON.stringify }, (e) => {
		...
		const s = Se.ambiguousCharacterData.value;
		let a = e.filter((u) => !u.startsWith("_") && Object.hasOwn(s, u));
		...
		const o = n(t(s._common), l);
		return new Se(o);
	}), Se._locales = new Xt(() => Object.keys(Se.ambiguousCharacterData.value).filter((e) => !e.startsWith("_")));
	var yt = class It {
		static getRawData() {
		...
		static getData() {
			return this._data || (this._data = new Set([...Object.values(It.getRawData())].flat())), this._data;
		}
	...
	en = yt, en._data = void 0;
	const Yt = "default", Ri = "$initialize";
	var Ai = class {
		...
		listen(e, t, n) {
			...
			const s = new ce({
				onWillAddFirstListener: () => {
					r = String(++this._lastSentReq), this._pendingEmitters.set(r, s), this._send(new Ci(this._workerId, r, e, t, n));
				},
				onDidRemoveLastListener: () => {
		...
		createProxyToRemoteChannel(e, t) {
			return new Proxy(Object.create(null), { get: (n, r) => (typeof r == "string" && !n[r] && (an(r) ? n[r] = (s) => this.listen(e, r, s) : sn(r) ? n[r] = this.listen(e, r, void 0) : r.charCodeAt(0) === 36 && (n[r] = async (...s) => (await t?.(), this.sendMessage(e, r, s)))), n[r]) });
		}
		_handleMessage(e) {
			switch (e.type) {
		...
		_handleRequestMessage(e) {
			...
			this._handler.handleMessage(e.channel, e.method, e.args).then((n) => {
				this._send(new rn(this._workerId, t, n, void 0));
			}, (n) => {
				n.detail instanceof Error && (n.detail = Ut(n.detail)), this._send(new rn(this._workerId, t, void 0, Ut(n)));
			});
		}
		_handleSubscribeEventMessage(e) {
			const t = e.req, n = this._handler.handleEvent(e.channel, e.eventName, e.arg)((r) => {
				this._send(new xi(this._workerId, t, r));
			});
	...
	function sn(e) {
		return e[0] === "o" && e[1] === "n" && tn(e.charCodeAt(2));
	}
	function an(e) {
		return /^onDynamic/.test(e) && tn(e.charCodeAt(9));
	}
	var Ei = class {
		constructor(e, t) {
		...
		_handleMessage(e, t, n) {
			if (e === Yt && t === Ri) return this.initialize(n[0]);
			const r = e === Yt ? this.requestHandler : this._localChannels.get(e);
	...
	let ln = !1;
	function Pi(e) {
		if (ln) throw new Error("WebWorker already initialized!");
		ln = !0;
	...
	const on = typeof Buffer < "u";
	new Xt(() => new Uint8Array(256));
	let Jt;
	var Di = class xs {
		static wrap(t) {
			return on && !Buffer.isBuffer(t) && (t = Buffer.from(t.buffer, t.byteOffset, t.byteLength)), new xs(t);
		}
		constructor(t) {
			this.buffer = t, this.byteLength = this.buffer.byteLength;
		}
		toString() {
			return on ? this.buffer.toString() : (Jt || (Jt = new TextDecoder()), Jt.decode(this.buffer));
		}
	};
	const un = "0123456789abcdef";
	function Ti({ buffer: e }) {
		let t = "";
	...
	function Fi(e, t) {
		t = hn(149417, t);
		for (let n = 0, r = e.length; n < r; n++) t = hn(e.charCodeAt(n), t);
		return t;
	}
	...
	function je(e, t = 32) {
		return e instanceof ArrayBuffer ? Ti(Di.wrap(new Uint8Array(e))) : (e >>> 0).toString(16).padStart(t / 4, "0");
	}
	cn = class ks {
		constructor() {
		...
		digest() {
			return this._finished || (this._finished = !0, this._leftoverHighSurrogate && (this._leftoverHighSurrogate = 0, this._buffLen = this._push(this._buff, this._buffLen, 65533)), this._totalLen += this._buffLen, this._wrapUp()), je(this._h0) + je(this._h1) + je(this._h2) + je(this._h3) + je(this._h4);
		}
		_wrapUp() {
		...
		_step() {
			...
			for (let h = 64; h < 320; h += 4) t.setUint32(h, Zt(t.getUint32(h - 12, !1) ^ t.getUint32(h - 32, !1) ^ t.getUint32(h - 56, !1) ^ t.getUint32(h - 64, !1), 1), !1);
			let r = this._h0, s = this._h1, a = this._h2, l = this._h3, o = this._h4, u, c, m;
			for (let h = 0; h < 80; h++) h < 20 ? (u = s & a | ~s & l, c = 1518500249) : h < 40 ? (u = s ^ a ^ l, c = 1859775393) : h < 60 ? (u = s & a | s & l | a & l, c = 2400959708) : (u = s ^ a ^ l, c = 3395469782), m = Zt(r, 5) + u + o + c + t.getUint32(h * 4, !1) & 4294967295, o = l, l = a, a = Zt(s, 30), s = r, r = m;
	...
	}, cn._bigBlock32 = /* @__PURE__ */ new DataView(/* @__PURE__ */ new ArrayBuffer(320));
	var mn = class {
		constructor(e) {
			this.source = e;
		}
	...
	function Vi(e, t, n) {
		return new gn(new mn(e), new mn(t)).ComputeDiff(n).changes;
	}
	var Pe = class {
		static Assert(e, t) {
		...
		constructor(t, n, r = null) {
			...
			const [s, a, l] = $e._getElements(t), [o, u, c] = $e._getElements(n);
		...
		static _getElements(t) {
			...
			if ($e._isStringArray(n)) {
				...
				for (let s = 0, a = n.length; s < a; s++) r[s] = Fi(n[s], 0);
				return [
					n,
					r,
					!0
		...
		ComputeDiffRecursive(t, n, r, s, a) {
			...
			if (t > n || r > s) {
				let h;
				return r <= s ? (Pe.Assert(t === n + 1, "originalStart should only be one more than originalEnd"), h = [new we(t, 0, r, s - r + 1)]) : t <= n ? (Pe.Assert(r === s + 1, "modifiedStart should only be one more than modifiedEnd"), h = [new we(t, n - t + 1, r, 0)]) : (Pe.Assert(t === n + 1, "originalStart should only be one more than originalEnd"), Pe.Assert(r === s + 1, "modifiedStart should only be one more than modifiedEnd"), h = []), h;
			}
		...
		WALKTRACE(t, n, r, s, a, l, o, u, c, m, h, d, f, b, p, y, v, L) {
			let _ = null, N = null, k = new fn(), M = n, w = r, C = f[0] - y[0] - s, T = -1073741824, U = this.m_forwardHistory.length - 1;
			do {
				const x = C + t;
			...
			if (_ = k.getReverseChanges(), L[0]) {
				...
				k = new fn(), M = l, w = o, C = f[0] - y[0] - u, T = 1073741824, U = v ? this.m_reverseHistory.length - 1 : this.m_reverseHistory.length - 2;
				do {
					const x = C + a;
		...
		ComputeRecursionPoint(t, n, r, s, a, l, o) {
			...
			for (let w = 1; w <= b / 2 + 1; w++) {
				...
				if (w <= 1447) {
					...
					x[0] = L - m + 1, De.Copy2(y, m, x, 1, h - m + 1), this.m_forwardHistory.push(x), x = new Int32Array(f - d + 2), x[0] = _ - d + 1, De.Copy2(v, d, x, 1, f - d + 1), this.m_reverseHistory.push(x);
				}
			}
		...
		ConcatenateChanges(t, n) {
			...
			if (this.ChangesOverlap(t[t.length - 1], n[0], r)) {
				...
				return De.Copy(t, 0, s, 0, t.length - 1), s[t.length - 1] = r[0], De.Copy(n, 1, s, t.length, n.length - 1), s;
			} else {
				const s = new Array(t.length + n.length);
				return De.Copy(t, 0, s, 0, t.length), De.Copy(n, 0, s, t.length, n.length), s;
			}
		}
	...
	function Wi() {
		if (Xe === null) {
			Xe = new Bi(0);
			const e = ` 	<>'"、。｡､，．：；‘〈「『〔（［｛｢｣｝］）〕』」〉’｀～…|`;
			for (let n = 0; n < 36; n++) Xe.set(e.charCodeAt(n), 1);
			const t = ".,;:";
			for (let n = 0; n < 4; n++) Xe.set(t.charCodeAt(n), 2);
		}
		return Xe;
	}
	var $i = class F1 {
			...
			for (let a = 1, l = t.getLineCount(); a <= l; a++) {
				...
				for (; c < u;) {
					...
					if (d === 13) {
						...
						_ === 1 && (s.push(F1._createLink(r, o, a, m, c)), v = !0);
					} else if (d === 12) {
						let _;
				...
				d === 13 && s.push(F1._createLink(r, o, a, m, u));
			}
			return s;
		}
	};
	function zi(e) {
		return !e || typeof e.getLineCount != "function" || typeof e.getLineContent != "function" ? [] : $i.computeLinks(e);
	}
	var e1, pn = class {
		constructor() {
		...
		navigateValueSet(e, t, n, r, s) {
			if (e && t) {
				const a = this.doNavigateValueSet(t, s);
				if (a) return {
					range: e,
					value: a
				};
		...
		numberReplace(e, t) {
			...
			return !isNaN(r) && !isNaN(s) && r === s ? r === 0 && !t ? null : (r = Math.floor(r * n), r += t ? n : -n, String(r / n)) : null;
		}
		textReplace(e, t) {
		...
		valueSetsReplace(e, t, n) {
			let r = null;
			for (let s = 0, a = e.length; r === null && s < a; s++) r = this.valueSetReplace(e[s], t, n);
			return r;
		}
	...
	(function(e) {
		function t(n) {
			return n === e.None || n === e.Cancelled || n instanceof vt ? !0 : !n || typeof n != "object" ? !1 : typeof n.isCancellationRequested == "boolean" && typeof n.onCancellationRequested == "function";
		}
	...
	var vt = class {
		...
		get onCancellationRequested() {
			return this._isCancelled ? bn : (this._emitter || (this._emitter = new ce()), this._emitter.event);
		}
		dispose() {
		...
		cancel() {
			this._token ? this._token instanceof vt && this._token.cancel() : this._token = _t.Cancelled;
		}
		dispose(e = !1) {
			e && this.cancel(), this._parentListener?.dispose(), this._token ? this._token instanceof vt && this._token.dispose() : this._token = _t.None;
		}
	}, t1 = class {
		constructor() {
	...
	const Lt = new t1(), n1 = new t1(), r1 = new t1(), Gi = new Array(230), ji = Object.create(null), Xi = Object.create(null), wn = [];
	for (let e = 0; e <= 193; e++) wn[e] = -1;
	(function() {
		const t = [
			[
				1,
		...
		for (const s of t) {
			...
			if (r[l] || (r[l] = !0, ji[o] = l, Xi[o.toLowerCase()] = l, a && (wn[l] = u)), !n[u]) {
				if (n[u] = !0, !c) throw new Error(`String representation missing for key code ${u} around scan code ${o}`);
				Lt.define(u, c), n1.define(u, d || c), r1.define(u, f || d || c);
			}
			m && (Gi[m] = u);
		}
	})();
	...
	(function(e) {
		...
		e.fromString = n;
		function r(o) {
			return n1.keyCodeToStr(o);
		}
		e.toUserSettingsUS = r;
		function s(o) {
			return r1.keyCodeToStr(o);
		}
		e.toUserSettingsGeneral = s;
		function a(o) {
			return n1.strToKeyCode(o) || r1.strToKeyCode(o);
		}
	...
	let Fe;
	const s1 = globalThis.vscode;
	if (typeof s1 < "u" && typeof s1.process < "u") {
		const e = s1.process;
		Fe = {
			get platform() {
				return e.platform;
		...
		get platform() {
			return Ge ? "win32" : li ? "darwin" : "linux";
		},
		get arch() {},
		get env() {
			return {};
	...
	const Nt = Fe.cwd, Yi = Fe.env, Ji = Fe.platform, Zi = 65, Ki = 97, ea = 90, ta = 122, Re = 46, X = 47, te = 92, ge = 58, na = 63;
	...
	function ra(e, t) {
		if (e === null || typeof e != "object") throw new _n(t, "Object", e);
	}
	function G(e, t) {
		if (typeof e != "string") throw new _n(t, "string", e);
	}
	const ye = Ji === "win32";
	function V(e) {
		return e === X || e === te;
	...
	function St(e, t, n, r) {
		...
		for (let c = 0; c <= e.length; ++c) {
			...
			if (r(u)) {
				if (!(l === c - 1 || o === 1)) if (o === 2) {
					if (s.length < 2 || a !== 2 || s.charCodeAt(s.length - 1) !== Re || s.charCodeAt(s.length - 2) !== Re) {
						if (s.length > 2) {
	...
	const K = {
		resolve(...e) {
			...
			for (let s = e.length - 1; s >= -1; s--) {
				...
				} else t.length === 0 ? a = Nt() : (a = Yi[`=${t}`] || Nt(), (a === void 0 || a.slice(0, 2).toLowerCase() !== t.toLowerCase() && a.charCodeAt(2) === te) && (a = `${t}\\`));
				const l = a.length;
				let o = 0, u = "", c = !1;
				...
				else de(m) && a.charCodeAt(1) === ge && (u = a.slice(0, 2), o = 2, l > 2 && V(a.charCodeAt(2)) && (c = !0, o = 3));
		...
		normalize(e) {
			...
			else de(a) && e.charCodeAt(1) === ge && (r = e.slice(0, 2), n = 2, t > 2 && V(e.charCodeAt(2)) && (s = !0, n = 3));
			let l = n < t ? St(e.slice(n), !s, "\\", V) : "";
			if (l.length === 0 && !s && (l = "."), l.length > 0 && V(e.charCodeAt(t - 1)) && (l += "\\"), !s && r === void 0 && e.includes(":")) {
				if (l.length >= 2 && de(l.charCodeAt(0)) && l.charCodeAt(1) === ge) return `.\\${l}`;
				let o = e.indexOf(":");
				do
		...
		isAbsolute(e) {
			...
			return V(n) || t > 2 && de(n) && e.charCodeAt(1) === ge && V(e.charCodeAt(2));
		},
		join(...e) {
		...
		toNamespacedPath(e) {
			...
			if (t.charCodeAt(0) === te) {
				if (t.charCodeAt(1) === te) {
					...
					if (n !== na && n !== Re) return `\\\\?\\UNC\\${t.slice(2)}`;
				}
			} else if (de(t.charCodeAt(0)) && t.charCodeAt(1) === ge && t.charCodeAt(2) === te) return `\\\\?\\${t}`;
			return t;
		},
		dirname(e) {
			G(e, "path");
			...
			} else de(s) && e.charCodeAt(1) === ge && (n = t > 2 && V(e.charCodeAt(2)) ? 3 : 2, r = n);
			let a = -1, l = !0;
		...
		basename(e, t) {
			...
			let n = 0, r = -1, s = !0, a;
			if (e.length >= 2 && de(e.charCodeAt(0)) && e.charCodeAt(1) === ge && (n = 2), t !== void 0 && t.length > 0 && t.length <= e.length) {
				if (t === e) return "";
		...
		extname(e) {
			...
			e.length >= 2 && e.charCodeAt(1) === ge && de(e.charCodeAt(0)) && (t = r = 2);
			for (let o = e.length - 1; o >= t; --o) {
				...
				s === -1 && (a = !1, s = o + 1), u === Re ? n === -1 ? n = o : l !== 1 && (l = 1) : n !== -1 && (l = -1);
			}
		...
		parse(e) {
			...
			} else if (de(s) && e.charCodeAt(1) === ge) {
			...
			for (; c >= r; --c) {
				...
				o === -1 && (u = !1, o = c + 1), s === Re ? a === -1 ? a = c : m !== 1 && (m = 1) : a !== -1 && (m = -1);
			}
		...
		resolve(...e) {
			...
			for (let r = e.length - 1; r >= 0 && !n; r--) {
				const s = e[r];
				G(s, `paths[${r}]`), s.length !== 0 && (t = `${s}/${t}`, n = s.charCodeAt(0) === X);
			}
			if (!n) {
				const r = ia();
				t = `${r}/${t}`, n = r.charCodeAt(0) === X;
			}
			return t = St(t, !n, "/", i1), n ? `/${t}` : t.length > 0 ? t : ".";
		},
		normalize(e) {
			if (G(e, "path"), e.length === 0) return ".";
			const t = e.charCodeAt(0) === X, n = e.charCodeAt(e.length - 1) === X;
			return e = St(e, !t, "/", i1), e.length === 0 ? t ? "/" : n ? "./" : "." : (n && (e += "/"), t ? `/${e}` : e);
		},
		isAbsolute(e) {
			return G(e, "path"), e.length > 0 && e.charCodeAt(0) === X;
		},
		join(...e) {
			if (e.length === 0) return ".";
			const t = [];
		...
		relative(e, t) {
			...
			if (c === o) if (l > o) {
				if (t.charCodeAt(a + c) === X) return t.slice(a + c + 1);
				if (c === 0) return t.slice(a + c);
			} else s > o && (e.charCodeAt(n + c) === X ? u = c : c === 0 && (u = 0));
			let m = "";
		...
		basename(e, t) {
			...
			for (a = e.length - 1; a >= 0; --a) if (e.charCodeAt(a) === X) {
				if (!s) {
					n = a + 1;
					break;
				}
		...
		parse(e) {
			...
			for (; u >= r; --u) {
				...
				l === -1 && (o = !1, l = u + 1), m === Re ? s === -1 ? s = u : c !== 1 && (c = 1) : s !== -1 && (c = -1);
			}
			if (l !== -1) {
				const m = a === 0 && n ? 1 : a;
	...
	function ua(e, t) {
		...
		if (e.scheme && !aa.test(e.scheme)) throw new Error("[UriError]: Scheme contains illegal characters.");
		if (e.path) {
			if (e.authority) {
			...
			} else if (oa.test(e.path)) throw new Error("[UriError]: If a URI does not contain an authority component, then the path cannot begin with two slash characters (\"//\")");
		}
	}
	function ca(e, t) {
		return !e && !t ? "file" : e;
	...
	var a1 = class Vt {
		static isUri(t) {
			return t instanceof Vt ? !0 : !t || typeof t != "object" ? !1 : typeof t.authority == "string" && typeof t.fragment == "string" && typeof t.path == "string" && typeof t.query == "string" && typeof t.scheme == "string" && typeof t.fsPath == "string" && typeof t.with == "function" && typeof t.toString == "function";
		}
		constructor(t, n, r, s, a, l = !1) {
			typeof t == "object" ? (this.scheme = t.scheme || z, this.authority = t.authority || z, this.path = t.path || z, this.query = t.query || z, this.fragment = t.fragment || z) : (this.scheme = ca(t, l), this.authority = n || z, this.path = ha(this.scheme, r || z), this.query = s || z, this.fragment = a || z, ua(this, l));
		}
		get fsPath() {
			return l1(this, !1);
		}
		with(t) {
			...
			return n === void 0 ? n = this.scheme : n === null && (n = z), r === void 0 ? r = this.authority : r === null && (r = z), s === void 0 ? s = this.path : s === null && (s = z), a === void 0 ? a = this.query : a === null && (a = z), l === void 0 ? l = this.fragment : l === null && (l = z), n === this.scheme && r === this.authority && s === this.path && a === this.query && l === this.fragment ? this : new Ie(n, r, s, a, l);
		}
		static parse(t, n = !1) {
			const r = ma.exec(t);
			return r ? new Ie(r[2] || z, Rt(r[4] || z), Rt(r[5] || z), Rt(r[7] || z), Rt(r[9] || z), n) : new Ie(z, z, z, z, z);
		}
		static file(t) {
			let n = z;
			if (Ge && (t = t.replace(/\\/g, le)), t[0] === le && t[1] === le) {
				const r = t.indexOf(le, 2);
				r === -1 ? (n = t.substring(2), t = le) : (n = t.substring(2, r), t = t.substring(r) || le);
			}
			return new Ie("file", n, t, z, z);
		}
		static from(t, n) {
		...
		static joinPath(t, ...n) {
			...
			let r;
			return Ge && t.scheme === "file" ? r = Vt.file(K.join(l1(t, !0), ...n)).path : r = ne.join(t.path, ...n), t.with({ path: r });
		}
		toString(t = !1) {
			return o1(this, t);
		}
		...
		static revive(t) {
			if (t) {
				...
				{
					const n = new Ie(t);
					return n._formatted = t.external ?? null, n._fsPath = t._sep === Ln ? t.fsPath ?? null : null, n;
				}
			} else return t;
		}
	};
	const Ln = Ge ? 1 : void 0;
	var Ie = class extends a1 {
		constructor() {
			super(...arguments), this._formatted = null, this._fsPath = null;
		}
		get fsPath() {
			return this._fsPath || (this._fsPath = l1(this, !1)), this._fsPath;
		}
		toString(e = !1) {
			return e ? o1(this, !0) : (this._formatted || (this._formatted = o1(this, !1)), this._formatted);
		}
		toJSON() {
			const e = { $mid: 1 };
			return this._fsPath && (e.fsPath = this._fsPath, e._sep = Ln), this._formatted && (e.external = this._formatted), this.path && (e.path = this.path), this.scheme && (e.scheme = this.scheme), this.authority && (e.authority = this.authority), this.query && (e.query = this.query), this.fragment && (e.fragment = this.fragment), e;
		}
	};
	const Nn = {
		58: "%3A",
	...
	function fa(e) {
		...
		for (let n = 0; n < e.length; n++) {
			const r = e.charCodeAt(n);
			r === 35 || r === 63 ? (t === void 0 && (t = e.substr(0, n)), t += Nn[r]) : t !== void 0 && (t += e[n]);
		}
		return t !== void 0 ? t : e;
	}
	function l1(e, t) {
		let n;
		return e.authority && e.path.length > 1 && e.scheme === "file" ? n = `//${e.authority}${e.path}` : e.path.charCodeAt(0) === 47 && (e.path.charCodeAt(1) >= 65 && e.path.charCodeAt(1) <= 90 || e.path.charCodeAt(1) >= 97 && e.path.charCodeAt(1) <= 122) && e.path.charCodeAt(2) === 58 ? t ? n = e.path.substr(1) : n = e.path[1].toLowerCase() + e.path.substr(2) : n = e.path, Ge && (n = n.replace(/\//g, "\\")), n;
	}
	function o1(e, t) {
		const n = t ? fa : Sn;
		...
		return o && (r += "?", r += n(o, !1, !1)), u && (r += "#", r += t ? u : Sn(u, !1, !1)), r;
	}
	function Rn(e) {
		try {
	...
	function Rt(e) {
		return e.match(An) ? e.replace(An, (t) => Rn(t)) : e;
	}
	var ga = class ae extends I {
		constructor(t, n, r, s) {
		...
		static createWithDirection(t, n, r, s, a) {
			return a === 0 ? new ae(t, n, r, s) : new ae(r, s, t, n);
		}
	};
	...
	var ba = class {
		...
		register(e, t) {
			return this._tokenizationSupports.set(e, t), this.handleChange([e]), mt(() => {
		...
		registerFactory(e, t) {
			...
			const n = new wa(this, e, t);
			return this._factories.set(e, n), mt(() => {
	...
	Ae = class at {
		static fromValue(t) {
			switch (t) {
				case "comment": return at.Comment;
				case "imports": return at.Imports;
				case "region": return at.Region;
			}
			return new at(t);
		}
		constructor(t) {
	...
	Qe = Er, Qe.CtrlCmd = 2048, Qe.Shift = 1024, Qe.Alt = 512, Qe.WinCtrl = 256;
	function va() {
		return {
			editor: void 0,
			languages: void 0,
	...
	Dr = (Tr = Symbol.toStringTag, Symbol.iterator);
	Pr = class lt {
		constructor(t, n) {
			if (this[Tr] = "ResourceMap", t instanceof lt) this.map = new Map(t.map), this.toKey = n ?? lt.defaultToKey;
			else if (Na(t)) {
		...
		set(t, n) {
			return this.map.set(this.toKey(t), new La(t, n)), this;
		}
		get(t) {
		...
		next(e) {
			...
			do {
				...
				if (r === this._prevMatchStartIndex && s === this._prevMatchLength) {
					if (s === 0) {
						_i(e, t, this._searchRegex.lastIndex) > 65535 ? this._searchRegex.lastIndex += 2 : this._searchRegex.lastIndex += 1;
						continue;
					}
					return null;
				}
	...
	function g1(e, t, n, r, s) {
		if (t = Wr(t), s || (s = ht.first($r)), n.length > s.maxLen) {
			let c = e - s.maxLen / 2;
			return c < 0 ? c = 0 : r += c, n = n.substring(c, e + s.maxLen / 2), g1(e, t, n, r, s);
		}
	...
	var Ia = class {
		static computeUnicodeHighlights(e, t, n) {
			const r = n ? n.startLineNumber : 1, s = n ? n.endLineNumber : e.getLineCount(), a = new zr(t), l = a.getCandidateCodePoints();
			let o;
			l === "allNonBasicAscii" ? o = /* @__PURE__ */ new RegExp("[^\\t\\n\\r\\x20-\\x7E]", "g") : o = new RegExp(`${Va(Array.from(l))}`, "g");
			const u = new Pa(null, o), c = [];
			...
			e: for (let p = r, y = s; p <= y; p++) {
				...
				do
					if (h = u.next(v), h) {
						...
						_ > 0 && bt(v.charCodeAt(_ - 1)) && _--, N + 1 < L && bt(v.charCodeAt(N - 1)) && N++;
		...
		static computeUnicodeHighlightReason(e, t) {
			...
			switch (n.shouldHighlightNonBasicASCII(e, null)) {
				...
				case 3: {
					const r = e.codePointAt(0), s = n.ambiguousCharacters.getPrimaryConfusable(r), a = wt.getLocales().filter((l) => !wt.getInstance(new Set([...t.allowedLocales, l])).isAmbiguous(r));
					return {
						kind: 0,
	...
	function Va(e, t) {
		return `[${fi(e.map((n) => String.fromCodePoint(n)).join(""))}]`;
	}
	var zr = class {
		constructor(e) {
			this.options = e, this.allowedCodePoints = new Set(e.allowedCodePoints), this.ambiguousCharacters = wt.getInstance(new Set(e.allowedLocales));
		}
		getCandidateCodePoints() {
			if (this.options.nonBasicASCII) return "allNonBasicAscii";
			const e = /* @__PURE__ */ new Set();
			if (this.options.invisibleCharacters) for (const t of yt.codePoints) Or(String.fromCodePoint(t)) || e.add(t);
		...
		shouldHighlightNonBasicASCII(e, t) {
			...
			if (t) for (const a of t) {
				const l = a.codePointAt(0), o = Li(a);
				r = r || o, !o && !this.ambiguousCharacters.isAmbiguous(l) && !yt.isInvisibleCharacter(l) && (s = !0);
			}
			return !r && s ? 0 : this.options.invisibleCharacters && !Or(e) && yt.isInvisibleCharacter(n) ? 2 : this.options.ambiguousCharacters && this.ambiguousCharacters.isAmbiguous(n) ? 3 : 0;
		}
	};
	function Or(e) {
		return e === " " || e === `
` || e === "	";
	...
	d1 = class I1 {
		...
		filter(t) {
			return new I1((n) => this.iterate((r) => t(r) ? n(r) : !0));
		}
		map(t) {
			return new I1((n) => this.iterate((r) => n(t(r))));
		}
		findLast(t) {
			let n;
		...
		findLastMaxBy(t) {
			let n, r = !0;
			return this.iterate((s) => ((r || p1.isGreaterThan(t(s, n))) && (r = !1, n = s), !0)), n;
		}
	}, d1.empty = new d1((e) => {});
	var $ = class se {
		static fromTo(t, n) {
	...
	function Be(e, t, n = 0, r = e.length) {
		let s = n, a = r;
		for (; s < a;) {
	...
	function b1(e, t, n = 0, r = e.length) {
		let s = n, a = r;
		for (; s < a;) {
			const l = Math.floor((s + a) / 2);
			t(e[l]) ? a = l : s = l + 1;
	...
	var w1 = class Es {
		...
		findLastMonotonous(t) {
			...
			const n = Be(this._array, t, this._findLastMonotonousLastIdx);
			return this._findLastMonotonousLastIdx = n + 1, n === -1 ? void 0 : this._array[n];
		}
	};
	Gr = w1, Gr.assertInvariants = !1;
	var jr, q = class oe {
		static ofLength(t, n) {
			return new oe(t, t + n);
		}
	...
	jr = q, jr.compareByStart = Ye((e) => e.startLineNumber, Je);
	var Ct = class ze {
		constructor(t = []) {
		...
		addRange(t) {
			if (t.length === 0) return;
			const n = b1(this._normalizedRanges, (s) => s.endLineNumberExclusive >= t.startLineNumber), r = Be(this._normalizedRanges, (s) => s.startLineNumber <= t.endLineNumberExclusive) + 1;
		...
		contains(t) {
			const n = Ve(this._normalizedRanges, (r) => r.startLineNumber <= t);
			return !!n && n.endLineNumberExclusive > t;
		}
		intersects(t) {
			const n = Ve(this._normalizedRanges, (r) => r.startLineNumber < t.endLineNumberExclusive);
		...
		getUnion(t) {
			...
			return a !== null && n.push(a), new ze(n);
		}
		subtractFrom(t) {
			const n = b1(this._normalizedRanges, (l) => l.endLineNumberExclusive >= t.startLineNumber), r = Be(this._normalizedRanges, (l) => l.startLineNumber <= t.endLineNumberExclusive) + 1;
			if (n === r) return new ze([t]);
			const s = [];
			...
			return a < t.endLineNumberExclusive && s.push(new q(a, t.endLineNumberExclusive)), new ze(s);
		}
		toString() {
...
` && (this.lineStartOffsetByLineIdx.push(t + 1), t > 0 && e.charAt(t - 1) === "\r" ? this.lineEndOffsetByLineIdx.push(t - 1) : this.lineEndOffsetByLineIdx.push(t));
		...
		getPosition(e) {
			const t = Be(this.lineStartOffsetByLineIdx, (n) => n <= e);
		...
		getTransformer() {
			return this._transformer || (this._transformer = new Xa(this.getValue())), this._transformer;
		}
		getLineAt(e) {
		...
		constructor(e, t) {
			Hs(t >= 1), super(), this._getLineContent = e, this._lineCount = t;
		}
		getValueOfRange(e) {
...
` + this._getLineContent(e.endLineNumber).substring(0, e.endColumn - 1), t;
		...
		get length() {
			...
			return new xt(this._lineCount - 1, e.length);
		}
	}, kt = class extends Ya {
		constructor(e) {
		...
		static joinReplacements(t, n) {
			...
			return new Me(I.fromPositions(r, s), a);
		}
		static fromStringReplacement(t, n) {
			return new Me(n.getTransformer().getRange(t.replaceRange), t.newText);
		}
		static delete(t) {
			return new Me(t, "");
		}
...
`, `
`), s = wi(n, r), a = xt.ofText(n.substring(0, s)).addToPosition(this.range.getStartPosition()), l = r.substring(s);
			return new Me(I.fromPositions(a, this.range.getEndPosition()), l);
		}
...
`), r = this.text.replaceAll(`\r
`, `
`), s = yi(n, r), a = xt.ofText(n.substring(0, n.length - s)).addToPosition(this.range.getStartPosition()), l = r.substring(0, r.length - s);
			return new Me(I.fromPositions(this.range.getStartPosition(), a), l);
		}
		toString() {
		...
		static inverse(t, n, r) {
			...
			let a = 1, l = 1;
			for (const u of t) {
				const c = new Oe(new q(a, u.original.startLineNumber), new q(l, u.modified.startLineNumber));
				c.modified.isEmpty || s.push(c), a = u.original.endLineNumberExclusive, l = u.modified.endLineNumberExclusive;
			}
			const o = new Oe(new q(a, n + 1), new q(l, r + 1));
		...
		toRangeMapping() {
			...
			if (this.original.startLineNumber === 1 || this.modified.startLineNumber === 1) {
				...
				return new pe(new I(this.original.startLineNumber, 1, this.original.endLineNumberExclusive, 1), new I(this.modified.startLineNumber, 1, this.modified.endLineNumberExclusive, 1));
			} else return new pe(new I(this.original.startLineNumber - 1, Number.MAX_SAFE_INTEGER, this.original.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), new I(this.modified.startLineNumber - 1, Number.MAX_SAFE_INTEGER, this.modified.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER));
		}
		toRangeMapping2(t, n) {
			if (Qr(this.original.endLineNumberExclusive, t) && Qr(this.modified.endLineNumberExclusive, n)) return new pe(new I(this.original.startLineNumber, 1, this.original.endLineNumberExclusive, 1), new I(this.modified.startLineNumber, 1, this.modified.endLineNumberExclusive, 1));
			if (!this.original.isEmpty && !this.modified.isEmpty) return new pe(I.fromPositions(new W(this.original.startLineNumber, 1), Ue(new W(this.original.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), t)), I.fromPositions(new W(this.modified.startLineNumber, 1), Ue(new W(this.modified.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), n)));
			if (this.original.startLineNumber > 1 && this.modified.startLineNumber > 1) return new pe(I.fromPositions(Ue(new W(this.original.startLineNumber - 1, Number.MAX_SAFE_INTEGER), t), Ue(new W(this.original.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), t)), I.fromPositions(Ue(new W(this.modified.startLineNumber - 1, Number.MAX_SAFE_INTEGER), n), Ue(new W(this.modified.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), n)));
			throw new Z();
		}
	};
	function Ue(e, t) {
	...
	var Ke = class Bt extends qe {
		static fromRangeMappings(t) {
			return new Bt(q.join(t.map((n) => q.fromRangeInclusive(n.originalRange))), q.join(t.map((n) => q.fromRangeInclusive(n.modifiedRange))), t);
		}
		constructor(t, n, r) {
			super(t, n), this.innerChanges = r;
		}
		flip() {
			return new Bt(this.modified, this.original, this.innerChanges?.map((t) => t.flip()));
		}
		withInnerChangesFromLineRanges() {
			return new Bt(this.original, this.modified, [this.toRangeMapping()]);
		}
	}, pe = class V1 {
		static fromEdit(t) {
			const n = t.getNewRanges();
			return t.replacements.map((r, s) => new V1(r.range, n[s]));
		}
		...
		flip() {
			return new V1(this.modifiedRange, this.originalRange);
		}
		toTextEdit(t) {
			const n = t.getValueOfRange(this.modifiedRange);
			return new Ja(this.originalRange, n);
		}
	};
	function Yr(e, t, n, r = !1) {
		const s = [];
		for (const a of Ua(e.map((l) => Za(l, t, n)), (l, o) => l.original.intersectsOrTouches(o.original) || l.modified.intersectsOrTouches(o.modified))) {
			const l = a[0], o = a[a.length - 1];
			s.push(new Ke(l.original.join(o.original), l.modified.join(o.modified), a.map((u) => u.innerChanges[0])));
		}
		return ct(() => !r && s.length > 0 && (s[0].modified.startLineNumber !== s[0].original.startLineNumber || n.length.lineCount - s[s.length - 1].modified.endLineNumberExclusive !== t.length.lineCount - s[s.length - 1].original.endLineNumberExclusive) ? !1 : W1(s, (a, l) => l.original.startLineNumber - a.original.endLineNumberExclusive === l.modified.startLineNumber - a.modified.endLineNumberExclusive && a.original.endLineNumberExclusive < l.original.startLineNumber && a.modified.endLineNumberExclusive < l.modified.startLineNumber)), s;
	}
	function Za(e, t, n) {
		let r = 0, s = 0;
		return e.modifiedRange.endColumn === 1 && e.originalRange.endColumn === 1 && e.originalRange.startLineNumber + r <= e.originalRange.endLineNumber && e.modifiedRange.startLineNumber + r <= e.modifiedRange.endLineNumber && (s = -1), e.modifiedRange.startColumn - 1 >= n.getLineLength(e.modifiedRange.startLineNumber) && e.originalRange.startColumn - 1 >= t.getLineLength(e.originalRange.startLineNumber) && e.originalRange.startLineNumber <= e.originalRange.endLineNumber + s && e.modifiedRange.startLineNumber <= e.modifiedRange.endLineNumber + s && (r = 1), new Ke(new q(e.originalRange.startLineNumber + r, e.originalRange.endLineNumber + 1 + s), new q(e.modifiedRange.startLineNumber + r, e.modifiedRange.endLineNumber + 1 + s), [e]);
	}
	const Ka = 3;
	var e0 = class {
		computeDiff(e, t, n) {
			...
			for (const l of r.changes) {
				...
				let c = new Ke(o, u, l.charChanges?.map((m) => new pe(new I(m.originalStartLineNumber, m.originalStartColumn, m.originalEndLineNumber, m.originalEndColumn), new I(m.modifiedStartLineNumber, m.modifiedStartColumn, m.modifiedEndLineNumber, m.modifiedEndColumn))));
				a && (a.modified.endLineNumberExclusive === c.modified.startLineNumber || a.original.endLineNumberExclusive === c.original.startLineNumber) && (c = new Ke(a.original.join(c.original), a.modified.join(c.modified), a.innerChanges && c.innerChanges ? a.innerChanges.concat(c.innerChanges) : void 0), s.pop()), s.push(c), a = c;
			}
			return ct(() => W1(s, (l, o) => o.original.startLineNumber - l.original.endLineNumberExclusive === o.modified.startLineNumber - l.modified.endLineNumberExclusive && l.original.endLineNumberExclusive < o.original.startLineNumber && l.modified.endLineNumberExclusive < o.modified.startLineNumber)), new At(s, [], r.quitEarly);
		}
	};
	function Jr(e, t, n, r) {
		return new gn(e, t, n).ComputeDiff(r);
	}
	var Zr = class {
		constructor(e) {
			const t = [], n = [];
			for (let r = 0, s = e.length; r < s; r++) t[r] = v1(e[r], 1), n[r] = L1(e[r], 1);
	...
	function n0(e) {
		...
		for (let r = 1, s = e.length; r < s; r++) {
			...
			Math.min(l, o) < Ka ? (n.originalLength = a.originalStart + a.originalLength - n.originalStart, n.modifiedLength = a.modifiedStart + a.modifiedLength - n.modifiedStart) : (t.push(a), n = a);
		}
		return t;
	}
	var _1 = class Ds {
		...
		computeDiff() {
			...
			const e = Jr(this.original, this.modified, this.continueLineDiff, this.shouldMakePrettyDiff), t = e.changes, n = e.quitEarly;
			if (this.shouldIgnoreTrimWhitespace) {
				const l = [];
				for (let o = 0, u = t.length; o < u; o++) l.push(_1.createFromDiffResult(this.shouldIgnoreTrimWhitespace, t[o], this.original, this.modified, this.continueCharDiff, this.shouldComputeCharChanges, this.shouldPostProcessCharChanges));
				return {
					quitEarly: n,
					changes: l
				};
			...
			for (let l = -1, o = t.length; l < o; l++) {
				...
				u && (r.push(_1.createFromDiffResult(this.shouldIgnoreTrimWhitespace, u, this.original, this.modified, this.continueCharDiff, this.shouldComputeCharChanges, this.shouldPostProcessCharChanges)), s += u.originalLength, a += u.modifiedLength);
			}
			return {
				quitEarly: n,
				changes: r
		...
		_pushTrimWhitespaceCharChange(e, t, n, r, s, a, l) {
			...
			let o;
			this.shouldComputeCharChanges && (o = [new Mt(t, n, t, r, s, a, s, l)]), e.push(new _1(t, t, s, s, o));
		}
	...
	var et, N1, He = class B1 {
		static trivial(t, n) {
			return new B1([new re($.ofLength(t.length), $.ofLength(n.length))], !1);
		}
		static trivialTimedOut(t, n) {
			return new B1([new re($.ofLength(t.length), $.ofLength(n.length))], !0);
		}
		constructor(t, n) {
		...
		static invert(t, n) {
			const r = [];
			return Ha(t, (s, a) => {
				r.push(be.fromOffsetPairs(s ? s.getEndExclusives() : _e.zero, a ? a.getStarts() : new _e(n, (s ? s.seq2Range.endExclusive - s.seq1Range.endExclusive : 0) + n)));
			}), r;
		}
		...
		delta(t) {
			return t === 0 ? this : new Ts(this.offset1 + t, this.offset2 + t);
		}
		equals(t) {
			return this.offset1 === t.offset1 && this.offset2 === t.offset2;
		}
	};
	et = _e, et.zero = new et(0, 0), et.max = new et(Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER);
	var Et = class {
		isValid() {
			return !0;
		}
	};
	N1 = Et, N1.instance = new N1();
	var s0 = class {
		constructor(e) {
...
`);
	...
	es = A1, es.chrKeys = /* @__PURE__ */ new Map();
	var i0 = class {
		compute(e, t, n = Et.instance, r) {
			if (e.length === 0 || t.length === 0) return He.trivial(e, t);
			const s = new S1(e.length, t.length), a = new S1(e.length, t.length), l = new S1(e.length, t.length);
			for (let f = 0; f < e.length; f++) for (let b = 0; b < t.length; b++) {
				if (!n.isValid()) return He.trivialTimedOut(e, t);
			...
			return m(-1, -1), o.reverse(), new He(o, !1);
		}
	}, ts = class {
		compute(e, t, n = Et.instance) {
			if (e.length === 0 || t.length === 0) return He.trivial(e, t);
			const r = e, s = t;
			function a(b, p) {
			...
			o.set(0, a(0, 0));
			const u = new l0();
			u.set(0, o.get(0) === 0 ? null : new ns(null, 0, 0, o.get(0)));
			let c = 0;
			e: for (;;) {
				if (l++, !n.isValid()) return He.trivialTimedOut(r, s);
				...
				for (c = b; c <= p; c += 2) {
					...
					if (u.set(c, N !== L ? new ns(k, L, _, N - L) : k), o.get(c) === r.length && o.get(c) - c === s.length) break e;
				}
			}
			let m = u.get(c);
			const h = [];
		...
		getBoundaryScore(e) {
			const t = is(e > 0 ? this.elements[e - 1] : -1), n = is(e < this.elements.length ? this.elements[e] : -1);
			if (t === 7 && n === 8) return 0;
			if (t === 8) return 150;
			let r = 0;
			return t !== n && (r += 10, t === 0 && n === 1 && (r += 1)), r += ss(t), r += ss(n), r;
		}
		translateOffset(e, t = "right") {
			const n = Be(this.firstElementOffsetByLineIdx, (s) => s <= e), r = e - this.firstElementOffsetByLineIdx[n];
		...
		findWordContaining(e) {
			if (e < 0 || e >= this.elements.length || !We(this.elements[e])) return;
			let t = e;
			for (; t > 0 && We(this.elements[t - 1]);) t--;
			let n = e;
			for (; n < this.elements.length && We(this.elements[n]);) n++;
			return new $(t, n);
		}
		findSubWordContaining(e) {
			if (e < 0 || e >= this.elements.length || !We(this.elements[e])) return;
			let t = e;
			for (; t > 0 && We(this.elements[t - 1]) && !rs(this.elements[t]);) t--;
			let n = e;
			for (; n < this.elements.length && We(this.elements[n]) && !rs(this.elements[n]);) n++;
			return new $(t, n);
		}
		countLinesIn(e) {
		...
		extendToFullLines(e) {
			return new $(Ve(this.firstElementOffsetByLineIdx, (t) => t <= e.start) ?? 0, Ga(this.firstElementOffsetByLineIdx, (t) => e.endExclusive <= t) ?? this.elements.length);
		}
	};
	function We(e) {
	...
	function is(e) {
		return e === 10 ? 8 : e === 13 ? 7 : R1(e) ? 6 : e >= 97 && e <= 122 ? 0 : e >= 65 && e <= 90 ? 1 : e >= 48 && e <= 57 ? 2 : e === -1 ? 3 : e === 44 || e === 59 ? 5 : 4;
	}
	function u0(e, t, n, r, s, a) {
		let { moves: l, excludedChanges: o } = h0(e, t, n, a);
		if (!a.isValid()) return [];
		const u = m0(e.filter((c) => !o.has(c)), r, s, t, n, a);
		return $a(l, u), l = f0(l), l = l.filter((c) => {
			const m = c.original.toOffsetRange().slice(t).map((h) => h.trim());
			return m.join(`
`).length >= 15 && c0(m, (h) => h.length >= 2) >= 2;
		}), l = g0(e, l), l;
	}
	function c0(e, t) {
		let n = 0;
	...
	function h0(e, t, n, r) {
		const s = [], a = e.filter((u) => u.modified.isEmpty && u.original.length >= 3).map((u) => new A1(u.original, t, u)), l = new Set(e.filter((u) => u.original.isEmpty && u.modified.length >= 3).map((u) => new A1(u.modified, n, u))), o = /* @__PURE__ */ new Set();
		for (const u of a) {
			let c = -1, m;
			for (const h of l) {
			...
			if (c > .9 && m && (l.delete(m), s.push(new qe(u.range, m.range)), o.add(u.source), o.add(m.source)), !r.isValid()) return {
				moves: s,
				excludedChanges: o
			};
		}
	...
	function m0(e, t, n, r, s, a) {
		...
		const u = [];
		e.sort(Ye((d) => d.modified.startLineNumber, Je));
		for (const d of e) {
			let f = [];
		...
		u.sort(za(Ye((d) => d.modifiedLineRange.length, Je)));
		const c = new Ct(), m = new Ct();
		for (const d of u) {
			...
			for (const v of y.ranges) {
				...
				l.push(new qe(_, L)), c.addRange(L), m.addRange(_);
			}
		}
		l.sort(Ye((d) => d.original.startLineNumber, Je));
		const h = new w1(e);
		for (let d = 0; d < l.length; d++) {
			const f = l[d], b = h.findLastMonotonous((M) => M.original.startLineNumber <= f.original.startLineNumber), p = Ve(e, (M) => M.modified.startLineNumber <= f.modified.startLineNumber), y = Math.max(f.original.startLineNumber - b.original.startLineNumber, f.modified.startLineNumber - p.modified.startLineNumber), v = h.findLastMonotonous((M) => M.original.startLineNumber < f.original.endLineNumberExclusive), L = Ve(e, (M) => M.modified.startLineNumber < f.modified.endLineNumberExclusive), _ = Math.max(v.original.endLineNumberExclusive - f.original.endLineNumberExclusive, L.modified.endLineNumberExclusive - f.modified.endLineNumberExclusive);
			let N;
			for (N = 0; N < y; N++) {
				const M = f.original.startLineNumber - N - 1, w = f.modified.startLineNumber - N - 1;
				if (M > r.length || w > s.length || c.contains(w) || m.contains(M) || !as(r[M - 1], s[w - 1], a)) break;
			}
			...
			k > 0 && (m.addRange(new q(f.original.endLineNumberExclusive, f.original.endLineNumberExclusive + k)), c.addRange(new q(f.modified.endLineNumberExclusive, f.modified.endLineNumberExclusive + k))), (N > 0 || k > 0) && (l[d] = new qe(new q(f.original.startLineNumber - N, f.original.endLineNumberExclusive + k), new q(f.modified.startLineNumber - N, f.modified.endLineNumberExclusive + k)));
		}
		return l;
	}
	function as(e, t, n) {
		...
		const r = new ts().compute(new Pt([e], new I(1, 1, 1, e.length), !1), new Pt([t], new I(1, 1, 1, t.length), !1), n);
		let s = 0;
		...
		function l(u) {
			let c = 0;
			for (let m = 0; m < e.length; m++) R1(u.charCodeAt(m)) || c++;
			return c;
		}
	...
	function f0(e) {
		...
		e.sort(Ye((n) => n.original.startLineNumber, Je));
		const t = [e[0]];
	...
	function g0(e, t) {
		const n = new w1(e);
		return t = t.filter((r) => (n.findLastMonotonous((s) => s.original.startLineNumber < r.original.endLineNumberExclusive) || new qe(new q(1, 1), new q(1, 1))) !== Ve(e, (s) => s.modified.startLineNumber < r.modified.endLineNumberExclusive)), t;
	}
	function ls(e, t, n) {
		let r = n;
		return r = os(e, t, r), r = os(e, t, r), r = d0(e, t, r), r;
	}
	function os(e, t, n) {
	...
	function d0(e, t, n) {
		...
		for (let r = 0; r < n.length; r++) {
			...
			a.seq1Range.isEmpty ? n[r] = us(a, e, t, o, u) : a.seq2Range.isEmpty && (n[r] = us(a.swap(), t, e, u, o).swap());
		}
		return n;
	}
	...
	function y0(e, t, n) {
		...
		return Wa(r, (u, c, m) => {
			...
			const y = re.fromOffsetPairs(u ? u.getEndExclusives() : _e.zero, m ? m.getStarts() : _e.max), v = h.intersect(y);
	...
	var hs = class {
		...
		getBoundaryScore(e) {
			return 1e3 - ((e === 0 ? 0 : ms(this.lines[e - 1])) + (e === this.lines.length ? 0 : ms(this.lines[e])));
		}
		getText(e) {
...
`);
	...
	var _0 = class {
		constructor() {
			this.dynamicProgrammingDiffing = new i0(), this.myersDiffingAlgorithm = new ts();
		}
		computeDiff(e, t, n) {
			if (e.length <= 1 && qa(e, t, (N, k) => N === k)) return new At([], [], !1);
			if (e.length === 1 && e[0].length === 0 || t.length === 1 && t[0].length === 0) return new At([new Ke(new q(1, e.length + 1), new q(1, t.length + 1), [new pe(new I(1, 1, e.length, e[e.length - 1].length + 1), new I(1, 1, t.length, t[t.length - 1].length + 1))])], [], !1);
			const r = n.maxComputationTimeMs === 0 ? Et.instance : new s0(n.maxComputationTimeMs), s = !n.ignoreTrimWhitespace, a = /* @__PURE__ */ new Map();
			function l(N) {
				let k = a.get(N);
				return k === void 0 && (k = a.size, a.set(N, k)), k;
			}
			const o = e.map((N) => l(N.trim())), u = t.map((N) => l(N.trim())), c = new hs(o, e), m = new hs(u, t), h = c.length + m.length < 1700 ? this.dynamicProgrammingDiffing.compute(c, m, r, (N, k) => e[N] === t[k] ? t[k].length === 0 ? .1 : 1 + Math.log(1 + t[k].length) : .99) : this.myersDiffingAlgorithm.compute(c, m, r);
			let d = h.diffs, f = h.hitTimeout;
			d = ls(c, m, d), d = w0(c, m, d);
			const b = [], p = (N) => {
			...
			let y = 0, v = 0;
			for (const N of d) {
				ct(() => N.seq1Range.start - y === N.seq2Range.start - v), p(N.seq1Range.start - y), y = N.seq1Range.endExclusive, v = N.seq2Range.endExclusive;
				const k = this.refineDiff(e, t, N, r, s, n);
				k.hitTimeout && (f = !0);
				for (const M of k.mappings) b.push(M);
			}
			p(e.length - y);
			const L = Yr(b, new kt(e), new kt(t));
			let _ = [];
			return n.computeMoves && (_ = this.computeMoves(L, e, t, o, u, r, s, n)), ct(() => {
				function N(M, w) {
		...
		computeMoves(e, t, n, r, s, a, l, o) {
			return u0(e, t, n, r, s, a).map((u) => new Ba(u, Yr(this.refineDiff(t, n, new re(u.original.toOffsetRange(), u.modified.toOffsetRange()), a, l, o).mappings, new kt(t), new kt(n), !0)));
		}
		refineDiff(e, t, n, r, s, a) {
			const l = v0(n).toRangeMapping2(e, t), o = new Pt(e, l.originalRange, s), u = new Pt(t, l.modifiedRange, s), c = o.length + u.length < 500 ? this.dynamicProgrammingDiffing.compute(o, u, r) : this.myersDiffingAlgorithm.compute(o, u, r);
			let m = c.diffs;
			return m = ls(o, u, m), m = cs(o, u, m, (h, d) => h.findWordContaining(d)), a.extendToSubwords && (m = cs(o, u, m, (h, d) => h.findSubWordContaining(d), !0)), m = p0(o, u, m), m = y0(o, u, m), {
				mappings: m.map((h) => new pe(o.translateRange(h.seq1Range), u.translateRange(h.seq2Range))),
				hitTimeout: c.hitTimeout
			};
		}
	};
	function v0(e) {
		return new qe(new q(e.seq1Range.start + 1, e.seq1Range.endExclusive + 1), new q(e.seq2Range.start + 1, e.seq2Range.endExclusive + 1));
	}
	const fs = {
		getLegacy: () => new e0(),
		getDefault: () => new _0()
	...
	var g = class {
		constructor(e, t, n, r = 1) {
			this._rgbaBrand = void 0, this.r = Math.min(255, Math.max(0, e)) | 0, this.g = Math.min(255, Math.max(0, t)) | 0, this.b = Math.min(255, Math.max(0, n)) | 0, this.a = ve(Math.max(Math.min(1, r), 0), 3);
		}
		static equals(e, t) {
		...
		constructor(t, n, r, s) {
			this._hslaBrand = void 0, this.h = Math.max(Math.min(360, t), 0) | 0, this.s = ve(Math.max(Math.min(1, n), 0), 3), this.l = ve(Math.max(Math.min(1, r), 0), 3), this.a = ve(Math.max(Math.min(1, s), 0), 3);
		}
		static equals(t, n) {
		...
		static toRGBA(t) {
			...
			if (r === 0) l = o = u = s;
				...
				l = ot._hue2rgb(m, c, n + 1 / 3), o = ot._hue2rgb(m, c, n), u = ot._hue2rgb(m, c, n - 1 / 3);
			}
			return new g(Math.round(l * 255), Math.round(o * 255), Math.round(u * 255), a);
		}
	}, Dt = class Fs {
		constructor(t, n, r, s) {
			this._hsvaBrand = void 0, this.h = Math.max(Math.min(360, t), 0) | 0, this.s = ve(Math.max(Math.min(1, n), 0), 3), this.v = ve(Math.max(Math.min(1, r), 0), 3), this.a = ve(Math.max(Math.min(1, s), 0), 3);
		}
		static equals(t, n) {
		...
		static fromRGBA(t) {
			...
			let u;
			return l === 0 ? u = 0 : a === n ? u = ((r - s) / l % 6 + 6) % 6 : a === r ? u = (s - n) / l + 2 : u = (n - r) / l + 4, new Fs(Math.round(u * 60), o, a, t.a);
		}
		static toRGBA(t) {
		...
		get hsla() {
			return this._hsla ? this._hsla : Ce.fromRGBA(this.rgba);
		}
		get hsva() {
		...
		constructor(t) {
			...
			else if (t instanceof Ce) this._hsla = t, this.rgba = Ce.toRGBA(t);
			else if (t instanceof Dt) this._hsva = t, this.rgba = Dt.toRGBA(t);
			else throw new Error("Invalid color ctor argument");
			else throw new Error("Color needs a value");
		}
		equals(t) {
			return !!t && g.equals(this.rgba, t.rgba) && Ce.equals(this.hsla, t.hsla) && Dt.equals(this.hsva, t.hsva);
		}
		getRelativeLuminance() {
			...
			return ve(.2126 * t + .7152 * n + .0722 * r, 4);
		}
		...
		lighten(t) {
			return new ie(new Ce(this.hsla.h, this.hsla.s, this.hsla.l + this.hsla.l * t, this.hsla.a));
		}
		darken(t) {
			return new ie(new Ce(this.hsla.h, this.hsla.s, this.hsla.l - this.hsla.l * t, this.hsla.a));
		}
		...
		opposite() {
			return new ie(new g(255 - this.rgba.r, 255 - this.rgba.g, 255 - this.rgba.b, this.rgba.a));
		}
		mix(t, n = .5) {
			const r = Math.min(Math.max(n, 0), 1), s = this.rgba, a = t.rgba;
			return new ie(new g(s.r + (a.r - s.r) * r, s.g + (a.g - s.g) * r, s.b + (a.b - s.b) * r, s.a + (a.a - s.a) * r));
		}
		makeOpaque(t) {
			if (this.isOpaque() || t.rgba.a !== 1) return this;
			const { r: n, g: r, b: s, a } = this.rgba;
			return new ie(new g(t.rgba.r - a * (t.rgba.r - n), t.rgba.g - a * (t.rgba.g - r), t.rgba.b - a * (t.rgba.b - s), 1));
		}
		toString() {
	...
	Y = tt, Y.white = new Y(new g(255, 255, 255, 1)), Y.black = new Y(new g(0, 0, 0, 1)), Y.red = new Y(new g(255, 0, 0, 1)), Y.blue = new Y(new g(0, 0, 255, 1)), Y.green = new Y(new g(0, 255, 0, 1)), Y.cyan = new Y(new g(0, 255, 255, 1)), Y.lightgrey = new Y(new g(211, 211, 211, 1)), Y.transparent = new Y(new g(0, 0, 0, 0)), (function(e) {
		(function(t) {
			(function(n) {
				function r(p) {
	...
	function L0(e, t) {
		if (!e) return;
		const n = tt.Format.CSS.parseHex(t);
		if (n) return {
			range: e,
			color: C1(n.rgba.r, n.rgba.g, n.rgba.b, n.rgba.a)
		};
	}
	function ds(e, t, n) {
		if (!e || t.length !== 1) return;
		const r = gs(t[0].values());
		return {
			range: e,
			color: C1(r[0], r[1], r[2], n ? r[3] : 1)
		};
	}
	function ps(e, t, n) {
		if (!e || t.length !== 1) return;
		const r = gs(t[0].values()), s = new tt(new Ce(r[0], r[1] / 100, r[2] / 100, n ? r[3] : 1));
		return {
			range: e,
			color: C1(s.rgba.r, s.rgba.g, s.rgba.b, s.rgba.a)
		};
	}
	function rt(e, t) {
	...
	function N0(e) {
		const t = [], n = rt(e, /\b(rgb|rgba|hsl|hsla)(\([0-9\s,.\%]*\))|^(#)([A-Fa-f0-9]{3})\b|^(#)([A-Fa-f0-9]{4})\b|^(#)([A-Fa-f0-9]{6})\b|^(#)([A-Fa-f0-9]{8})\b|(?<=['"\s])(#)([A-Fa-f0-9]{3})\b|(?<=['"\s])(#)([A-Fa-f0-9]{4})\b|(?<=['"\s])(#)([A-Fa-f0-9]{6})\b|(?<=['"\s])(#)([A-Fa-f0-9]{8})\b/gm);
		if (n.length > 0) for (const r of n) {
			const s = r.filter((u) => u !== void 0), a = s[1], l = s[2];
			if (!l) continue;
			let o;
			a === "rgb" ? o = ds(nt(e, r), rt(l, /^\(\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*,\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*,\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*\)$/gm), !1) : a === "rgba" ? o = ds(nt(e, r), rt(l, /^\(\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*,\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*,\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*,\s*(0[.][0-9]+|[.][0-9]+|[01][.]|[01])\s*\)$/gm), !0) : a === "hsl" ? o = ps(nt(e, r), rt(l, /^\(\s*((?:360(?:\.0+)?|(?:36[0]|3[0-5][0-9]|[12][0-9][0-9]|[1-9]?[0-9])(?:\.\d+)?))\s*[\s,]\s*(100|\d{1,2}[.]\d*|\d{1,2})%\s*[\s,]\s*(100|\d{1,2}[.]\d*|\d{1,2})%\s*\)$/gm), !1) : a === "hsla" ? o = ps(nt(e, r), rt(l, /^\(\s*((?:360(?:\.0+)?|(?:36[0]|3[0-5][0-9]|[12][0-9][0-9]|[1-9]?[0-9])(?:\.\d+)?))\s*[\s,]\s*(100|\d{1,2}[.]\d*|\d{1,2})%\s*[\s,]\s*(100|\d{1,2}[.]\d*|\d{1,2})%\s*[\s,]\s*(0[.][0-9]+|[.][0-9]+|[01][.]0*|[01])\s*\)$/gm), !0) : a === "#" && (o = L0(nt(e, r), a + l)), o && t.push(o);
		}
		return t;
	}
	function S0(e) {
		return !e || typeof e.getValue != "function" || typeof e.positionAt != "function" ? [] : N0(e);
	}
	const R0 = /^-+|-+$/g, bs = 100, A0 = 5;
	function C0(e, t) {
		let n = [];
	...
	function k0(e, t) {
		...
		const s = xa(t.markSectionHeaderRegex), a = new RegExp(t.markSectionHeaderRegex, `gdm${s ? "s" : ""}`);
		if (gi(a)) return n;
		for (let l = 1; l <= r; l += bs - A0) {
			const o = Math.min(l + bs - 1, r), u = [];
...
`) + 1, L = m.index - v + 1, _ = b[b.length - 1], N = {
	...
	x1 = class ue {
		constructor(t, n) {
			this._onReturn = n, this._producerConsumer = new D0(), this._iterator = {
	...
	var F0 = class {
		...
		setValue(e, t) {
			return e = Te(e), t = Te(t), this.values[e] === t ? !1 : (this.values[e] = t, e - 1 < this.prefixSumValidIndex[0] && (this.prefixSumValidIndex[0] = e - 1), !0);
		}
		removeValues(e, t) {
			e = Te(e), t = Te(t);
		...
		$acceptNewModel(e) {
			this._models[e.url] = new q0(a1.parse(e.url), e.lines, e.EOL, e.versionId);
		}
		...
		getWordAtPosition(e, t) {
			const n = g1(e.column, Wr(t), this._lines[e.lineNumber - 1], 0);
		...
		constructor(t = null) {
			this._foreignModule = t, this._requestHandlerBrand = void 0, this._workerTextModelSyncServer = new B0();
		}
		dispose() {}
		async $ping() {
			return "pong";
		...
		async $computeDiff(t, n, r, s) {
			...
			return !a || !l ? null : qt.computeDiff(a, l, r, s);
		}
		static computeDiff(t, n, r, s) {
			const a = s === "advanced" ? fs.getDefault() : fs.getLegacy(), l = t.getLinesContent(), o = n.getLinesContent(), u = a.computeDiff(l, o, r), c = u.changes.length > 0 ? !1 : this._modelsAreIdentical(t, n);
			function m(h) {
				return h.map((d) => [
		...
		async $computeMoreMinimalEdits(t, n, r) {
			...
			for (let { range: u, text: c, eol: m } of n) {
				...
				if (Math.max(c.length, h.length) > qt._diffLimit) {
					a.push({
						range: u,
						text: c
					});
				...
				const d = Vi(h, c, r), f = s.offsetAt(I.lift(u).getStartPosition());
				for (const b of d) {
		...
		async $textualSuggest(t, n, r, s) {
			const a = new G1(), l = new RegExp(r, s), o = /* @__PURE__ */ new Set();
			e: for (const u of t) {
				const c = this._getModel(u);
				if (c) {
					for (const m of c.words(l)) if (!(m === n || !isNaN(Number(m))) && (o.add(m), o.size > qt._suggestionsLimit)) break e;
				}
			}
			return {
				words: Array.from(o),
		...
		async $navigateValueSet(t, n, r, s, a) {
			...
			return pn.INSTANCE.navigateValueSet(n, u, c, m, r);
		}
		$fmr(t, n) {
	...
	M1 = _s, M1._diffLimit = 1e5, M1._suggestionsLimit = 1e4, typeof importScripts == "function" && (globalThis.monaco = va());
	var vs, Ls = class U1 {
		static getChannel(t) {
			return t.getChannel(U1.CHANNEL_NAME);
		}
		static setChannel(t, n) {
			t.setChannel(U1.CHANNEL_NAME, n);
		}
	};
	vs = Ls, vs.CHANNEL_NAME = "editorWorkerHost";
	function U0(e) {
		let t;
		const n = Pi((r) => {
			const s = Ls.getChannel(r);
			return t = e({
